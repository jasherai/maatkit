...
OS:           linux
Start:        Thu Jun 11 20:27:14 2009
Finish:       Thu Jun 11 20:27:15 2009
...
17                                                    # ###########################################################################
18                                                    # DuplicateKeyFinder package $Revision: 3893 $
19                                                    # ###########################################################################
...
29                                                    
30                                                    sub new {
31             1                    1            29      my ( $class, %args ) = @_;
32             1                                  4      my $self = {};
33             1                                 17      return bless $self, $class;
34                                                    }
35                                                    
...
54            24                   24           125      my ( $self, $keys,  %args ) = @_;
55    ***     24     50                         104      die "I need a keys argument" unless $keys;
56            24                                139      my %keys = %$keys;  # Copy keys because we remove non-duplicates.
57            24                                 70      my $primary_key;
58            24                                 59      my @unique_keys;
59            24                                 54      my @normal_keys;
60            24                                 57      my @fulltext_keys;
61            24                                 55      my @dupes;
62                                                    
63                                                       KEY:
64            24                                 96      foreach my $key ( values %keys ) {
65                                                          # Save real columns before we potentially re-order them.  These are
66                                                          # columns we want to print if the key is a duplicate.
67            59                                236         $key->{real_cols} = $key->{colnames}; 
68                                                    
69                                                          # We use column lengths to compare keys.
70            59                               1147         $key->{len_cols}  = length $key->{colnames};
71                                                    
...
83            49    100                         201         my $is_fulltext = $key->{type} eq 'FULLTEXT' ? 1 : 0;
84            49    100    100                  364         if ( $args{ignore_order} || $is_fulltext  ) {
85            11                                 95            my $ordered_cols = join(',', sort(split(/,/, $key->{colnames})));
86            11                                 29            MKDEBUG && _d('Reordered', $key->{name}, 'cols from',
...
94            49    100                         206         my $push_to = $key->{is_unique} ? \@unique_keys : \@normal_keys;
95            49    100                         202         if ( !$args{ignore_structure} ) {
96            47    100                         167            $push_to = \@fulltext_keys if $is_fulltext;
97                                                             # TODO:
98                                                             # $push_to = \@hash_keys     if $is_hash;
99                                                             # $push_to = \@spatial_keys  if $is_spatial;
100                                                         }
101           49                                191         push @$push_to, $key; 
102                                                      }
103                                                   
104                                                      # Redundantly constrained unique keys are treated as normal keys.
105           24                                131      push @normal_keys, $self->unconstrain_keys($primary_key, \@unique_keys);
106                                                   
...
115           24    100                          97      if ( $primary_key ) {
116           10                                 21         MKDEBUG && _d('Comparing PRIMARY KEY to UNIQUE keys');
117           10                                 64         push @dupes,
118                                                            $self->remove_prefix_duplicates([$primary_key], \@unique_keys, %args);
119                                                   
120           10                                 27         MKDEBUG && _d('Comparing PRIMARY KEY to normal keys');
121           10                                 61         push @dupes,
122                                                            $self->remove_prefix_duplicates([$primary_key], \@normal_keys, %args);
123                                                      }
124                                                   
125           24                                 61      MKDEBUG && _d('Comparing UNIQUE keys to normal keys');
126           24                                137      push @dupes,
127                                                         $self->remove_prefix_duplicates(\@unique_keys, \@normal_keys, %args);
128                                                   
129           24                                 52      MKDEBUG && _d('Comparing normal keys');
130           24                                141      push @dupes,
...
135           24                                 71      MKDEBUG && _d('Comparing FULLTEXT keys');
136           24                                134      push @dupes,
...
142           24    100    100                  252      if ( $primary_key
      ***                   66                        
                           100                        
143                                                           && $args{clustered}
...
176            3                                 10            my $j_pcols = join(',', sort @{$fks[$j]->{parent_cols}} );
               3                                 16   
177                                                   
178   ***      3    100     66                   56            if ( $fks[$i]->{parent_tbl} eq $fks[$j]->{parent_tbl}
      ***                   66                        
...
235           92                   92           446      my ( $self, $left_keys, $right_keys, %args ) = @_;
236           92                                246      my @dupes;
237           92                                207      my $right_offset;
238           92                                208      my $last_left_key;
239           92                                283      my $last_right_key = scalar(@$right_keys) - 1;
240                                                   
...
244                                                   
245           92    100                         314      if ( $right_keys != $left_keys ) {
246                                                         # Right and left keys are different lists.
247                                                   
248            1                                  6         @$left_keys = sort { $a->{colnames} cmp $b->{colnames} }
              27                                155   
249           44                                172                       grep { defined $_; }
250                                                                       @$left_keys;
251           31                                119         @$right_keys = sort { $a->{colnames} cmp $b->{colnames} }
              48                                156   
252           44                                156                        grep { defined $_; }
253                                                                       @$right_keys;
254                                                   
...
259           44                                125         $right_offset = 0;
260                                                      }
261                                                      else {
262                                                         # Right and left keys are the same list.
263                                                   
264           17                                 63         @$left_keys = reverse sort { $a->{colnames} cmp $b->{colnames} }
              36                                118   
265           48                                185                       grep { defined $_; }
266                                                                       @$left_keys;
267                                                         
268                                                         # Last left key is its second-to-last key.
269                                                         # The very last left key will be used as a right key.
270           48                                155         $last_left_key = scalar(@$left_keys) - 2;
271                                                   
...
275           48                                122         $right_offset = 1;
276                                                      }
277                                                   
278                                                      LEFT_KEY:
279           92                                336      foreach my $left_index ( 0..$last_left_key ) {
280           41    100                         175         next LEFT_KEY unless defined $left_keys->[$left_index];
281                                                   
282                                                         RIGHT_KEY:
283           40                                159         foreach my $right_index ( $left_index+$right_offset..$last_right_key ) {
284   ***     36     50                         168            next RIGHT_KEY unless defined $right_keys->[$right_index];
285                                                   
286           36                                129            my $left_name      = $left_keys->[$left_index]->{name};
287           36                                129            my $left_cols      = $left_keys->[$left_index]->{colnames};
288           36                                115            my $left_len_cols  = $left_keys->[$left_index]->{len_cols};
289           36                                122            my $right_name     = $right_keys->[$right_index]->{name};
290           36                                123            my $right_cols     = $right_keys->[$right_index]->{colnames};
291           36                                118            my $right_len_cols = $right_keys->[$right_index]->{len_cols};
292                                                   
293           36                                 79            MKDEBUG && _d('Comparing left', $left_name, '(',$left_cols,')',
...
299           36    100                         171            if (    substr($left_cols,  0, $right_len_cols)
...
304           18    100    100                  121               if ( $args{exact_duplicates} && ($right_len_cols<$left_len_cols) ) {
305            1                                  3                  MKDEBUG && _d($right_name, 'not exact duplicate of', $left_name);
...
312           17    100                          74               if ( exists $right_keys->[$right_index]->{unique_col} ) {
313            1                                  3                  MKDEBUG && _d('Cannot remove', $right_name,
314                                                                     'because is constrains col',
315                                                                     $right_keys->[$right_index]->{cols}->[0]);
316            1                                  3                  next RIGHT_KEY;
317                                                               }
318                                                   
319           16                                 35               MKDEBUG && _d('Remove', $right_name);
320           16                                 41               my $reason;
321           16    100                          70               if ( $right_keys->[$right_index]->{unconstrained} ) {
322            3                                363                  $reason .= "Uniqueness of $right_name ignored because "
323                                                                     . $right_keys->[$right_index]->{constraining_key}->{name}
324                                                                     . " is a stronger constraint\n"; 
325                                                               }
326           16    100                          79               $reason .= $right_name
...
335                                                                  reason            => $reason,
336                                                               };
337           16                                 48               push @dupes, $dupe;
338           16                                 52               delete $right_keys->[$right_index];
339                                                   
340   ***     16     50                         113               $args{callback}->($dupe, %args) if $args{callback};
341                                                            }
342                                                            else {
343           18                                 39               MKDEBUG && _d($right_name, 'not left-prefix of', $left_name);
344           18                                 73               next LEFT_KEY;
345                                                            }
346                                                         } # RIGHT_KEY
347                                                      } # LEFT_KEY
348           92                                283      MKDEBUG && _d('No more keys');
349                                                   
350                                                      # Cleanup the lists: remove removed keys.
351           92                                297      @$left_keys  = grep { defined $_; } @$left_keys;
              53                                208   
352           92                                320      @$right_keys = grep { defined $_; } @$right_keys;
              72                                238   
353                                                   
354           92                                358      return @dupes;
355                                                   }
356                                                   
...
370            4                    4            27      my ( $self, $primary_key, $keys, %args ) = @_;
371   ***      4     50                          16      die "I need a primary_key argument" unless $primary_key;
372   ***      4     50                          15      die "I need a keys argument"        unless $keys;
373            4                                 13      my $pkcols = $primary_key->{colnames};
374            4                                  9      my @dupes;
375                                                   
376                                                      KEY:
377            4                                 16      for my $i ( 0 .. @$keys - 1 ) {
378            2                                  8         my $suffix = $keys->[$i]->{colnames};
379                                                         SUFFIX:
380            2                                 15         while ( $suffix =~ s/`[^`]+`,// ) {
381            1                                 12            my $len = min(length($pkcols), length($suffix));
...
409           24                   24           112      my ( $self, $primary_key, $unique_keys ) = @_;
410   ***     24     50                          93      die "I need a unique_keys argument" unless $unique_keys;
411           24                                 53      my %unique_cols;
412           24                                 55      my @unique_sets;
413           24                                 57      my %unconstrain;
414           24                                 55      my @unconstrained_keys;
415                                                   
416           24                                 48      MKDEBUG && _d('Unconstraining redundantly unique keys');
417                                                   
...
422           36    100                         138         next unless $unique_key; # primary key may be undefined
423           22                                 66         my $cols = $unique_key->{cols};
424           22    100                          81         if ( @$cols == 1 ) {
425           11                                 23            MKDEBUG && _d($unique_key->{name},'defines unique column:',$cols->[0]);
...
429   ***     11     50                          55            if ( !exists $unique_cols{$cols->[0]} ) {
430           11                                 37               $unique_cols{$cols->[0]}  = $unique_key;
431           11                                 54               $unique_key->{unique_col} = 1;
432                                                            }
433                                                         }
434                                                         else {
435           11                                 35            local $LIST_SEPARATOR = '-';
436           11                                 22            MKDEBUG && _d($unique_key->{name}, 'defines unique set:', @$cols);
437           11                                 63            push @unique_sets, { cols => $cols, key => $unique_key };
438                                                         }
439                                                      }
440                                                   
...
443                                                      UNIQUE_SET:
444           24                                 87      foreach my $unique_set ( @unique_sets ) {
445           11                                 26         my $n_unique_cols = 0;
446           11                                 42         COL:
447           11                                 31         foreach my $col ( @{$unique_set->{cols}} ) {
448           24    100                         100            if ( exists $unique_cols{$col} ) {
449            7                                 17               MKDEBUG && _d('Unique set', $unique_set->{key}->{name},
450                                                                  'has unique col', $col);
451   ***      7     50                          26               last COL if ++$n_unique_cols > 1;
452            7                                 28               $unique_set->{constraining_key} = $unique_cols{$col};
453                                                            }
454                                                         }
455           11    100    100                   89         if ( $n_unique_cols && $unique_set->{key}->{name} ne 'PRIMARY' ) {
456                                                            # Unique set is redundantly constrained.
457            6                                 18            MKDEBUG && _d('Will unconstrain unique set',
...
462            6                                 35            $unconstrain{$unique_set->{key}->{name}}
...
470           24                                111      for my $i ( 0..(scalar @$unique_keys-1) ) {
471           12    100                          65         if ( exists $unconstrain{$unique_keys->[$i]->{name}} ) {
472            6                                 13            MKDEBUG && _d('Unconstraining', $unique_keys->[$i]->{name});
473            6                                 24            $unique_keys->[$i]->{unconstrained} = 1;
474            6                                 37            $unique_keys->[$i]->{constraining_key}
...
479                                                      }
480                                                   
481           24                                 54      MKDEBUG && _d('No more keys');
482           24                                107      return @unconstrained_keys;
483                                                   }
484                                                   
485                                                   sub _d {
486            1                    1            23      my ($package, undef, $line) = caller 0;
487   ***      2     50                           7      @_ = map { (my $temp = $_) =~ s/\n/\n# /g; $temp; }
               2                                  9   
               2                                 11   
