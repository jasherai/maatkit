...
OS:           linux
Start:        Wed Jun 10 17:21:07 2009
Finish:       Wed Jun 10 17:21:07 2009
...
45                                                    sub crc32 {
46             1                    1             6      my ( $self, $string ) = @_;
47             1                                  3      my $poly = 0xEDB88320;
48             1                                  4      my $crc  = 0xFFFFFFFF;
49             1                                  9      foreach my $char ( split(//, $string) ) {
50            11                                 35         my $comp = ($crc ^ ord($char)) & 0xFF;
51            11                                 35         for ( 1 .. 8 ) {
52            88    100                         362            $comp = $comp & 1 ? $poly ^ ($comp >> 1) : $comp >> 1;
53                                                          }
54            11                                 45         $crc = (($crc >> 8) & 0x00FFFFFF) ^ $comp;
55                                                       }
56             1                                  7      return $crc ^ 0xFFFFFFFF;
57                                                    }
58                                                    
...
75             2                                  6      my $type   = '';
76             2                                  6      my $length = 0;
77             2                                  8      my $sql    = "SELECT $func('a')";
78             2                                  6      my $sth    = $dbh->prepare($sql);
79             2                                 11      eval {
80             2                                368         $sth->execute();
81             2                                 40         $type   = $sth->{mysql_type_name}->[0];
82             2                                 13         $length = $sth->{mysql_length}->[0];
83             2                                 10         MKDEBUG && _d($sql, $type, $length);
84    ***      2    100     66                   27         if ( $type eq 'bigint' && $length < 20 ) {
85             1                                  5            $type = 'int';
86                                                          }
87                                                       };
88             2                                 20      $sth->finish;
89             2                                 59      return ($type, $length);
90                                                    }
91                                                    
92                                                    # Options:
...
101           12                   12           194      my ( $self, %args ) = @_;
102           12                                 65      my ($alg, $vp, $dbh) = @args{ qw(algorithm vp dbh) };
103           12                                 22      my @choices = sort { $ALGOS{$a}->{pref} <=> $ALGOS{$b}->{pref} } keys %ALGOS;
              36                                155   
104           12    100    100                  127      die "Invalid checksum algorithm $alg"
105                                                         if $alg && !$ALGOS{$alg};
106                                                   
107                                                      # CHECKSUM is eliminated by lots of things...
108           11    100    100                  163      if (
                           100                        
                           100                        
...
114            5                                 16         @choices = grep { $_ ne 'CHECKSUM' } @choices;
              15                                 59   
115                                                      }
116                                                   
117                                                      # BIT_XOR isn't available till 4.1.1 either
118           11    100                          48      if ( !$vp->version_ge($dbh, '4.1.1') ) {
119            2                                  5         MKDEBUG && _d('Cannot use BIT_XOR algorithm');
120            2                                  7         @choices = grep { $_ ne 'BIT_XOR' } @choices;
               4                                 17   
121                                                      }
122                                                   
123                                                      # Choose the best (fastest) among the remaining choices.
124           11    100    100                   55      if ( $alg && grep { $_ eq $alg } @choices ) {
              20                                 96   
125                                                         # Honor explicit choices.
126            4                                  9         MKDEBUG && _d('User requested', $alg, 'algorithm');
127            4                                 29         return $alg;
128                                                      }
129                                                   
130                                                      # If the user wants a count, prefer something other than CHECKSUM, because it
131                                                      # requires an extra query for the count.
132   ***      7    100     66                   39      if ( $args{count} && grep { $_ ne 'CHECKSUM' } @choices ) {
               3                                 14   
133            1                                  3         MKDEBUG && _d('Not using CHECKSUM algorithm because COUNT desired');
134            1                                  4         @choices = grep { $_ ne 'CHECKSUM' } @choices;
               3                                 13   
135                                                      }
136                                                   
137            7                                 15      MKDEBUG && _d('Algorithms, in order:', @choices);
138            7                                 51      return $choices[0];
139                                                   }
140                                                   
141                                                   sub is_hash_algorithm {
142            3                    3            15      my ( $self, $algorithm ) = @_;
143   ***      3            66                   41      return $ALGOS{$algorithm} && $ALGOS{$algorithm}->{hash};
144                                                   }
145                                                   
146                                                   # Picks a hash function, in order of speed.
147                                                   sub choose_hash_func {
148            3                    3            23      my ( $self, %args ) = @_;
149            3                                 16      my @funcs = qw(CRC32 FNV_64 MD5 SHA1);
150            3    100                          16      if ( $args{func} ) {
...
158            4                                 17            my $sql = "SELECT $func('test-string')";
159            4                                  8            MKDEBUG && _d($sql);
160            4                                915            $args{dbh}->do($sql);
161            3                                 14            $result = $func;
162                                                         };
163   ***      4    100     66                   69         if ( $EVAL_ERROR && $EVAL_ERROR =~ m/failed: (.*?) at \S+ line/ ) {
164            1                                  8            $error .= qq{$func cannot be used because "$1"\n};
165            1                                 10            MKDEBUG && _d($func, 'cannot be used because', $1);
166                                                         }
167                                                      } while ( @funcs && !$result );
168                                                   
169   ***      3     50                          13      die $error unless $result;
170            3                                 42      return $result;
171                                                   }
172                                                   
...
182            2                                 12      my ( $dbh, $func ) = @args{qw(dbh func)};
183                                                   
184   ***      2     50                          13      die "$func never needs the BIT_XOR optimization"
185                                                         if $func =~ m/^(?:FNV_64|CRC32)$/i;
186                                                   
187            2                                  6      my $opt_slice = 0;
188            2                                  4      my $unsliced  = uc $dbh->selectall_arrayref("SELECT $func('a')")->[0]->[0];
189            2                                358      my $sliced    = '';
190            2                                  7      my $start     = 1;
191   ***      2     50                          23      my $crc_wid   = length($unsliced) < 16 ? 16 : length($unsliced);
192                                                   
193   ***      2            66                    7      do { # Try different positions till sliced result equals non-sliced.
194            5                                 10         MKDEBUG && _d('Trying slice', $opt_slice);
195            5                                436         $dbh->do('SET @crc := "", @cnt := 0');
196            5                                 39         my $slices = $self->make_xor_slices(
197                                                            query     => "\@crc := $func('a')",
198                                                            crc_wid   => $crc_wid,
199                                                            opt_slice => $opt_slice,
200                                                         );
201                                                   
202            5                                 21         my $sql = "SELECT CONCAT($slices) AS TEST FROM (SELECT NULL) AS x";
203            5                                 12         $sliced = ($dbh->selectrow_array($sql))[0];
204            5    100                        1124         if ( $sliced ne $unsliced ) {
...
210                                                   
211   ***      2     50                           9      if ( $sliced eq $unsliced ) {
212            2                                  5         MKDEBUG && _d('Slice', $opt_slice, 'works');
213            2                                 18         return $opt_slice;
214                                                      }
215                                                      else {
...
228           13                   13            94      my ( $self, %args ) = @_;
229           13                                 69      my ( $query, $crc_wid, $opt_slice )
...
235           29                                 92         my $len = $crc_wid - $start + 1;
236           29    100                         119         if ( $len > 16 ) {
237           16                                 47            $len = 16;
238                                                         }
239           29                                253         push @slices,
...
248   ***     13    100     66                  102      if ( defined $opt_slice && $opt_slice < @slices ) {
249            7                                 67         $slices[$opt_slice] =~ s/\@crc/\@crc := $query/;
250                                                      }
251                                                      else {
252            6                                 19         map { s/\@crc/$query/ } @slices;
              12                                 63   
253                                                      }
254                                                   
255           13                                107      return join(', ', @slices);
256                                                   }
257                                                   
...
267           20                   20           241      my ( $self, %args ) = @_;
268           20                                116      my ( $table, $quoter, $func )
269                                                         = @args{ qw(table quoter func) };
270                                                   
271           20           100                  144      my $sep = $args{sep} || '#';
272           20                                 82      $sep =~ s/'//g;
273           20           100                   71      $sep ||= '#';
274                                                   
275                                                      # This allows a simpler grep when building %cols below.
276           20                                 51      my %ignorecols = map { $_ => 1 } @{$args{ignorecols}};
               1                                  6   
              20                                 94   
277                                                   
...
281           98                                401      my %cols = map { lc($_) => 1 }
              99                                324   
282           11                                 40                 grep { !exists $ignorecols{$_} }
283           20    100                          93                 ($args{cols} ? @{$args{cols}} : @{$table->{cols}});
               9                                 38   
284           98                                380      my @cols =
285                                                         map {
286          228                                682            my $type = $table->{type_for}->{$_};
287           98                                388            my $result = $quoter->quote($_);
288           98    100    100                  873            if ( $type eq 'timestamp' ) {
                    100    100                        
                    100                               
289            6                                 19               $result .= ' + 0';
290                                                            }
...
300           20                                 81            $cols{$_}
301                                                         }
302           20                                 96         @{$table->{cols}};
303                                                   
304           20                                 66      my $query;
305           20    100                          77      if ( uc $func ne 'FNV_64' ) {
306                                                         # Add a bitmap of which nullable columns are NULL.
307           16                                 44         my @nulls = grep { $cols{$_} } @{$table->{null_cols}};
              78                                244   
              16                                 69   
308           16    100                          69         if ( @nulls ) {
309           24                                 92            my $bitmap = "CONCAT("
310            4                                 14               . join(', ', map { 'ISNULL(' . $quoter->quote($_) . ')' } @nulls)
311                                                               . ")";
312            4                                 21            push @cols, $bitmap;
313                                                         }
314                                                   
315           16    100                         115         $query = @cols > 1
...
322            4                                 25         $query = 'FNV_64(' . join(', ', @cols) . ')';
323                                                      }
324                                                   
325           20                                166      return $query;
326                                                   }
327                                                   
...
344           12                   12           225      my ( $self, %args ) = @_;
345           12                                 96      my @arg_names = qw(dbname tblname table quoter algorithm
346                                                           func crc_wid crc_type opt_slice);
347           12                                 42      foreach my $arg( @arg_names ) {
348   ***    108     50                         439         die "You must specify argument $arg" unless exists $args{$arg};
349                                                      }
350           12                                 76      my ( $dbname, $tblname, $table, $quoter, $algorithm,
351                                                           $func, $crc_wid, $crc_type, $opt_slice ) = @args{ @arg_names };
352   ***     12    100     66                  139      die "Invalid or missing checksum algorithm"
353                                                         unless $algorithm && $ALGOS{$algorithm};
354                                                   
355           11                                 24      my $result;
356                                                   
357           11    100                          47      if ( $algorithm eq 'CHECKSUM' ) {
358            1                                  5         return "CHECKSUM TABLE " . $quoter->quote($dbname, $tblname);
359                                                      }
360                                                   
361           10                                 61      my $expr = $self->make_row_checksum(%args);
362                                                   
363           10    100                          50      if ( $algorithm eq 'BIT_XOR' ) {
...
375            5    100                          21         if ( $crc_type =~ m/int$/ ) {
376            3                                 10            $result = "LOWER(CONV(BIT_XOR(CAST($expr AS UNSIGNED)), 10, 16)) AS crc ";
377                                                         }
378                                                         else {
379            2                                 15            my $slices = $self->make_xor_slices( query => $expr, %args );
380            2                                 10            $result = "LOWER(CONCAT($slices)) AS crc ";
381                                                         }
382                                                      }
383                                                      else {
...
395                                                         # important).
396            5    100                          28         if ( $crc_type =~ m/int$/ ) {
397            3                                 27            $result = "RIGHT(MAX("
...
407                                                         }
408                                                      }
409           10    100                          42      if ( $args{replicate} ) {
410            2                                 26         $result = "REPLACE /*PROGRESS_COMMENT*/ INTO $args{replicate} "
411                                                            . "(db, tbl, chunk, boundaries, this_cnt, this_crc) "
412                                                            . "SELECT ?, ?, /*CHUNK_NUM*/ ?, COUNT(*) AS cnt, $result";
413                                                      }
414                                                      else {
415            8    100                          54         $result = "SELECT "
416                                                            . ($args{buffer} ? 'SQL_BUFFER_RESULT ' : '')
417                                                            . "/*PROGRESS_COMMENT*//*CHUNK_NUM*/ COUNT(*) AS cnt, $result";
418                                                      }
419           10                                 99      return $result . "FROM /*DB_TBL*//*INDEX_HINT*//*WHERE*/";
420                                                   }
421                                                   
