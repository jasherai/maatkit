...
sub new {
   ...
   # Default, skeleton metrics data structure.
   my $metrics = {
      all => {             # metrics for all events
         'count' => 0,
      },
      'unique' => {        # metrics for unique events (fingerprints)
      },
   };
   ...
   bless {
      ...
      n_events_in_buffer => 0, # number of events currently in buffer
      n_event_buffers    => 0, # number of event buffers filled/calced
   }, $class;
}
...
sub record_event {
   ...
   $self->{n_events}++;
   $self->{n_events_in_buffer}++;
   push @buffered_events, $event;
   MKDEBUG && _d("$self->{n_events} events total, "
                 . "$self->{n_events_in_buffer} events in buffer");
   ...
   return if $self->{n_events_in_buffer} < $self->{buffer_n_events};
...
sub calc_metrics {
   ...
   foreach my $event ( @buffered_events ) {
      ...
      foreach my $metric ( keys %{ $self->{handlers} } ) {
         ...
         # Get shortcut to data store for this metric, for this fingerprint.
         my $m_ds = $fp_ds->{ $metric } ||= { };

         # ########################## #
         # Finally, calc this metric. #
         # ########################## #
         ...
         if ( $handler->{type} == METRIC_TYPE_NUMERIC ) {
            ...
            $m_ds->{total} += $metric_val if $handler->{total};
            
            if ( $handler->{min} ) {
               $m_ds->{min} = 0 if !defined $m_ds->{min};
               $m_ds->{min} = $metric_val if $metric_val < $m_ds->{min};
            }
            if ( $handler->{max} ) {
               $m_ds->{max} = 0 if !defined $m_ds->{min};
               $m_ds->{max} = $metric_val if $metric_val > $m_ds->{min};
            }
            if ( $handler->{avg} ) {
               my $avg = $m_ds->{total} / $fp_ds->{count};
               $m_ds->{avg} = defined $handler->{transform}
                                 ? $handler->{transform}->($avg)
                                 : $avg;
            }
         }
   ...
   if ( $self->{buffer_n_events} >= $self->{n_events_in_buffer} ) {
      @buffered_events = ();
