mysql>  explain select * from (select foo from(select 1 as foo from sakila.actor as actor_1 union select 1 from sakila.actor as actor_2) as der_1 join (select 1 as foo from sakila.actor as actor_3) as der_2 using(foo)) as der_3;
+----+--------------+------------+--------+---------------+---------+---------+------+------+-------------+
