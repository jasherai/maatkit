...
OS:           linux
Start:        Fri Jul 31 18:52:43 2009
Finish:       Fri Jul 31 18:52:43 2009
...
31             1                    1             6   use warnings FATAL => 'all';
               1                                  2   
               1                                  8   
32             1                    1             5   use English qw(-no_match_vars);
               1                                  2   
               1                                  8   
33                                                    
...
75                                                    sub make_event {
76            17                   17           682      my ( $self, $event ) = @_;
77    ***     17     50                          63      return unless $event;
78                                                    
79            17    100    100                  152      if ( !$event->{cmd} || !$event->{key} ) {
80             3                                  6         MKDEBUG && _d('Event has no cmd or key:', Dumper($event));
81             3                                 12         return;
82                                                       }
83                                                    
84            14    100                          60      if ( !$cmds{$event->{cmd}} ) {
...
91            13                                 73      $event->{arg}         = "$event->{cmd} $event->{key}";
92            13                                 52      $event->{fingerprint} = $self->fingerprint($event->{arg});
93            13                                 55      $event->{key_print}   = $self->fingerprint($event->{key});
94                                                    
...
100           13                                 59      map { $event->{"Memc_$_"} = 'No' } keys %cmds;
             143                                582   
101           13                                 68      $event->{"Memc_$event->{cmd}"} = 'Yes';  # Got this cmd.
102           13                                 40      $event->{Memc_error}           = 'No';  # A handler may change this.
103           13                                 40      $event->{Memc_miss}            = 'No';
104           13    100                          50      if ( $event->{res} ) {
105           11    100                          51         $event->{Memc_miss}         = 'Yes' if $event->{res} eq 'NOT_FOUND';
106                                                      }
107                                                      else {
...
114           13    100                          55      if ( $cmd_handler_for{$event->{cmd}} ) {
115            7                                 29         return $cmd_handler_for{$event->{cmd}}->($event);
116                                                      }
117                                                   
...
122                                                   sub fingerprint {
123           26                   26            93      my ( $self, $val ) = @_;
124           26                                148      $val =~ s/[0-9A-Fa-f]{16,}|\d+/?/g;
125           26                                109      return $val;
126                                                   }
127                                                   
...
142            2                    2             7      my ( $event ) = @_;
143                                                   
144                                                      # There should be a result for any storage cmd.   
145   ***      2     50                           9      if ( !$event->{res} ) {
146   ***      0                                  0         MKDEBUG && _d('No result for event:', Dumper($event));
147   ***      0                                  0         return;
148                                                      }
149                                                   
150   ***      2     50                          11      $event->{'Memc_Not_Stored'} = $event->{res} eq 'NOT_STORED' ? 'Yes' : 'No';
151   ***      2     50                          10      $event->{'Memc_Exists'}     = $event->{res} eq 'EXISTS'     ? 'Yes' : 'No';
152                                                   
153            2                                 10      return $event;
154                                                   }
155                                                   
...
166            5                    5            17      my ( $event ) = @_;
167                                                   
168                                                      # There should be a result for any retr cmd.   
169   ***      5     50                          21      if ( !$event->{res} ) {
170   ***      0                                  0         MKDEBUG && _d('No result for event:', Dumper($event));
171   ***      0                                  0         return;
172                                                      }
173                                                   
174            5    100                          23      $event->{'Memc_error'} = $event->{res} eq 'INTERRUPTED' ? 'Yes' : 'No';
175                                                   
176            5                                 25      return $event;
177                                                   }
178                                                   
