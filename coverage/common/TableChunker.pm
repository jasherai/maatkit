...
---------------------------- ------ ------ ------ ------ ------ ------ ------
...it/common/TableChunker.pm   93.9   70.2   73.7   93.1    0.0    4.8   83.7
TableChunker.t                 99.4   50.0   33.3  100.0    n/a   95.2   92.2
...
OS:           linux
Start:        Mon Jun 21 20:53:47 2010
Finish:       Mon Jun 21 20:53:47 2010

Run:          TableChunker.t
Perl version: 118.53.46.49.48.46.48
OS:           linux
Start:        Mon Jun 21 20:53:49 2010
Finish:       Mon Jun 21 20:53:51 2010
...
42             1                    1             6   use strict;
               1                                  2   
               1                                  5   
43             1                    1             6   use warnings FATAL => 'all';
               1                                  2   
               1                                  5   
44             1                    1             6   use English qw(-no_match_vars);
               ...
               1                                  4   
45                                                    
46             1                    1            14   use POSIX qw(ceil);
               1                                  4   
               1                                  8   
47             1                    1             9   use List::Util qw(min max);
               1                                  3   
               1                                 15   
48             1                    1             7   use Data::Dumper;
               1                                  2   
               1                                  8   
...
52                                                    
53    ***      1            50      1             7   use constant MKDEBUG => $ENV{MKDEBUG} || 0;
               1                                  2   
               1                                 16   
54                                                    
55                                                    sub new {
56    ***      1                    1      0      6      my ( $class, %args ) = @_;
57             1                                  6      foreach my $arg ( qw(Quoter MySQLDump) ) {
58    ***      2     50                          12         die "I need a $arg argument" unless $args{$arg};
59                                                       }
60             1                                  5      my $self = { %args };
...
78    ***      9                    9      0     89      my ( $self, %args ) = @_;
79             9                                 59      foreach my $arg ( qw(tbl_struct) ) {
80    ***      9     50                          77         die "I need a $arg argument" unless $args{$arg};
81                                                       }
82             9                                 45      my $tbl_struct = $args{tbl_struct};
83                                                    
84                                                       # See if there's an index that will support chunking.
85             9                                 35      my @possible_indexes;
86             9                                 33      foreach my $index ( values %{ $tbl_struct->{keys} } ) {
               9                                 77   
87                                                    
88                                                          # Accept only BTREE indexes.
89    ***     35     50                         224         next unless $index->{type} eq 'BTREE';
90                                                    
91                                                          # Reject indexes with prefixed columns.
92            35           100                  101         defined $_ && next for @{ $index->{col_prefixes} };
              35                                111   
              35                                347   
93                                                    
94                                                          # If exact, accept only unique, single-column indexes.
95            35    100                         205         if ( $args{exact} ) {
96    ***      4    100     66                   27            next unless $index->{is_unique} && @{$index->{cols}} == 1;
               1                                  9   
97                                                          }
98                                                    
99            32                                155         push @possible_indexes, $index;
100                                                      }
101                                                      MKDEBUG && _d('Possible chunk indexes in order:',
102            9                                 29         join(', ', map { $_->{name} } @possible_indexes));
103                                                   
104                                                      # Build list of candidate chunk columns.   
105            9                                 36      my $can_chunk_exact = 0;
106            9                                 34      my @candidate_cols;
107            9                                 42      foreach my $index ( @possible_indexes ) { 
108           32                                182         my $col = $index->{cols}->[0];
109                                                   
110                                                         # Accept only integer or real number type columns.
111   ***     32    100     66                  408         next unless ( $int_types{$tbl_struct->{type_for}->{$col}}
112                                                                       || $real_types{$tbl_struct->{type_for}->{$col}} );
113                                                   
114                                                         # Save the candidate column and its index.
115           24                                228         push @candidate_cols, { column => $col, index => $index->{name} };
116                                                      }
117                                                   
118            9    100    100                   81      $can_chunk_exact = 1 if $args{exact} && scalar @candidate_cols;
119                                                   
120            9                                 31      if ( MKDEBUG ) {
...
128            9                                 31      my @result;
129            9                                 27      MKDEBUG && _d('Ordering columns by order in tbl, PK first');
130   ***      9     50                          80      if ( $tbl_struct->{keys}->{PRIMARY} ) {
131            9                                 72         my $pk_first_col = $tbl_struct->{keys}->{PRIMARY}->{cols}->[0];
132            9                                 39         @result          = grep { $_->{column} eq $pk_first_col } @candidate_cols;
              24                                155   
133            9                                 47         @candidate_cols  = grep { $_->{column} ne $pk_first_col } @candidate_cols;
              24                                150   
134                                                      }
135            9                                 41      my $i = 0;
136            9                                 34      my %col_pos = map { $_ => $i++ } @{$tbl_struct->{cols}};
             107                                656   
               9                                 68   
137            9                                 38      push @result, sort { $col_pos{$a->{column}} <=> $col_pos{$b->{column}} }
               7                                 55   
138                                                                       @candidate_cols;
139                                                   
140            9                                 45      if ( MKDEBUG ) {
...
144                                                      }
145                                                   
146            9                                145      return ($can_chunk_exact, @result);
147                                                   }
148                                                   
...
170   ***     23                   23      0    444      my ( $self, %args ) = @_;
171           23                                278      my @required_args = qw(dbh db tbl tbl_struct chunk_col min max rows_in_range
172                                                                             chunk_size);
173           23                                125      foreach my $arg ( @required_args ) {
174          200    100                        1182         die "I need a $arg argument" unless defined $args{$arg};
175                                                      }
176           22                                134      MKDEBUG && _d('Calculate chunks for', Dumper(\%args));
177           22                                180      my ($dbh, $db, $tbl) = @args{@required_args};
178           22                                108      my $q        = $self->{Quoter};
179           22                                156      my $db_tbl   = $q->quote($db, $tbl);
180           22                               1233      my $col_type = $args{tbl_struct}->{type_for}->{$args{chunk_col}};
181           22                                 60      MKDEBUG && _d('chunk col type:', $col_type);
182                                                   
183           22                                145      my $range_func = $self->range_func_for($col_type);
184           22                                 82      my ($start_point, $end_point);
185           22                                 71      eval {
186           22                                169         $start_point = $self->value_to_number(
187                                                            value       => $args{min},
188                                                            column_type => $col_type,
189                                                            dbh         => $dbh,
190                                                         );
191           22                                157         $end_point  = $self->value_to_number(
192                                                            value       => $args{max},
193                                                            column_type => $col_type,
194                                                            dbh         => $dbh,
195                                                         );
196                                                      };
197   ***     22     50                         113      if ( $EVAL_ERROR ) {
...
222           22    100                         112      if ( !defined $start_point ) {
223            1                                  3         MKDEBUG && _d('Start point is undefined');
224            1                                  3         $start_point = 0;
225                                                      }
226   ***     22     50     33                  321      if ( !defined $end_point || $end_point < $start_point ) {
227   ***      0                                  0         MKDEBUG && _d('End point is undefined or before start point');
228   ***      0                                  0         $end_point = 0;
229                                                      }
230           22                                 59      MKDEBUG && _d("Actual chunk range:", $start_point, "to", $end_point);
231                                                   
232                                                      # Determine if we can include a zero chunk (col = 0).  If yes, then
233                                                      # make sure the start point is non-zero.
234           22                                 81      my $have_zero_chunk = 0;
235           22    100                         129      if ( $args{zero_chunk} ) {
236   ***      6    100     66                  131         if ( $start_point != $end_point && $start_point >= 0 ) {
237            4                                 20            MKDEBUG && _d('Zero chunking');
...
248            4                                 40            $start_point = $self->value_to_number(
249                                                               value       => $nonzero_val,
250                                                               column_type => $col_type,
251                                                               dbh         => $dbh,
252                                                            );
253            4                                 20            $have_zero_chunk = 1;
254                                                         }
255                                                         else {
256            2                                  8            MKDEBUG && _d("Cannot zero chunk");
257                                                         }
258                                                      }
259           22                                 64      MKDEBUG && _d("Using chunk range:", $start_point, "to", $end_point);
260                                                   
...
265           22                                190      my $interval = $args{chunk_size}
266                                                                   * ($end_point - $start_point)
267                                                                   / $args{rows_in_range};
268           22    100                         124      if ( $int_types{$col_type} ) {
269           17                                134         $interval = ceil($interval);
270                                                      }
271           22           100                  107      $interval ||= $args{chunk_size};
272   ***     22     50                         120      if ( $args{exact} ) {
273   ***      0                                  0         $interval = $args{chunk_size};
274                                                      }
275           22                                 60      MKDEBUG && _d('Chunk interval:', $interval, 'units');
276                                                   
...
287                                                      # rows.
288           22                                 75      my @chunks;
289           22                                151      my $col = $q->quote($args{chunk_col});
290           22    100                         890      if ( $start_point < $end_point ) {
291                                                   
...
297           21    100                         114         push @chunks, "$col = 0" if $have_zero_chunk;
298                                                   
299           21                                 73         my ( $beg, $end );
300           21                                 73         my $iter = 0;
301                                                         for ( my $i = $start_point; $i < $end_point; $i += $interval ) {
302           66                               1177            ( $beg, $end ) = $self->$range_func($dbh, $i, $interval, $end_point);
303                                                   
304                                                            # The first chunk.
305           65    100                        4562            if ( $iter++ == 0 ) {
306           20    100                         214               push @chunks,
307                                                                  ($have_zero_chunk ? "$col > 0 AND " : "")
308                                                                  ."$col < " . $q->quote_val($end);
309                                                            }
310                                                            else {
311                                                               # The normal case is a chunk in the middle of the range somewhere.
312           45                                333               push @chunks, "$col >= " . $q->quote_val($beg) . " AND $col < " . $q->quote_val($end);
313                                                            }
314           21                                 76         }
315                                                   
...
319           20                                156         my $nullable = $args{tbl_struct}->{is_nullable}->{$args{chunk_col}};
320           20                                 94         pop @chunks;
321           20    100                          98         if ( @chunks ) {
322           19                                136            push @chunks, "$col >= " . $q->quote_val($beg);
323                                                         }
324                                                         else {
325   ***      1     50                           5            push @chunks, $nullable ? "$col IS NOT NULL" : '1=1';
326                                                         }
327           20    100                         616         if ( $nullable ) {
328            2                                 14            push @chunks, "$col IS NULL";
329                                                         }
330                                                      }
331                                                      else {
...
353   ***      5                    5      0     60      my ( $self, %args ) = @_;
354            5                                 34      foreach my $arg ( qw(tbl_struct) ) {
355   ***      5     50                          49         die "I need a $arg argument" unless $args{$arg};
356                                                      }
357                                                   
...
361            5                                 58      my ($exact, @cols) = $self->find_chunk_columns(%args);
362            5                                 38      my $col = $cols[0]->{column};
363            5                                 30      my $idx = $cols[0]->{index};
364                                                   
...
391            1                                  6               $idx = $chunkable_col->{index};
392            1                                  5               last;
393                                                            }
394                                                         }
395                                                      }
...
399                                                         # col/index.
400            1                                  6         foreach my $chunkable_col ( @cols ) {
401            3    100                          26            if ( $wanted_idx eq $chunkable_col->{index} ) {
402                                                               # The wanted index has a chunkable column, so use it and overwrite
403                                                               # the defaults.
404            1                                  6               $col = $chunkable_col->{column};
405            1                                  5               $idx = $wanted_idx;
406            1                                 12               last;
407                                                            }
408                                                         }
409                                                      }
410                                                   
411            5                                 18      MKDEBUG && _d('First chunkable col/index:', $col, $idx);
412            5                                 92      return $col, $idx;
413                                                   }
414                                                   
...
421   ***      5                    5      0     71      my ( $self, %args ) = @_;
422            5                                 56      my @required_args = qw(dbh db tbl chunk_size);
423            5                                 28      foreach my $arg ( @required_args ) {
424   ***     20     50                         152         die "I need a $arg argument" unless $args{$arg};
425                                                      }
426            5                                 45      my ($dbh, $db, $tbl, $chunk_size) = @args{@required_args};
427            5                                 31      my $q  = $self->{Quoter};
428            5                                 26      my $du = $self->{MySQLDump};
429                                                   
430            5                                 25      my ($n_rows, $avg_row_length);
431                                                   
432            5                                 66      my ( $num, $suffix ) = $chunk_size =~ m/^(\d+)([MGk])?$/;
433            5    100                          40      if ( $suffix ) { # Convert to bytes.
                    100                               
434   ***      2      0                          22         $chunk_size = $suffix eq 'k' ? $num * 1_024
      ***            50                               
...
440                                                      }
441                                                      else {
442            1                                  4         die "Invalid chunk size $chunk_size; must be an integer "
443                                                            . "with optional suffix kMG";
444                                                      }
445                                                   
446            4    100    100                   57      if ( $suffix || $args{avg_row_length} ) {
447            3                                 36         my ($status) = $du->get_table_status($dbh, $q, $db, $tbl);
448            3                                768         $avg_row_length = $status->{avg_row_length};
449            3    100                          31         if ( !defined $n_rows ) {
450   ***      2     50                          46            $n_rows = $avg_row_length ? ceil($chunk_size / $avg_row_length) : undef;
451                                                         }
452                                                      }
453                                                   
454            4                                 59      return $n_rows, $avg_row_length;
455                                                   }
456                                                   
...
473   ***     12                   12      0    261      my ( $self, %args ) = @_;
474           12                                127      my @required_args = qw(dbh db tbl chunk_col tbl_struct);
475           12                                 90      foreach my $arg ( @required_args ) {
476   ***     60     50                         458         die "I need a $arg argument" unless $args{$arg};
477                                                      }
478           12                                184      my ($dbh, $db, $tbl, $col) = @args{@required_args};
479           12                                 61      my $where = $args{where};
480           12                                 76      my $q     = $self->{Quoter};
481                                                   
482           12                                 98      my $col_type       = $args{tbl_struct}->{type_for}->{$col};
483           12                                 84      my $col_is_numeric = $args{tbl_struct}->{is_numeric}->{$col};
484                                                   
485                                                      # Quote these once so we don't have to do it again. 
486           12                                107      my $db_tbl = $q->quote($db, $tbl);
487           12                                837      $col       = $q->quote($col);
488                                                   
489           12                                543      my ($min, $max);
490           12                                 56      eval {
491                                                         # First get the actual end points, whatever MySQL considers the
492                                                         # min and max values to be for this column.
493   ***     12     50                         185         my $sql = "SELECT MIN($col), MAX($col) FROM $db_tbl"
                    100                               
494                                                                 . ($args{index_hint} ? " $args{index_hint}" : "")
495                                                                 . ($where ? " WHERE ($where)" : '');
496           12                                 48         MKDEBUG && _d($dbh, $sql);
497           12                                 45         ($min, $max) = $dbh->selectrow_array($sql);
498           11                               3588         MKDEBUG && _d("Actual end points:", $min, $max);
499                                                   
...
504           11                                239         ($min, $max) = $self->get_valid_end_points(
505                                                            %args,
506                                                            dbh      => $dbh,
507                                                            db_tbl   => $db_tbl,
508                                                            col      => $col,
...
511                                                            max      => $max,
512                                                         );
513           10                                 57         MKDEBUG && _d("Valid end points:", $min, $max);
514                                                      };
515           12    100                         165      if ( $EVAL_ERROR ) {
516            2                                  8         die "Error getting min and max values for table $db_tbl "
...
522   ***     10     50                         142      my $sql = "EXPLAIN SELECT * FROM $db_tbl"
      ***            50                               
523                                                              . ($args{index_hint} ? " $args{index_hint}" : "")
524                                                              . ($where ? " WHERE $where" : '');
525           10                                 39      MKDEBUG && _d($sql);
526           10                                 36      my $expl = $dbh->selectrow_hashref($sql);
527                                                   
528                                                      return (
529           10                                347         min           => $min,
530                                                         max           => $max,
...
538   ***      4                    4      0     53      my ( $self, %args ) = @_;
539            4                                 33      foreach my $arg ( qw(database table chunks chunk_num query) ) {
540   ***     20     50                         104         die "I need a $arg argument" unless defined $args{$arg};
541                                                      }
542            4                                 11      MKDEBUG && _d('Injecting chunk', $args{chunk_num});
543            4                                 21      my $query   = $args{query};
544            4                                 28      my $comment = sprintf("/*%s.%s:%d/%d*/",
545                                                         $args{database}, $args{table},
546            4                                 28         $args{chunk_num} + 1, scalar @{$args{chunks}});
547            4                                 27      $query =~ s!/\*PROGRESS_COMMENT\*/!$comment!;
548            4                                 27      my $where = "WHERE (" . $args{chunks}->[$args{chunk_num}] . ')';
549   ***      4    100     66                   39      if ( $args{where} && grep { $_ } @{$args{where}} ) {
               5                                 26   
               4                                 26   
550            4                                 21         $where .= " AND ("
551            3                                 11            . join(" AND ", map { "($_)" } grep { $_ } @{$args{where}} )
               5                                 16   
               3                                 10   
552                                                            . ")";
553                                                      }
554            4                                 38      my $db_tbl     = $self->{Quoter}->quote(@args{qw(database table)});
555            4           100                  199      my $index_hint = $args{index_hint} || '';
556                                                   
557            4                                 11      MKDEBUG && _d('Parameters:',
558                                                         Dumper({WHERE => $where, DB_TBL => $db_tbl, INDEX_HINT => $index_hint}));
559            4                                 35      $query =~ s!/\*WHERE\*/! $where!;
560            4                                 17      $query =~ s!/\*DB_TBL\*/!$db_tbl!;
561            4                                 17      $query =~ s!/\*INDEX_HINT\*/! $index_hint!;
562            4                                 31      $query =~ s!/\*CHUNK_NUM\*/! $args{chunk_num} AS chunk_num,!;
563                                                   
...
579   ***     48                   48      0    446      my ( $self, %args ) = @_;
580           48                                314      my @required_args = qw(value column_type dbh);
581           48                                215      foreach my $arg ( @required_args ) {
582   ***    144     50                         899         die "I need a $arg argument" unless defined $args{$arg};
583                                                      }
584           48                                304      my ($val, $col_type, $dbh) = @args{@required_args};
585           48                                132      MKDEBUG && _d('Converting MySQL', $col_type, $val);
586                                                   
...
590                                                      # and hex values.
591           48                                383      my %mysql_conv_func_for = (
...
599           48                                143      my $num;
600           48    100                         524      if ( $col_type =~ m/(?:int|year|float|double|decimal)$/ ) {
                    100                               
      ***            50                               
...
606           11                                 44         my $func = $mysql_conv_func_for{$col_type};
607           11                                 48         my $sql = "SELECT $func(?)";
608           11                                 29         MKDEBUG && _d($dbh, $sql, $val);
609           11                                 27         my $sth = $dbh->prepare($sql);
610           11                               1735         $sth->execute($val);
611           11                                340         ($num) = $sth->fetchrow_array();
612                                                      }
...
617            9                                 72         $num = $self->timestampdiff($dbh, $val);
618                                                      }
619                                                      else {
620   ***      0                                  0         die "I don't know how to chunk $col_type\n";
621                                                      }
622           48                                142      MKDEBUG && _d('Converts to', $num);
623           48                                367      return $num;
624                                                   }
625                                                   
626                                                   sub range_func_for {
627   ***     22                   22      0    119      my ( $self, $col_type ) = @_;
628   ***     22     50                         115      return unless $col_type;
629           22                                 62      my $range_func;
630           22    100                         300      if ( $col_type =~ m/(?:int|year|float|double|decimal)$/ ) {
                    100                               
      ***            50                               
631           13                                 54         $range_func  = 'range_num';
632                                                      }
633                                                      elsif ( $col_type =~ m/^(?:timestamp|date|time)$/ ) {
634            5                                 17         $range_func  = "range_$col_type";
635                                                      }
636                                                      elsif ( $col_type eq 'datetime' ) {
637            4                                 18         $range_func  = 'range_datetime';
638                                                      }
639           22                                111      return $range_func;
640                                                   }
641                                                   
642                                                   # ###########################################################################
643                                                   # Range functions.
644                                                   # ###########################################################################
645                                                   sub range_num {
646   ***     38                   38      0    292      my ( $self, $dbh, $start, $interval, $max ) = @_;
647           38                                301      my $end = min($max, $start + $interval);
648                                                   
649                                                   
650                                                      # "Remove" scientific notation so the regex below does not make
651                                                      # 6.123456e+18 into 6.12345.
652   ***     38     50                         281      $start = sprintf('%.17f', $start) if $start =~ /e/;
653   ***     38     50                         276      $end   = sprintf('%.17f', $end)   if $end   =~ /e/;
654                                                   
655                                                      # Trim decimal places, if needed.  This helps avoid issues with float
656                                                      # precision differing on different platforms.
657           38                                202      $start =~ s/\.(\d{5}).*$/.$1/;
658           38                                208      $end   =~ s/\.(\d{5}).*$/.$1/;
659                                                   
660           38    100                         225      if ( $end > $start ) {
661           37                                286         return ( $start, $end );
662                                                      }
663                                                      else {
664            1                                  4         die "Chunk size is too small: $end !> $start\n";
665                                                      }
666                                                   }
667                                                   
668                                                   sub range_time {
669   ***      3                    3      0     15      my ( $self, $dbh, $start, $interval, $max ) = @_;
670            3                                 29      my $sql = "SELECT SEC_TO_TIME($start), SEC_TO_TIME(LEAST($max, $start + $interval))";
671            3                                  7      MKDEBUG && _d($sql);
672            3                                  6      return $dbh->selectrow_array($sql);
673                                                   }
674                                                   
675                                                   sub range_date {
676   ***     13                   13      0     73      my ( $self, $dbh, $start, $interval, $max ) = @_;
677           13                                124      my $sql = "SELECT FROM_DAYS($start), FROM_DAYS(LEAST($max, $start + $interval))";
678           13                                 31      MKDEBUG && _d($sql);
679           13                                 28      return $dbh->selectrow_array($sql);
680                                                   }
681                                                   
682                                                   sub range_datetime {
683   ***     12                   12      0     83      my ( $self, $dbh, $start, $interval, $max ) = @_;
684           12                                188      my $sql = "SELECT DATE_ADD('$EPOCH', INTERVAL $start SECOND), "
685                                                          . "DATE_ADD('$EPOCH', INTERVAL LEAST($max, $start + $interval) SECOND)";
686           12                                 34      MKDEBUG && _d($sql);
687           12                                 33      return $dbh->selectrow_array($sql);
688                                                   }
689                                                   
...
705   ***      9                    9      0     56      my ( $self, $dbh, $time ) = @_;
706            9                                 89      my $sql = "SELECT (COALESCE(TO_DAYS('$time'), 0) * 86400 + TIME_TO_SEC('$time')) "
707                                                         . "- TO_DAYS('$EPOCH 00:00:00') * 86400";
708            9                                 23      MKDEBUG && _d($sql);
709            9                                 25      my ( $diff ) = $dbh->selectrow_array($sql);
710            9                               2028      $sql = "SELECT DATE_ADD('$EPOCH', INTERVAL $diff SECOND)";
711            9                                 30      MKDEBUG && _d($sql);
712            9                                 26      my ( $check ) = $dbh->selectrow_array($sql);
713   ***      9     50                        1565      die <<"   EOF"
...
751   ***     11                   11      0    291      my ( $self, %args ) = @_;
752           11                                141      my @required_args = qw(dbh db_tbl col col_type);
753           11                                 85      foreach my $arg ( @required_args ) {
754   ***     44     50                         349         die "I need a $arg argument" unless $args{$arg};
755                                                      }
756           11                                112      my ($dbh, $db_tbl, $col, $col_type) = @args{@required_args};
757           11                                 87      my ($real_min, $real_max)           = @args{qw(min max)};
758                                                   
759                                                      # Common error message format in case there's a problem with
760                                                      # finding a valid min or max value.
761   ***     11     50                         168      my $err_fmt = "Error finding a valid %s value for table $db_tbl on "
...
768           11                                 52      my $valid_min = $real_min;
769   ***     11     50                          78      if ( defined $valid_min ) {
770                                                         # Get a valid min end point.
771           11                                 37         MKDEBUG && _d("Validating min end point:", $real_min);
772           11                                145         $valid_min = $self->_get_valid_end_point(
773                                                            %args,
774                                                            val      => $real_min,
775                                                            endpoint => 'min',
776                                                         );
777   ***     11    100     50                   92         die sprintf($err_fmt, 'minimum', 'minimum', ($real_min || "NULL"))
778                                                            unless defined $valid_min;
779                                                      }
780                                                      
781                                                      # Validate max value if it's not NULL.  NULL is valid.
782           10                                 46      my $valid_max = $real_max;
783   ***     10     50                          71      if ( defined $valid_max ) {
...
786           10                                 35         MKDEBUG && _d("Validating max end point:", $real_min);
787           10                                110         $valid_max = $self->_get_valid_end_point(
788                                                            %args,
789                                                            val      => $real_max,
790                                                            endpoint => 'max',
791                                                         );
792   ***     10     50      0                  103         die sprintf($err_fmt, 'maximum', 'maximum', ($real_max || "NULL"))
793                                                            unless defined $valid_max;
794                                                      }
795                                                   
796           10                                119      return $valid_min, $valid_max;
797                                                   }
798                                                   
799                                                   # Does the actual work for get_valid_end_points() for each end point.
800                                                   sub _get_valid_end_point {
801           21                   21           442      my ( $self, %args ) = @_;
802           21                                225      my @required_args = qw(dbh db_tbl col col_type);
803           21                                118      foreach my $arg ( @required_args ) {
804   ***     84     50                         622         die "I need a $arg argument" unless $args{$arg};
805                                                      }
806           21                                177      my ($dbh, $db_tbl, $col, $col_type) = @args{@required_args};
807           21                                109      my $val = $args{val};
808                                                   
809                                                      # NULL is valid.
810   ***     21     50                         133      return $val unless defined $val;
811                                                   
...
816           21    100                         249      my $validate = $col_type =~ m/time|date/ ? \&_validate_temporal_value
817                                                                   :                             undef;
818                                                   
819                                                      # If we cannot validate the value, assume it's valid.
820           21    100                         139      if ( !$validate ) {
821           10                                 32         MKDEBUG && _d("No validator for", $col_type, "values");
822           10                                107         return $val;
823                                                      }
824                                                   
825                                                      # Return the value if it's already valid.
826           11    100                          86      return $val if defined $validate->($dbh, $val);
827                                                   
828                                                      # The value is not valid so find the first one in the table that is.
829            6                                 20      MKDEBUG && _d("Value is invalid, getting first valid value");
830            6                                 85      $val = $self->get_first_valid_value(
831                                                         %args,
832                                                         val      => $val,
...
855   ***      6                    6      0    152      my ( $self, %args ) = @_;
856            6                                 73      my @required_args = qw(dbh db_tbl col validate endpoint);
857            6                                 39      foreach my $arg ( @required_args ) {
858   ***     30     50                         223         die "I need a $arg argument" unless $args{$arg};
859                                                      }
860            6                                 57      my ($dbh, $db_tbl, $col, $validate, $endpoint) = @args{@required_args};
861            6    100                          45      my $tries = defined $args{tries} ? $args{tries} : 5;
862            6                                 32      my $val   = $args{val};
863                                                   
864                                                      # NULL values are valid and shouldn't be passed to us.
865   ***      6     50                          39      return unless defined $val;
866                                                   
867                                                      # Prep a sth for fetching the next col val.
868   ***      6      0                          63      my $cmp = $endpoint =~ m/min/i ? '>'
      ***            50                               
869                                                              : $endpoint =~ m/max/i ? '<'
870                                                              :                        die "Invalid endpoint arg: $endpoint";
871   ***      6     50                         128      my $sql = "SELECT $col FROM $db_tbl "
      ***            50                               
...
877            6                                 21      my $sth = $dbh->prepare($sql);
878                                                   
...
881            6                                 74      my $last_val = $val;
882            6                                 44      while ( $tries-- ) {
883           16                               4875         $sth->execute($last_val);
884           16                                347         my ($next_val) = $sth->fetchrow_array();
885           16                                 69         MKDEBUG && _d('Next value:', $next_val, '; tries left:', $tries);
886   ***     16     50                         116         if ( !defined $next_val ) {
887   ***      0                                  0            MKDEBUG && _d('No more rows in table');
888   ***      0                                  0            last;
889                                                         }
890           16    100                         103         if ( defined $validate->($dbh, $next_val) ) {
891            5                                 18            MKDEBUG && _d('First valid value:', $next_val);
892            5                                 68            $sth->finish();
893            5                                155            return $next_val;
894                                                         }
895           11                                 88         $last_val = $next_val;
896                                                      }
897            1                                 18      $sth->finish();
898            1                                  5      $val = undef;  # no valid value found
899                                                   
900            1                                 32      return $val;
901                                                   }
902                                                   
...
907           47                   47           350      my ( $dbh, $val ) = @_;
908           47                                230      my $sql = "SELECT IF(TIME_FORMAT(?,'%H:%i:%s')=?, TIME_TO_SEC(?), TO_DAYS(?))";
909           47                                161      my $res;
910           47                                190      eval {
911           47                                150         MKDEBUG && _d($dbh, $sql, $val);
912           47                                155         my $sth = $dbh->prepare($sql);
913           47                              12184         $sth->execute($val, $val, $val, $val);
914           47                               1007         ($res) = $sth->fetchrow_array();
915           47                               1674         $sth->finish();
916                                                      };
917   ***     47     50                         321      if ( $EVAL_ERROR ) {
918   ***      0                                  0         MKDEBUG && _d($EVAL_ERROR);
919                                                      }
920           47                                437      return $res;
921                                                   }
922                                                   
923                                                   sub get_nonzero_value {
924   ***      4                    4      0    146      my ( $self, %args ) = @_;
925            4                                 47      my @required_args = qw(dbh db_tbl col col_type);
926            4                                 27      foreach my $arg ( @required_args ) {
927   ***     16     50                         123         die "I need a $arg argument" unless $args{$arg};
928                                                      }
929            4                                 36      my ($dbh, $db_tbl, $col, $col_type) = @args{@required_args};
930   ***      4     50                          29      my $tries = defined $args{tries} ? $args{tries} : 5;
...
937            4    100             3            63                     :                             sub { return $_[1]; };
               3                                 29   
938                                                   
939            4    100                          26      if ( !$is_nonzero->($dbh, $val) ) {  # quasi-double-negative, sorry
940            1                                  4         MKDEBUG && _d('Discarding zero value:', $val);
941   ***      1     50                          30         my $sql = "SELECT $col FROM $db_tbl "
      ***            50                               
...
947            1                                  3         my $sth = $dbh->prepare($sql);
948                                                   
949            1                                 11         my $last_val = $val;
950            1                                 10         while ( $tries-- ) {
951            1                                756            $sth->execute($last_val);
952            1                                 33            my ($next_val) = $sth->fetchrow_array();
953   ***      1     50                           8            if ( $is_nonzero->($dbh, $next_val) ) {
954            1                                  4               MKDEBUG && _d('First non-zero value:', $next_val);
955            1                                 18               $sth->finish();
956            1                                 49               return $next_val;
957                                                            }
...
12             1                    1            10   use Test::More;
               ...
               1                                  9   
13                                                    
14             1                    1            16   use TableParser;
               1                                  3   
               1                                 11   
15             1                    1            10   use TableChunker;
               1                                  3   
               1                                 16   
16             1                    1            15   use MySQLDump;
               1                                  3   
               1                                 12   
17             1                    1            11   use Quoter;
               1                                  6   
               1                                 15   
18             1                    1            13   use DSNParser;
               1                                  2   
               1                                 13   
19             1                    1            13   use Sandbox;
               1                                  3   
               1                                 10   
20             1                    1            10   use MaatkitTest;
               1                                  8   
               1                                 39   
21                                                    
22             1                                 13   my $dp = new DSNParser(opts=>$dsn_opts);
23             1                                232   my $sb = new Sandbox(basedir => '/tmp', DSNParser => $dp);
24             1                                 54   my $dbh = $sb->get_dbh_for('master');
25                                                    
26    ***      1     50                         378   if ( !$dbh ) {
...
31                                                    }
32                                                    
33             1                                279   $sb->create_dbs($dbh, ['test']);
34                                                    
35             1                                876   my $q  = new Quoter();
36             1                                 35   my $p  = new TableParser(Quoter => $q);
37             1                                 55   my $du = new MySQLDump();
38             1                                 34   my $c  = new TableChunker(Quoter => $q, MySQLDump => $du);
39             1                                  3   my $t;
40                                                    
41             1                                  6   $t = $p->parse( load_file('common/t/samples/sakila.film.sql') );
42             1                               1576   is_deeply(
...
74                                                    #);
75                                                    
76             1                                 14   $t = $p->parse( load_file('common/t/samples/pk_not_first.sql') );
77             1                               1522   is_deeply(
...
130                                                   # #############################################################################
131            1                                  4   SKIP: {
132            1                                  5      skip 'Sandbox master does not have the sakila database', 21
133   ***      1     50                           3         unless @{$dbh->selectcol_arrayref('SHOW DATABASES LIKE "sakila"')};
134                                                   
135            1                                510      my @chunks;
136                                                   
137            1                                 13      @chunks = $c->calculate_chunks(
138                                                         tbl_struct    => $t,
...
157                                                      );
158                                                   
159            1                                 12      @chunks = $c->calculate_chunks(
160                                                         tbl_struct    => $t,
...
195                                                      );
196                                                   
197            1                                 12      @chunks = $c->calculate_chunks(
198                                                         tbl_struct    => $t,
...
216                                                      );
217                                                   
218            1                                 13      $t = $p->parse( load_file('common/t/samples/daycol.sql') );
219                                                   
220            1                                680      @chunks = $c->calculate_chunks(
221                                                         tbl_struct    => $t,
222                                                         chunk_col     => 'a',
223                                                         min           => '2001-01-01',
224                                                         max           => '2002-01-01',
...
241                                                      );
242                                                   
243            1                                 11      $t = $p->parse( load_file('common/t/samples/date.sql') );
244            1                                632      @chunks = $c->calculate_chunks(
245                                                         tbl_struct    => $t,
246                                                         chunk_col     => 'a',
247                                                         min           => '2000-01-01',
248                                                         max           => '2005-11-26',
...
263                                                      );
264                                                   
265            1                                 13      @chunks = $c->calculate_chunks(
266                                                         tbl_struct    => $t,
267                                                         chunk_col     => 'a',
268                                                         min           => '0000-00-00',
269                                                         max           => '2005-11-26',
...
287            1                                545      @chunks = $c->calculate_chunks(
288                                                         tbl_struct    => $t,
289                                                         chunk_col     => 'a',
...
306                                                      );
307                                                   
308            1                                 14      @chunks = $c->calculate_chunks(
309                                                         tbl_struct    => $t,
310                                                         chunk_col     => 'a',
...
330            1                                625      @chunks = $c->calculate_chunks(
331                                                         tbl_struct    => $t,
332                                                         chunk_col     => 'a',
333                                                         min           => '00:59:19',
334                                                         max           => '09:03:15',
...
349                                                      );
350                                                   
351            1                                 12      $t = $p->parse( load_file('common/t/samples/doublecol.sql') );
352            1                                598      @chunks = $c->calculate_chunks(
353                                                         tbl_struct    => $t,
354                                                         chunk_col     => 'a',
355                                                         min           => '1',
356                                                         max           => '99.999',
...
371                                                      );
372                                                   
373            1                                 23      @chunks = $c->calculate_chunks(
374                                                         tbl_struct    => $t,
375                                                         chunk_col     => 'a',
376                                                         min           => '1',
377                                                         max           => '2',
...
392                                                   
393            1                                 12      eval {
394            1                                 14         @chunks = $c->calculate_chunks(
395                                                            tbl_struct    => $t,
396                                                            chunk_col     => 'a',
397                                                            min           => '1',
398                                                            max           => '2',
...
410                                                      );
411                                                   
412            1                                 11      $t = $p->parse( load_file('common/t/samples/floatcol.sql') );
413            1                                801      @chunks = $c->calculate_chunks(
414                                                         tbl_struct    => $t,
415                                                         chunk_col     => 'a',
416                                                         min           => '1',
417                                                         max           => '99.999',
...
432                                                      );
433                                                   
434            1                                 24      $t = $p->parse( load_file('common/t/samples/decimalcol.sql') );
435            1                                766      @chunks = $c->calculate_chunks(
436                                                         tbl_struct    => $t,
437                                                         chunk_col     => 'a',
438                                                         min           => '1',
439                                                         max           => '99.999',
...
455                                                   
456                                                      throws_ok(
457            1                    1            45         sub { $c->get_range_statistics(
458                                                               dbh        => $dbh,
459                                                               db         => 'sakila',
460                                                               tbl        => 'film',
...
467                                                            )
468                                                         },
469            1                                 57         qr/WHERE \(film_id>\)/,
470                                                         'Shows full SQL on error',
471                                                      );
472                                                   
473                                                      throws_ok(
474            1                    1            39         sub { $c->size_to_rows(
475                                                               dbh        => $dbh,
476                                                               db         => 'sakila',
477                                                               tbl        => 'film',
478                                                               chunk_size => 'foo'
479                                                            )
480                                                         },
481            1                                 42         qr/Invalid chunk size/,
...
493                                                         'Numeric size'
494                                                      );
495            1                                 20      my ($size) = $c->size_to_rows(
496                                                         dbh        => $dbh,
497                                                         db         => 'sakila',
498                                                         tbl        => 'film',
499                                                         chunk_size => '5k'
500                                                      );
501   ***      1            33                   23      ok($size >= 20 && $size <= 30, 'Convert bytes to rows');
502                                                   
503            1                                  6      my $avg;
504            1                                 12      ($size, $avg) = $c->size_to_rows(
505                                                         dbh        => $dbh,
506                                                         db         => 'sakila',
507                                                         tbl        => 'film',
508                                                         chunk_size => '5k'
...
515                                                      );
516                                                   
517            1                                 14      ($size, $avg) = $c->size_to_rows(
518                                                         dbh            => $dbh,
519                                                         db             => 'sakila',
520                                                         tbl            => 'film',
521                                                         chunk_size     => 5,
...
532                                                   # #############################################################################
533            1                                 18   $sb->load_file('master', 'common/t/samples/issue_47.sql');
534            1                             129533   $t = $p->parse( $du->get_create_table($dbh, $q, 'test', 'issue_47') );
535            1                               1329   my %params = $c->get_range_statistics(
536                                                      dbh        => $dbh,
537                                                      db         => 'test',
538                                                      tbl        => 'issue_47',
...
545            1                                  9   my @chunks;
546            1                                  4   eval {
547            1                                 28      @chunks = $c->calculate_chunks(
548                                                         dbh        => $dbh,
549                                                         tbl_struct => $t,
...
553                                                      );
554                                                   };
555            1                                 34   unlike($EVAL_ERROR, qr/Chunk size is too small/, 'Does not die chunking unsigned bitint (issue 47)');
556                                                   
557                                                   # #############################################################################
...
572                                                   );
573                                                   
574            1                                 17   $sb->load_file('master', 'common/t/samples/issue_8.sql');
575            1                             175863   $t = $p->parse( $du->get_create_table($dbh, $q, 'test', 'issue_8') );
576            1                               1906   my @candidates = $c->find_chunk_columns(tbl_struct=>$t);
577            1                                 35   is_deeply(
578                                                      \@candidates,
579                                                      [
580                                                         0,
...
589                                                   # #############################################################################
590            1                    1             7   use Data::Dumper;
               1                                  2   
               1                                  7   
591            1                                 30   $Data::Dumper::Indent    = 1;
...
614            6                    6            63      my ( $tbl, $range, $chunks, $zero_chunk ) = @_;
615   ***      6     50                          67      $zero_chunk = 1 unless defined $zero_chunk;
616            6                                 95      $t = $p->parse( $du->get_create_table($dbh, $q, 'issue_941', $tbl) );
617            6                               7784      %params = $c->get_range_statistics(
618                                                         dbh        => $dbh,
619                                                         db         => 'issue_941',
620                                                         tbl        => $tbl,
621                                                         chunk_col  => $tbl,
...
740                                                   # #############################################################################
741            1                                 27   $sb->load_file('master', 'mk-table-checksum/t/samples/issue_602.sql');
742            1                             186982   $t = $p->parse( $du->get_create_table($dbh, $q, 'issue_602', 't') );
743            1                               1428   %params = $c->get_range_statistics(
744                                                      dbh        => $dbh,
745                                                      db         => 'issue_602',
746                                                      tbl        => 't',
747                                                      chunk_col  => 'b',
...
764                                                   throws_ok(
765                                                      sub {
766            1                    1            46         @chunks = $c->calculate_chunks(
767                                                            dbh        => $dbh,
768                                                            db         => 'issue_602',
769                                                            tbl        => 't',
770                                                            tbl_struct => $t,
...
781            1                                 34   $t = $p->parse( $du->get_create_table($dbh, $q, 'issue_602', 't2') );
782                                                   throws_ok(
783                                                      sub {
784            1                    1            44         $c->get_range_statistics(
785                                                            dbh        => $dbh,
786                                                            db         => 'issue_602',
787                                                            tbl        => 't2',
788                                                            chunk_col  => 'b',
...
793                                                         );
794                                                      },
795            1                               1233      qr/Error finding a valid minimum value/,
...
801            1                                 41   %params = $c->get_range_statistics(
802                                                      dbh        => $dbh,
803                                                      db         => 'issue_602',
804                                                      tbl        => 't2',
805                                                      chunk_col  => 'b',
...
826                                                   # #############################################################################
827                                                   
828            1                                346   $dbh->do("insert into issue_602.t values ('12', '0000-00-00 00:00:00')");
829                                                   # Now we have:
...
875           12                                107      my $res = TableChunker::_validate_temporal_value($dbh, $t);
876           12                                129      is(
877                                                         $res,
878                                                         undef,
879                                                         "$t is invalid"
