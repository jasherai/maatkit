...
OS:           linux
Start:        Sat Aug 29 15:01:28 2009
Finish:       Sat Aug 29 15:01:28 2009
...
44                                                    sub parse_event {
45             4                    4           456      my ( $self, $fh, $misc, @callbacks ) = @_;
46             4                                 12      my $oktorun_here = 1;
47    ***      4     50                          18      my $oktorun      = $misc->{oktorun} ? $misc->{oktorun} : \$oktorun_here;
48             4                                 11      my $num_events   = 0;
49                                                    
50             4                                 19      local $INPUT_RECORD_SEPARATOR = ";\n#";
51             4                                 20      my $pos_in_log = tell($fh);
52             4                                  8      my $stmt;
53             4                                 13      my ($delim, $delim_len) = (undef, 0);
54                                                    
55                                                       EVENT:
56    ***      4            66                   90      while ( $$oktorun && defined($stmt = <$fh>) ) {
57            18                                 69         my @properties = ('pos_in_log', $pos_in_log);
58            18                                 53         my ($ts, $sid, $end, $type, $rest);
59            18                                 51         $pos_in_log = tell($fh);
60            18                                 98         $stmt =~ s/;\n#?\Z//;
61                                                    
62            18                                 47         my ( $got_offset, $got_hdr );
63            18                                 42         my $pos = 0;
64            18                                 63         my $len = length($stmt);
65            18                                 45         my $found_arg = 0;
66                                                          LINE:
67            18                                 91         while ( $stmt =~ m/^(.*)$/mg ) { # /g requires scalar match.
68            74                                205            $pos     = pos($stmt);  # Be careful not to mess this up!
69            74                                238            my $line = $1;          # Necessary for /g and pos() to work.
70            74    100                         344            $line    =~ s/$delim// if $delim;
71            74                                152            MKDEBUG && _d($line);
72                                                    
73            74    100                         275            if ( $line =~ m/^\/\*.+\*\/;/ ) {
74             6                                 13               MKDEBUG && _d('Comment line');
75             6                                 33               next LINE;
76                                                             }
77                                                     
78            68    100                         230            if ( $line =~ m/^DELIMITER/m ) {
79             3                                 20               my ( $del ) = $line =~ m/^DELIMITER (\S*)$/m;
80             3    100                          12               if ( $del ) {
81             2                                  5                  $delim_len = length $del;
82             2                                  7                  $delim     = quotemeta $del;
83             2                                  5                  MKDEBUG && _d('delimiter:', $delim);
84                                                                }
85                                                                else {
...
94                                                             }
95                                                    
96            65    100                         224            next LINE if $line =~ m/End of log file/;
97                                                    
98                                                             # Match the beginning of an event in the binary log.
99            63    100    100                  623            if ( !$got_offset && (my ( $offset ) = $line =~ m/$binlog_line_1/m) ) {
                    100    100                        
                    100                               
100           14                                 28               MKDEBUG && _d('Got the at offset line');
101           14                                 45               push @properties, 'offset', $offset;
102           14                                 90               $got_offset++;
103                                                            }
104                                                   
105                                                            # Match the 2nd line of binary log header, after "# at OFFSET".
106                                                            elsif ( !$got_hdr && $line =~ m/^#(\d{6}\s+\d{1,2}:\d\d:\d\d)/ ) {
107           14                                152               ($ts, $sid, $end, $type, $rest) = $line =~ m/$binlog_line_2/m;
108           14                                 42               MKDEBUG && _d('Got the header line; type:', $type, 'rest:', $rest);
109           14                                 67               push @properties, 'cmd', 'Query', 'ts', $ts, 'server_id', $sid,
110                                                                  'end_log_pos', $end;
111           14                                 77               $got_hdr++;
112                                                            }
113                                                   
...
119           20    100                         171               if ( my ( $db ) = $line =~ m/^use ([^;]+)/ ) {
                    100                               
120            2                                  4                  MKDEBUG && _d("Got a default database:", $db);
121            2                                 15                  push @properties, 'db', $db;
122                                                               }
123                                                   
...
131           17                                 36                  MKDEBUG && _d("Got some setting:", $setting);
132           17                                217                  push @properties, map { s/\s+//; lc } split(/,|\s*=\s*/, $setting);
              50                                126   
              50                                235   
133                                                               }
134                                                   
...
143           15                                 32               MKDEBUG && _d("Got the query/arg line at pos", $pos);
144           15                                 36               $found_arg++;
145   ***     15    100     66                  100               if ( $got_offset && $got_hdr ) {
146           13    100                          64                  if ( $type eq 'Xid' ) {
                    100                               
147            3                                 14                     my ($xid) = $rest =~ m/(\d+)/;
148            3                                 12                     push @properties, 'Xid', $xid;
149                                                                  }
150                                                                  elsif ( $type eq 'Query' ) {
151            9                                 80                     my ($i, $t, $c) = $rest =~ m/$binlog_line_2_rest/m;
152            9                                 49                     push @properties, 'Thread_id', $i, 'Query_time', $t,
153                                                                                       'error_code', $c;
154                                                                  }
155                                                                  else {
156   ***      1     50                          10                     die "Unknown event type $type"
...
173           15    100                          60               my $delim_len = ($pos == length($stmt) ? $delim_len : 0);
174           15                                 59               my $arg = substr($stmt, $pos - length($line) - $delim_len);
175                                                   
176           15    100                          84               $arg =~ s/$delim// if $delim; # Remove the delimiter.
177                                                   
...
182           15    100                          60               if ( $arg =~ m/^DELIMITER/m ) {
...
197                                                               }
198                                                   
199           15                                 47               $arg =~ s/;$//gm;  # Ensure ending ; are gone.
200           15                                 88               $arg =~ s/\s+$//;  # Remove trailing spaces and newlines.
201                                                   
202           15                                 70               push @properties, 'arg', $arg, 'bytes', length($arg);
203           15                                 41               last LINE;
204                                                            }
205                                                         } # LINE
206                                                   
207           18    100                          55         if ( $found_arg ) {
...
211           15                                123            my $event = { @properties };
212           15                                 48            foreach my $callback ( @callbacks ) {
213   ***     15     50                          56               last unless $event = $callback->($event);
214                                                            }
215           15                                287            ++$num_events;
216                                                         }
217                                                         else {
