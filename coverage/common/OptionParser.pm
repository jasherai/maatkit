...
---------------------------- ------ ------ ------ ------ ------ ------ ------
...it/common/OptionParser.pm   93.5   83.5   86.0   94.6    0.0   93.5   87.4
OptionParser.t                100.0   50.0   33.3  100.0    n/a    6.5   97.6
...
OS:           linux
Start:        Fri Jan 29 19:07:02 2010
Finish:       Fri Jan 29 19:07:02 2010

Run:          OptionParser.t
Perl version: 118.53.46.49.48.46.48
OS:           linux
Start:        Fri Jan 29 19:07:04 2010
Finish:       Fri Jan 29 19:07:05 2010
...
41                                                    
42                                                    sub new {
43    ***     37                   37      0    316      my ( $class, %args ) = @_;
44            37                                264      foreach my $arg ( qw(description) ) {
45    ***     37     50                         261         die "I need a $arg argument" unless $args{$arg};
46                                                       }
47            37                                408      my ($program_name) = $PROGRAM_NAME =~ m/([.A-Za-z-]+)$/;
48    ***     37            50                  196      $program_name ||= $PROGRAM_NAME;
49    ***     37            33                  388      my $home = $ENV{HOME} || $ENV{HOMEPATH} || $ENV{USERPROFILE} || '.';
      ***                   33                        
      ***                   50                        
50                                                    
51            37    100    100                 1602      my $self = {
                           100                        
...
106           11                   11            54      my ( $self, $file ) = @_;
107   ***     11            50                   52      $file ||= __FILE__;
108   ***     11     50                         514      open my $fh, '<', $file or die "Cannot open $file: $OS_ERROR";
109                                                   
110           11                                182      my %types = (
...
127                                                      # are reached...
128           11                                161      local $INPUT_RECORD_SEPARATOR = '';
129           11                              30829      while ( $para = <$fh> ) {
130         2991    100                       22394         next unless $para =~ m/^=head1 OPTIONS/;
131           10                                 47         last;
132                                                      }
133                                                   
134                                                      # ... then read any option rules...
135           11                                 99      while ( $para = <$fh> ) {
136           21    100                         150         last if $para =~ m/^=over/;
137           11                                 53         chomp $para;
138           11                                121         $para =~ s/\s+/ /g;
139           11                                291         $para =~ s/$POD_link_re/$1/go;
140           11                                 41         MKDEBUG && _d('Option rule:', $para);
141           11                                134         push @rules, $para;
142                                                      }
143                                                   
144           11    100                          58      die 'POD has no OPTIONS section' unless $para;
145                                                   
146                                                      # ... then start reading options.
147           10                                 45      do {
148          232    100                        1888         if ( my ($option) = $para =~ m/^=item --(.*)/ ) {
149          222                                725            chomp $para;
150          222                                623            MKDEBUG && _d($para);
151          222                                574            my %attribs;
152                                                   
153          222                               1180            $para = <$fh>; # read next paragraph, possibly attributes
154                                                   
155          222    100                        1014            if ( $para =~ m/: / ) { # attributes
156          158                                812               $para =~ s/\s+\Z//g;
157          228                               1156               %attribs = map {
158          158                                834                     my ( $attrib, $val) = split(/: /, $_);
159          228    100                        1104                     die "Unrecognized attribute for --$option: $attrib"
160                                                                        unless $attributes{$attrib};
161          227                               1253                     ($attrib, $val);
162                                                                  } split(/; /, $para);
163          157    100                         772               if ( $attribs{'short form'} ) {
164           27                                151                  $attribs{'short form'} =~ s/-//;
165                                                               }
166          157                                759               $para = <$fh>; # read next paragraph, probably short help desc
167                                                            }
168                                                            else {
169           64                                201               MKDEBUG && _d('Option has no attributes');
170                                                            }
171                                                   
172                                                            # Remove extra spaces and POD formatting (L<"">).
173          221                               1638            $para =~ s/\s+\Z//g;
174          221                               1577            $para =~ s/\s+/ /g;
175          221                               1216            $para =~ s/$POD_link_re/$1/go;
176                                                   
177                                                            # Take the first period-terminated sentence as the option's short help
178                                                            # description.
179          221                               1044            $para =~ s/\.(?:\n.*| [A-Z].*|\Z)//s;
180          221                                567            MKDEBUG && _d('Short help:', $para);
181                                                   
182          221    100                        1140            die "No description after option spec $option" if $para =~ m/^=item/;
183                                                   
184                                                            # Change [no]foo to foo and set negatable attrib. See issue 140.
185          220    100                        1190            if ( my ($base_option) =  $option =~ m/^\[no\](.*)/ ) {
186           11                                 36               $option = $base_option;
187           11                                 60               $attribs{'negatable'} = 1;
188                                                            }
189                                                   
190          220    100                        4224            push @specs, {
                    100                               
                    100                               
                    100                               
                    100                               
...
199                                                            };
200                                                         }
201          230                               1828         while ( $para = <$fh> ) {
202   ***    688     50                        3002            last unless $para;
203                                                   
...
207                                                   
208          688    100                        4606            if ( $para =~ m/^=head1/ ) {
209            8                                 28               $para = undef; # Can't 'last' out of a do {} block.
210            8                                 52               last;
211                                                            }
212          680    100                        4986            last if $para =~ m/^=item --/;
213                                                         }
214                                                      } while ( $para );
215                                                   
216            8    100                          37      die 'No valid specs in POD OPTIONS' unless @specs;
217                                                   
218            7                                 93      close $fh;
219            7                                 26      return @specs, @rules;
220                                                   }
221                                                   
...
234           36                   36           237      my ( $self, @specs ) = @_;
235           36                                119      my %disables; # special rule that requires deferred checking
236                                                   
237           36                                155      foreach my $opt ( @specs ) {
238          317    100                        1303         if ( ref $opt ) { # It's an option spec, not a rule.
239                                                            MKDEBUG && _d('Parsing opt spec:',
240          295                                698               map { ($_, '=>', $opt->{$_}) } keys %$opt);
241                                                   
242          295                               2395            my ( $long, $short ) = $opt->{spec} =~ m/^([\w-]+)(?:\|([^!+=]*))?/;
243   ***    295     50                        1346            if ( !$long ) {
244                                                               # This shouldn't happen.
245   ***      0                                  0               die "Cannot parse long option from spec $opt->{spec}";
246                                                            }
247          295                               1119            $opt->{long} = $long;
248                                                   
249   ***    295     50                        1497            die "Duplicate long option --$long" if exists $self->{opts}->{$long};
250          295                               1340            $self->{opts}->{$long} = $opt;
251                                                   
252          295    100                        1397            if ( length $long == 1 ) {
253            5                                 11               MKDEBUG && _d('Long opt', $long, 'looks like short opt');
254            5                                 20               $self->{short_opts}->{$long} = $long;
255                                                            }
256                                                   
257          295    100                        1047            if ( $short ) {
258   ***     60     50                         293               die "Duplicate short option -$short"
259                                                                  if exists $self->{short_opts}->{$short};
260           60                                257               $self->{short_opts}->{$short} = $long;
261           60                                237               $opt->{short} = $short;
262                                                            }
263                                                            else {
264          235                                899               $opt->{short} = undef;
265                                                            }
266                                                   
267          295    100                        1695            $opt->{is_negatable}  = $opt->{spec} =~ m/!/        ? 1 : 0;
268          295    100                        1593            $opt->{is_cumulative} = $opt->{spec} =~ m/\+/       ? 1 : 0;
269          295    100                        1739            $opt->{is_required}   = $opt->{desc} =~ m/required/ ? 1 : 0;
270                                                   
271          295           100                 1443            $opt->{group} ||= 'default';
272          295                               1593            $self->{groups}->{ $opt->{group} }->{$long} = 1;
273                                                   
274          295                               1063            $opt->{value} = undef;
275          295                               1060            $opt->{got}   = 0;
276                                                   
277          295                               1621            my ( $type ) = $opt->{spec} =~ m/=(.)/;
278          295                               1144            $opt->{type} = $type;
279          295                                686            MKDEBUG && _d($long, 'type:', $type);
280                                                   
281   ***    295     50    100                 2734            if ( $type && $type eq 'd' && !$self->{dp} ) {
      ***                   66                        
...
288          295    100    100                 2386            $opt->{spec} =~ s/=./=s/ if ( $type && $type =~ m/[HhAadzm]/ );
289                                                   
...
292                                                            # to set_defaults().
293          295    100                        1932            if ( (my ($def) = $opt->{desc} =~ m/default\b(?: ([^)]+))?/) ) {
294           66    100                         386               $self->{defaults}->{$long} = defined $def ? $def : 1;
295           66                                164               MKDEBUG && _d($long, 'default:', $def);
296                                                            }
297                                                   
298                                                            # Handle special behavior for --config.
299          295    100                        1307            if ( $long eq 'config' ) {
300            8                                 58               $self->{defaults}->{$long} = join(',', $self->get_defaults_files());
301                                                            }
302                                                   
303                                                            # Option disable another option if its desc says 'disable'.
304          295    100                        1678            if ( (my ($dis) = $opt->{desc} =~ m/(disables .*)/) ) {
305                                                               # Defer checking till later because of possible forward references.
306            4                                 17               $disables{$long} = $dis;
307            4                                 12               MKDEBUG && _d('Deferring check of disables rule for', $opt, $dis);
308                                                            }
309                                                   
310                                                            # Save the option.
311          295                               1596            $self->{opts}->{$long} = $opt;
312                                                         }
313                                                         else { # It's an option rule, not a spec.
314           22                                 62            MKDEBUG && _d('Parsing rule:', $opt); 
315           22                                 84            push @{$self->{rules}}, $opt;
              22                                124   
316           22                                124            my @participants = $self->_get_participants($opt);
317           20                                 81            my $rule_ok = 0;
318                                                   
319           20    100                         197            if ( $opt =~ m/mutually exclusive|one and only one/ ) {
320           10                                 35               $rule_ok = 1;
321           10                                 34               push @{$self->{mutex}}, \@participants;
              10                                 63   
322           10                                 43               MKDEBUG && _d(@participants, 'are mutually exclusive');
323                                                            }
324           20    100                         162            if ( $opt =~ m/at least one|one and only one/ ) {
325            5                                 17               $rule_ok = 1;
326            5                                 15               push @{$self->{atleast1}}, \@participants;
               5                                 27   
327            5                                 15               MKDEBUG && _d(@participants, 'require at least one');
328                                                            }
329           20    100                         113            if ( $opt =~ m/default to/ ) {
330            6                                 25               $rule_ok = 1;
331                                                               # Example: "DSN values in L<"--dest"> default to values
332                                                               # from L<"--source">."
333            6                                 41               $self->{defaults_to}->{$participants[0]} = $participants[1];
334            6                                 19               MKDEBUG && _d($participants[0], 'defaults to', $participants[1]);
335                                                            }
336           20    100                         111            if ( $opt =~ m/restricted to option groups/ ) {
337            1                                  3               $rule_ok = 1;
338            1                                  8               my ($groups) = $opt =~ m/groups ([\w\s\,]+)/;
339            1                                  5               my @groups = split(',', $groups);
...
344                                                            }
345                                                   
346   ***     20     50                         126            die "Unrecognized option rule: $opt" unless $rule_ok;
347                                                         }
348                                                      }
349                                                   
350                                                      # Check forward references in 'disables' rules.
351           34                                202      foreach my $long ( keys %disables ) {
...
365           27                   27           148      my ( $self, $str ) = @_;
366           27                                 88      my @participants;
367           27                                269      foreach my $long ( $str =~ m/--(?:\[no\])?([\w-]+)/g ) {
368           56    100                         326         die "Option --$long does not exist while processing rule $str"
369                                                            unless exists $self->{opts}->{$long};
370           53                                253         push @participants, $long;
371                                                      }
372           24                                 76      MKDEBUG && _d('Participants for', $str, ':', @participants);
373           24                                168      return @participants;
374                                                   }
375                                                   
...
391   ***      5                    5      0     27      my ( $self, %defaults ) = @_;
392            5                                 21      $self->{defaults} = {};
393            5                                 25      foreach my $long ( keys %defaults ) {
...
404            3                                 20      return $self->{defaults};
405                                                   }
406                                                   
407                                                   sub get_groups {
408   ***      1                    1      0      4      my ( $self ) = @_;
409            1                                 16      return $self->{groups};
410                                                   }
411                                                   
...
415                                                   sub _set_option {
416           77                   77           368      my ( $self, $opt, $val ) = @_;
417   ***     77      0                         201      my $long = exists $self->{opts}->{$opt}       ? $opt
      ***            50                               
418                                                               : exists $self->{short_opts}->{$opt} ? $self->{short_opts}->{$opt}
419                                                               : die "Getopt::Long gave a nonexistent option: $opt";
420                                                   
421                                                      # Reassign $opt.
422           77                                178      $opt = $self->{opts}->{$long};
423           77    100                         981      if ( $opt->{is_cumulative} ) {
424            8                                 32         $opt->{value}++;
425                                                      }
426                                                      else {
427           69                                284         $opt->{value} = $val;
428                                                      }
429           77                                282      $opt->{got} = 1;
430           77                                298      MKDEBUG && _d('Got option', $long, '=', $val);
431                                                   }
432                                                   
...
436                                                   sub get_opts {
437   ***     59                   59      0    246      my ( $self ) = @_; 
438                                                   
439                                                      # Reset opts. 
440           59                                190      foreach my $long ( keys %{$self->{opts}} ) {
              59                                441   
441          569                               2527         $self->{opts}->{$long}->{got} = 0;
442          569    100                        5131         $self->{opts}->{$long}->{value}
                    100                               
443                                                            = exists $self->{defaults}->{$long}       ? $self->{defaults}->{$long}
444                                                            : $self->{opts}->{$long}->{is_cumulative} ? 0
445                                                            : undef;
446                                                      }
447           59                                260      $self->{got_opts} = 0;
448                                                   
449                                                      # Reset errors.
450           59                                243      $self->{errors} = [];
451                                                   
452                                                      # --config is special-case; parse them manually and remove them from @ARGV
453           59    100    100                  585      if ( @ARGV && $ARGV[0] eq "--config" ) {
454            4                                 14         shift @ARGV;
455            4                                 25         $self->_set_option('config', shift @ARGV);
456                                                      }
457           59    100                         264      if ( $self->has('config') ) {
458           12                                 46         my @extra_args;
459           12                                 87         foreach my $filename ( split(',', $self->get('config')) ) {
...
462                                                            # can't be opened.
463           37                                122            eval {
464           37                                208               push @extra_args, $self->_read_config_file($filename);
465                                                            };
466           37    100                         283            if ( $EVAL_ERROR ) {
467           32    100                         164               if ( $self->got('config') ) {
...
473                                                            }
474                                                         }
475           11                                 80         unshift @ARGV, @extra_args;
476                                                      }
477                                                   
478           58                                361      Getopt::Long::Configure('no_ignore_case', 'bundling');
479                                                      GetOptions(
480                                                         # Make Getopt::Long specs for each option with custom handler subs.
481          556                   73          4118         map    { $_->{spec} => sub { $self->_set_option(@_); } }
              73                             111350   
             567                               2464   
482           58                                351         grep   { $_->{long} ne 'config' } # --config is handled specially above.
483           58    100                        6883         values %{$self->{opts}}
484                                                      ) or $self->save_error('Error parsing options');
485                                                   
486   ***     58     50     66                12056      if ( exists $self->{opts}->{version} && $self->{opts}->{version}->{got} ) {
...
491                                                      }
492                                                   
493           58    100    100                  334      if ( @ARGV && $self->{strict} ) {
494            1                                 10         $self->save_error("Unrecognized command-line options @ARGV");
495                                                      }
496                                                   
497                                                      # Check mutex options.
498           58                                152      foreach my $mutex ( @{$self->{mutex}} ) {
              58                                316   
499           18                                 74         my @set = grep { $self->{opts}->{$_}->{got} } @$mutex;
              39                                218   
500           18    100                         104         if ( @set > 1 ) {
501            5                                 39            my $err = join(', ', map { "--$self->{opts}->{$_}->{long}" }
               3                                 11   
502            3                                 18                         @{$mutex}[ 0 .. scalar(@$mutex) - 2] )
503                                                                    . ' and --'.$self->{opts}->{$mutex->[-1]}->{long}
504                                                                    . ' are mutually exclusive.';
505            3                                 13            $self->save_error($err);
506                                                         }
507                                                      }
508                                                   
509           58                                161      foreach my $required ( @{$self->{atleast1}} ) {
              58                                271   
510            6                                 25         my @set = grep { $self->{opts}->{$_}->{got} } @$required;
              18                                 95   
511            6    100                          34         if ( @set == 0 ) {
512            6                                 55            my $err = join(', ', map { "--$self->{opts}->{$_}->{long}" }
               3                                 13   
513            3                                 26                         @{$required}[ 0 .. scalar(@$required) - 2] )
514                                                                    .' or --'.$self->{opts}->{$required->[-1]}->{long};
515            3                                 23            $self->save_error("Specify at least one of $err");
516                                                         }
517                                                      }
518                                                   
519           58                                162      foreach my $long ( keys %{$self->{opts}} ) {
              58                                349   
520          567                               2152         my $opt = $self->{opts}->{$long};
521          567    100                        3066         if ( $opt->{got} ) {
                    100                               
522                                                            # Rule: opt disables other opts.
523           71    100                         355            if ( exists $self->{disables}->{$long} ) {
524            1                                  3               my @disable_opts = @{$self->{disables}->{$long}};
               1                                  5   
525            1                                  4               map { $self->{opts}->{$_}->{value} = undef; } @disable_opts;
               1                                  5   
526            1                                  2               MKDEBUG && _d('Unset options', @disable_opts,
527                                                                  'because', $long,'disables them');
528                                                            }
529                                                   
530                                                            # Group restrictions.
531           71    100                         364            if ( exists $self->{allowed_groups}->{$long} ) {
...
535                                                   
536           10                                 45               my @restricted_groups = grep {
...
545                                                                  {
546            4    100                          19                     next RESTRICTED_OPT if $restricted_opt eq $long;
...
571            3                                 17            $self->save_error("Required option --$long must be specified");
572                                                         }
573                                                   
574          567                               2171         $self->_validate_type($opt);
575                                                      }
576                                                   
577           58                                261      $self->{got_opts} = 1;
578           58                                182      return;
579                                                   }
580                                                   
581                                                   sub _validate_type {
582          567                  567          2036      my ( $self, $opt ) = @_;
583   ***    567    100     66                 5159      return unless $opt && $opt->{type};
584          348                               1144      my $val = $opt->{value};
585                                                   
586          348    100    100                 8768      if ( $val && $opt->{type} eq 'm' ) {  # type time
                    100    100                        
                    100    100                        
                    100    100                        
                    100    100                        
                           100                        
                           100                        
587           15                                 47         MKDEBUG && _d('Parsing option', $opt->{long}, 'as a time value');
588           15                                134         my ( $prefix, $num, $suffix ) = $val =~ m/([+-]?)(\d+)([a-z])?$/;
589                                                         # The suffix defaults to 's' unless otherwise specified.
590           15    100                          77         if ( !$suffix ) {
591            7                                 40            my ( $s ) = $opt->{desc} =~ m/\(suffix (.)\)/;
592            7           100                   43            $suffix = $s || 's';
593            7                                 18            MKDEBUG && _d('No suffix given; using', $suffix, 'for',
594                                                               $opt->{long}, '(value:', $val, ')');
595                                                         }
596           15    100                          70         if ( $suffix =~ m/[smhd]/ ) {
597           14    100                          88            $val = $suffix eq 's' ? $num            # Seconds
                    100                               
                    100                               
598                                                                 : $suffix eq 'm' ? $num * 60       # Minutes
599                                                                 : $suffix eq 'h' ? $num * 3600     # Hours
600                                                                 :                  $num * 86400;   # Days
601           14           100                  120            $opt->{value} = ($prefix || '') . $val;
...
609           10                                 25         MKDEBUG && _d('Parsing option', $opt->{long}, 'as a DSN');
...
613           10                                 40         my $prev = {};
614           10                                 48         my $from_key = $self->{defaults_to}->{ $opt->{long} };
615           10    100                          46         if ( $from_key ) {
616            4                                 11            MKDEBUG && _d($opt->{long}, 'DSN copies from', $from_key, 'DSN');
617            4                                 21            $prev = $self->{opts}->{$from_key}->{value};
618                                                         }
619           10                                 67         my $defaults = $self->{dp}->parse_options($self);
620           10                               1127         $opt->{value} = $self->{dp}->parse($val, $prev, $defaults);
621                                                      }
622                                                      elsif ( $val && $opt->{type} eq 'z' ) {  # type size
623            6                                 12         MKDEBUG && _d('Parsing option', $opt->{long}, 'as a size value');
624            6                                 31         my %factor_for = (k => 1_024, M => 1_048_576, G => 1_073_741_824);
625            6                                 41         my ($pre, $num, $factor) = $val =~ m/^([+-])?(\d+)([kMG])?$/;
626            6    100                          25         if ( defined $num ) {
...
639           10           100                   80         $opt->{value} = { map { $_ => 1 } split(',', ($val || '')) };
              24                                117   
640                                                      }
641                                                      elsif ( $opt->{type} eq 'A' || (defined $val && $opt->{type} eq 'a') ) {
642           62           100                  586         $opt->{value} = [ split(/(?<!\\),\s*/, ($val || '')) ];
643                                                      }
644                                                      else {
645          245                                608         MKDEBUG && _d('Nothing to validate for option',
...
654                                                   sub get {
655   ***     99                   99      0    537      my ( $self, $opt ) = @_;
656           99    100                         585      my $long = (length $opt == 1 ? $self->{short_opts}->{$opt} : $opt);
657           99    100    100                 1176      die "Option $opt does not exist"
658                                                         unless $long && exists $self->{opts}->{$long};
659           96                                868      return $self->{opts}->{$long}->{value};
660                                                   }
661                                                   
...
665                                                   sub got {
666   ***     54                   54      0    291      my ( $self, $opt ) = @_;
667           54    100                         295      my $long = (length $opt == 1 ? $self->{short_opts}->{$opt} : $opt);
668           54    100    100                  592      die "Option $opt does not exist"
669                                                         unless $long && exists $self->{opts}->{$long};
670           52                                409      return $self->{opts}->{$long}->{got};
671                                                   }
672                                                   
673                                                   # Returns true if the option exists.
674                                                   sub has {
675   ***    146                  146      0   1128      my ( $self, $opt ) = @_;
676          146    100                         763      my $long = (length $opt == 1 ? $self->{short_opts}->{$opt} : $opt);
677          146    100                        1124      return defined $long ? exists $self->{opts}->{$long} : 0;
678                                                   }
679                                                   
...
687            5    100                          31      my $long = (length $opt == 1 ? $self->{short_opts}->{$opt} : $opt);
688            5    100    100                   45      die "Option $opt does not exist"
689                                                         unless $long && exists $self->{opts}->{$long};
690            3                                 16      $self->{opts}->{$long}->{value} = $val;
691            3                                 10      return;
692                                                   }
693                                                   
...
696                                                   sub save_error {
697   ***     16                   16      0   3421      my ( $self, $error ) = @_;
698           16                                 53      push @{$self->{errors}}, $error;
              16                                 95   
699                                                   }
700                                                   
701                                                   # Return arrayref of errors (mostly for testing).
702                                                   sub errors {
703   ***     13                   13      0     55      my ( $self ) = @_;
704           13                                 95      return $self->{errors};
705                                                   }
706                                                   
707                                                   sub prompt {
708   ***     11                   11      0     42      my ( $self ) = @_;
709           11                                 92      return "Usage: $PROGRAM_NAME $self->{prompt}\n";
710                                                   }
711                                                   
712                                                   sub descr {
713   ***     11                   11      0     49      my ( $self ) = @_;
714   ***     11            50                  126      my $descr  = $self->{program_name} . ' ' . ($self->{description} || '')
...
720   ***     11     50                          61      $descr = join("\n", $descr =~ m/(.{0,80})(?:\s+|$)/g)
721                                                         unless $ENV{DONT_BREAK_LINES};
722           11                                108      $descr =~ s/ +$//mg;
723           11                                 84      return $descr;
724                                                   }
725                                                   
...
742   ***      1                    1      0      7      my ( $self ) = @_;
743            1                                  7      my $usage = $self->prompt() . "\n";
744   ***      1     50                           5      if ( (my @errors = @{$self->{errors}}) ) {
               1                                 14   
745            1                                 10         $usage .= join("\n  * ", 'Errors in command-line arguments:', @errors)
746                                                                 . "\n";
747                                                      }
748            1                                 18      return $usage . "\n" . $self->descr();
749                                                   }
750                                                   
...
758                                                   sub print_usage {
759   ***     10                   10      0     46      my ( $self ) = @_;
760   ***     10     50                          58      die "Run get_opts() before print_usage()" unless $self->{got_opts};
761           10                                 27      my @opts = values %{$self->{opts}};
              10                                 60   
762                                                   
763                                                      # Find how wide the widest long option is.
764           32    100                         227      my $maxl = max(
765           10                                 40         map { length($_->{long}) + ($_->{is_negatable} ? 4 : 0) }
766                                                         @opts);
767                                                   
768                                                      # Find how wide the widest option with a short option is.
769   ***     12     50                          72      my $maxs = max(0,
770           10                                 58         map { length($_) + ($self->{opts}->{$_}->{is_negatable} ? 4 : 0) }
771           10                                 38         values %{$self->{short_opts}});
772                                                   
...
775           10                                 50      my $lcol = max($maxl, ($maxs + 3));
776           10                                 39      my $rcol = 80 - $lcol - 6;
777           10                                 46      my $rpad = ' ' x ( 80 - $rcol );
778                                                   
779                                                      # Adjust the width of the options that have long and short both.
780           10                                 46      $maxs = max($lcol - 3, $maxs);
781                                                   
782                                                      # Format and return the options.
783           10                                 54      my $usage = $self->descr() . "\n" . $self->prompt();
784                                                   
785                                                      # Sort groups alphabetically but make 'default' first.
786           10                                 37      my @groups = reverse sort grep { $_ ne 'default'; } keys %{$self->{groups}};
              14                                 78   
              10                                 56   
787           10                                 36      push @groups, 'default';
788                                                   
789           10                                 41      foreach my $group ( reverse @groups ) {
790           14    100                          67         $usage .= "\n".($group eq 'default' ? 'Options' : $group).":\n\n";
791           14                                 48         foreach my $opt (
              22                                 92   
792           64                                227            sort { $a->{long} cmp $b->{long} }
793                                                            grep { $_->{group} eq $group }
794                                                            @opts )
795                                                         {
796           32    100                         185            my $long  = $opt->{is_negatable} ? "[no]$opt->{long}" : $opt->{long};
797           32                                102            my $short = $opt->{short};
798           32                                112            my $desc  = $opt->{desc};
799                                                            # Expand suffix help for time options.
800           32    100    100                  253            if ( $opt->{type} && $opt->{type} eq 'm' ) {
801            2                                 10               my ($s) = $desc =~ m/\(suffix (.)\)/;
802            2           100                    9               $s    ||= 's';
803            2                                  8               $desc =~ s/\s+\(suffix .\)//;
804            2                                 10               $desc .= ".  Optional suffix s=seconds, m=minutes, h=hours, "
805                                                                      . "d=days; if no suffix, $s is used.";
806                                                            }
807                                                            # Wrap long descriptions
808           32                                440            $desc = join("\n$rpad", grep { $_ } $desc =~ m/(.{0,$rcol})(?:\s+|$)/g);
              69                                238   
809           32                                168            $desc =~ s/ +$//mg;
810           32    100                         110            if ( $short ) {
811           12                                 90               $usage .= sprintf("  --%-${maxs}s -%s  %s\n", $long, $short, $desc);
812                                                            }
813                                                            else {
814           20                                152               $usage .= sprintf("  --%-${lcol}s  %s\n", $long, $desc);
815                                                            }
816                                                         }
817                                                      }
818                                                   
819           10    100                          29      if ( (my @rules = @{$self->{rules}}) ) {
              10                                 70   
820            4                                 13         $usage .= "\nRules:\n\n";
821            4                                 15         $usage .= join("\n", map { "  $_" } @rules) . "\n";
               4                                 23   
822                                                      }
823           10    100                          50      if ( $self->{dp} ) {
824            2                                 15         $usage .= "\n" . $self->{dp}->usage();
825                                                      }
826           10                                320      $usage .= "\nOptions and values after processing arguments:\n\n";
827           10                                 19      foreach my $opt ( sort { $a->{long} cmp $b->{long} } @opts ) {
              34                                122   
828           32                                126         my $val   = $opt->{value};
829           32           100                  182         my $type  = $opt->{type} || '';
830           32                                190         my $bool  = $opt->{spec} =~ m/^[\w-]+(?:\|[\w-])?!?$/;
831           32    100                         234         $val      = $bool                     ? ( $val ? 'TRUE' : 'FALSE' )
                    100                               
                    100                               
                    100                               
                    100                               
...
836                                                                   :                             $val;
837           32                                391         $usage .= sprintf("  --%-${lcol}s  %s\n", $opt->{long}, $val);
838                                                      }
839           10                                140      return $usage;
840                                                   }
841                                                   
...
886           38                   38           186      my ( $self, $filename ) = @_;
887           38    100                         314      open my $fh, "<", $filename or die "Cannot open $filename: $OS_ERROR\n";
888            6                                 21      my @args;
889            6                                 24      my $prefix = '--';
890            6                                 21      my $parse  = 1;
891                                                   
892                                                      LINE:
893            6                               8365      while ( my $line = <$fh> ) {
894           24                                 82         chomp $line;
895                                                         # Skip comments and empty lines
896           24    100                         199         next LINE if $line =~ m/^\s*(?:\#|\;|$)/;
897                                                         # Remove inline comments
898           20                                 92         $line =~ s/\s+#.*$//g;
899                                                         # Remove whitespace
900           20                                143         $line =~ s/^\s+|\s+$//g;
901                                                         # Watch for the beginning of the literal values (not to be interpreted as
902                                                         # options)
903           20    100                         110         if ( $line eq '--' ) {
904            4                                 13            $prefix = '';
905            4                                 13            $parse  = 0;
906            4                                 27            next LINE;
907                                                         }
908   ***     16    100     66                  242         if ( $parse
      ***            50                               
909                                                            && (my($opt, $arg) = $line =~ m/^\s*([^=\s]+?)(?:\s*=\s*(.*?)\s*)?$/)
910                                                         ) {
911            9                                 47            push @args, grep { defined $_ } ("$prefix$opt", $arg);
              18                                131   
912                                                         }
913                                                         elsif ( $line =~ m/./ ) {
914            7                                 82            push @args, $line;
915                                                         }
916                                                         else {
...
928   ***      2     50                          55      open my $fh, "<", $file or die "Can't open $file: $OS_ERROR";
929            2                                 11      local $INPUT_RECORD_SEPARATOR = '';
930            2                                  5      my $para;
931            2                                242      while ( $para = <$fh> ) {
932            6    100                          53         next unless $para =~ m/^=pod$/m;
933            2                                  8         last;
934                                                      }
935            2                                 15      while ( $para = <$fh> ) {
936            7    100                          58         next unless $para =~ m/$regex/;
937            2                                  7         last;
938                                                      }
939            2                                  9      $para = <$fh>;
940            2                                  9      chomp($para);
941   ***      2     50                          25      close $fh or die "Can't close $file: $OS_ERROR";
942            2                                  6      return $para;
943                                                   }
944                                                   
...
952            3                                  9      my %clone = map {
953            1                                  5         my $hashref  = $self->{$_};
954            3                                 10         my $val_copy = {};
955            3                                 12         foreach my $key ( keys %$hashref ) {
956            5                                 18            my $ref = ref $hashref->{$key};
957            3                                 31            $val_copy->{$key} = !$ref           ? $hashref->{$key}
958   ***      0                                  0                              : $ref eq 'HASH'  ? { %{$hashref->{$key}} }
959   ***      5      0                          51                              : $ref eq 'ARRAY' ? [ @{$hashref->{$key}} ]
      ***            50                               
                    100                               
960                                                                              : $hashref->{$key};
961                                                         }
...
975   ***      2     50                          13      @_ = map { (my $temp = $_) =~ s/\n/\n# /g; $temp; }
               2                                  8   
               2                                 11   
976            1                                  5           map { defined $_ ? $_ : 'undef' }
977                                                           @_;
978            1                                  4      print STDERR "# $package:$line $PID ", join(' ', @_), "\n";
979                                                   }
980                                                   
981                                                   1;
982                                                   
...
16             1                    1            14   use MaatkitTest;
               ...
               1                                 11   
17                                                    
18             1                                  8   my $dp = new DSNParser();
19             1                                 87   my $o  = new OptionParser(
...
52                                                       dp           => $dp,
53                                                    );
54             1                                 13   ok(!$o->has('time'), 'There is no --time yet');
...
73                                                    );
74                                                    
75             1                                 50   $o->_parse_specs(@opt_specs);
76             1                                  8   ok($o->has('time'), 'There is a --time now');
77             1                                  8   %opts = $o->opts();
78             1                                 92   is_deeply(
79                                                       \%opts,
80                                                       {
...
338            1                                  8   @ARGV = qw(--bar);
339            1                                  4   eval {
340            1                                  6      local *STDERR;
341            1                                 42      open STDERR, '>', '/dev/null';
342            1                                  5      $o->get_opts();
343            1                                  5      $o->get('bar');
344                                                   };
345            1                                 10   like(
...
370                                                   # #############################################################################
371            1                                  6   eval { $o->_pod_to_specs("$trunk/common/t/samples/pod_sample_02.txt"); };
               1                                  8   
372            1                                 11   like(
373                                                      $EVAL_ERROR,
...
383                                                   );
384                                                   
385            1                                  7   eval { $o->_pod_to_specs("$trunk/common/t/samples/pod_sample_04.txt"); };
               1                                  7   
386            1                                 10   like(
387                                                      $EVAL_ERROR,
...
513                                                   );
514                                                   
515            1                                 18   $o->set_defaults('dog' => 'fido');
516                                                   
517            1                                  4   @ARGV = ();
518            1                                  6   $o->get_opts();
519            1                                  5   is(
...
689                                                   # #############################################################################
690            1                                  8   $o = new OptionParser(
691                                                      description  => 'parses command line options.',
692                                                   );
693            1                                 20   $o->_parse_specs(
...
815                                                   );
816            1                                  6   @ARGV = qw(--ignore --replace);
817            1                                  6   $o->get_opts();
818            1                                  5   is_deeply(
819                                                      $o->errors(),
...
845                                                   );
846            1                                  4   @ARGV = qw(--ignore --replace);
847            1                                  5   $o->get_opts();
848            1                                  5   is_deeply(
849                                                      $o->errors(),
...
867                                                   );
868                                                   
869            1                                 13   $o = new OptionParser(
870                                                      description  => 'parses command line options.',
871                                                   );
872            1                                 16   $o->_parse_specs(
873                                                      @ird_specs,
...
882                                                   );
883                                                   
884            1                                 12   $o = new OptionParser(
885                                                      description  => 'parses command line options.',
886                                                   );
887            1                                 15   $o->_parse_specs(
888                                                      @ird_specs,
...
1106                                                  # #############################################################################
1107           1                                 12   $o = new OptionParser(
1108                                                     description  => 'parses command line options.',
1109                                                  );
1110           1                                 22   $o->_parse_specs(
...
1377                                                  
1378           1                                 10   @ARGV = ('-C', 'a,b', '-t', 'd,e', '-d', 'f,g', '-b', 'o,p' );
1379           1                                  5   $o->get_opts();
1380           1                                  5   %opts = (
1381                                                     C => $o->get('C'),
1382                                                     t => $o->get('t'),
...
1506                                                  );
1507                                                  
1508           1                                  6   @ARGV = qw(--schema --algorithm ACCUM);
1509           1                                  5   $o->get_opts();
1510           1                                  5   is_deeply(
1511                                                     $o->errors(),
...
1591                                                  );
1592                                                  
1593           1                                 20   $o = new OptionParser(
1594                                                     description  => 'parses command line options.',
1595                                                  );
1596           1                                 35   $o->_parse_specs(
...
1607                                                  );
1608           1                                 16   ok(!$o->got('config'), 'Did not got --config');
1609                                                  
1610           1                                  9   $o = new OptionParser(
1611                                                     description  => 'parses command line options.',
1612                                                  );
1613           1                                 23   $o->_parse_specs(
...
1639                                                  );
1640                                                  
1641           1                                  9   @ARGV=qw(--config /path/to/config --cat);
1642           1                                 12   $o = new OptionParser(
1643                                                     description  => 'parses command line options.',
1644                                                  );
1645                                                  
1646           1                                 29   $o->_parse_specs(
...
1650                                                  );
1651           1                                  4   eval { $o->get_opts(); };
               1                                  6   
1652           1                                 15   like($EVAL_ERROR, qr/Cannot open/, 'No config file found');
1653                                                  
1654           1                                 15   @ARGV = ('--config',"$trunk/common/t/samples/empty",'--cat');
1655           1                                 11   $o->get_opts();
1656           1                                  7   ok($o->got('config'), 'Got --config');
1657                                                  
1658           1                                 10   is(
1659                                                     $o->print_usage(),
1660                                                  <<EOF
...
1677                                                  );
1678                                                  
1679           1                                 16   $o = new OptionParser(
1680                                                     description  => 'parses command line options.',
1681                                                  );
1682           1                                 29   $o->_parse_specs(
...
1686                                                  );
1687                                                  
1688           1                                  7   @ARGV=qw(--cat --config /path/to/config);
1689                                                  {
1690           1                                  4      local *STDERR;
               1                                  5   
1691           1                                 49      open STDERR, '>', '/dev/null';
1692           1                                  8      $o->get_opts();
1693                                                  }
1694                                                  is_deeply(
1695           1                                 19      $o->errors(),
...
1701           1                                 20   $o = new OptionParser(
1702                                                     description  => 'parses command line options.',
1703                                                     strict       => 0,
1704                                                  );
1705           1                                 30   $o->_parse_specs(
...
1710                                                  );
1711           1                                  9   is($o->{strict}, 0, 'setting strict to 0 worked');
1712                                                  
...
1719                                                  );
1720           1                                 15   ok($o->got('foo'), 'Got --foo');
1721           1                                  9   is($o->get('foo'), 'bar', 'Got --foo value');
1722           1                                  9   ok($o->got('verbose'), 'Got --verbose');
1723           1                                  9   is($o->get('verbose'), 1, 'Got --verbose value');
1724                                                  
1725           1                                 14   @ARGV = ('--config', "$trunk/common/t/samples/config_file_1.conf,$trunk/common/t/samples/config_file_2.conf");
1726           1                                  8   $o->get_opts();
1727           1                                 12   is_deeply(
1728                                                     [@ARGV],
...
1731                                                  );
1732           1                                 16   ok($o->got('foo'), 'Got --foo again');
1733           1                                  9   is($o->get('foo'), 'baz', 'Got overridden --foo value');
1734           1                                 11   ok($o->got('verbose'), 'Got --verbose twice');
1735           1                                  9   is($o->get('verbose'), 2, 'Got --verbose value twice');
1736                                                  
1737                                                  # #############################################################################
1738                                                  # Issue 409: OptionParser modifies second value of
1739                                                  # ' -- .*','(\w+): ([^,]+)' for array type opt
1740                                                  # #############################################################################
1741           1                                 13   $o = new OptionParser(
1742                                                     description  => 'parses command line options.',
1743                                                  );
1744           1                                 29   $o->_parse_specs(
...
1760                                                  # #############################################################################
1761           1                                 15   $o = new OptionParser(
1762                                                     description  => 'parses command line options.',
1763                                                  );
1764           1                                 15   eval { $o->get_specs("$trunk/common/t/samples/pod_sample_06.txt"); };
               1                                 11   
1765           1                                 16   like(
1766                                                     $EVAL_ERROR,
...
1777           1                                 17   $o = new OptionParser(
1778                                                     description  => 'parses command line options.',
1779                                                     dp           => $dp,
1780                                                  );
1781           1                                 19   $o->_parse_specs(
...
1789                                                  );
1790           1                                  7   $o->get_opts();
1791           1                                  7   my $dest_dsn = $o->get('dest');
1792           1                                 15   is_deeply(
1793                                                     $dest_dsn,
1794                                                     {
1795                                                        A => undef,
...
1812                                                  
1813                                                  # case ii.
1814           1                                 18   $o = new OptionParser(
1815                                                     description  => 'parses command line options.',
1816                                                     dp           => $dp,
1817                                                  );
1818           1                                 25   $o->get_specs("$trunk/mk-archiver/mk-archiver");
1819           1                                 14   @ARGV = (
...
1826                                                  );
1827           1                                  9   $o->get_opts();
1828           1                                  8   my $src_dsn = $o->get('source');
1829           1                                 21   is_deeply(
1830                                                     $src_dsn,
1831                                                     {
1832                                                        A => undef,
...
1871                                                  # #############################################################################
1872           1                               4693   diag(`echo "iterations=4" > ~/.OptionParser.t.conf`);
1873           1                                161   $o = new OptionParser(
1874                                                     description  => 'parses command line options.',
1875                                                     dp           => $dp,
1876                                                  );
1877           1                                311   $o->get_specs("$trunk/mk-query-digest/mk-query-digest");
1878           1                                  9   @ARGV = (qw(--iterations 9));
1879           1                                 16   $o->get_opts();
1880           1                                  6   is(
1881                                                     $o->get('iterations'),
1882                                                     9,
1883                                                     'Cmd line opt overrides conf (issue 617)'
1884                                                  );
1885           1                               5818   diag(`rm -rf ~/.OptionParser.t.conf`);
1886                                                  
1887                                                  # #############################################################################
...
1892           1                                165   $o = new OptionParser(
1893                                                     description  => 'parses command line options.',
1894                                                     dp           => $dp,
1895                                                  );
1896           1                                354   $o->get_specs("$trunk/mk-query-digest/mk-query-digest");
1897           1                                  9   @ARGV = (qw(--run-time +9));
...
1946           1                                  9   my $output = '';
1947                                                  {
1948           1                                  3      local *STDERR;
               1                                  6   
1949           1                    1             2      open STDERR, '>', \$output;
               1                               9245   
               1                                  3   
               1                                  8   
1950           1                                 26      $o->_d('Complete test coverage');
1951                                                  }
1952                                                  like(
1953           1                                 19      $output,
