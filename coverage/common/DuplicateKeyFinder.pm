...
OS:           linux
Start:        Fri Jun 12 18:40:36 2009
Finish:       Fri Jun 12 18:40:36 2009
...
55    ***     24     50                         101      die "I need a keys argument" unless $keys;
56            24                                144      my %keys = %$keys;  # Copy keys because we remove non-duplicates.
57            24                                 60      my $primary_key;
58            24                                 57      my @unique_keys;
59            24                                 52      my @normal_keys;
60            24                                 52      my @fulltext_keys;
61            24                                 57      my @dupes;
62                                                    
63                                                       KEY:
64            24                                 88      foreach my $key ( values %keys ) {
65                                                          # Save real columns before we potentially re-order them.  These are
66                                                          # columns we want to print if the key is a duplicate.
67            59                                237         $key->{real_cols} = $key->{colnames}; 
68                                                    
69                                                          # We use column lengths to compare keys.
70            59                                230         $key->{len_cols}  = length $key->{colnames};
71                                                    
...
76            59    100                         247         if ( $key->{name} eq 'PRIMARY' ) {
77            10                                 23            $primary_key = $key;
78            10                                 46            next KEY;
79                                                          }
80                                                    
...
83            49    100                         204         my $is_fulltext = $key->{type} eq 'FULLTEXT' ? 1 : 0;
84            49    100    100                  364         if ( $args{ignore_order} || $is_fulltext  ) {
85            11                                 81            my $ordered_cols = join(',', sort(split(/,/, $key->{colnames})));
...
94            49    100                         207         my $push_to = $key->{is_unique} ? \@unique_keys : \@normal_keys;
95            49    100                         201         if ( !$args{ignore_structure} ) {
96            47    100                         171            $push_to = \@fulltext_keys if $is_fulltext;
97                                                             # TODO:
98                                                             # $push_to = \@hash_keys     if $is_hash;
99                                                             # $push_to = \@spatial_keys  if $is_spatial;
100                                                         }
101           49                                180         push @$push_to, $key; 
102                                                      }
103                                                   
104                                                      # Redundantly constrained unique keys are treated as normal keys.
105           24                                127      push @normal_keys, $self->unconstrain_keys($primary_key, \@unique_keys);
106                                                   
...
115           24    100                          94      if ( $primary_key ) {
116           10                                 23         MKDEBUG && _d('Comparing PRIMARY KEY to UNIQUE keys');
117           10                                 69         push @dupes,
...
123                                                      }
124                                                   
125           24                                 62      MKDEBUG && _d('Comparing UNIQUE keys to normal keys');
126           24                                136      push @dupes,
127                                                         $self->remove_prefix_duplicates(\@unique_keys, \@normal_keys, %args);
128                                                   
129           24                                 57      MKDEBUG && _d('Comparing normal keys');
130           24                                125      push @dupes,
...
142           24    100    100                  253      if ( $primary_key
      ***                   66                        
                           100                        
143                                                           && $args{clustered}
...
146                                                      {
147            2                                  4         MKDEBUG && _d('Removing UNIQUE dupes of clustered key');
148            2                                 13         push @dupes,
...
173            3                                  9            my $i_cols  = join(',', sort @{$fks[$i]->{cols}} );
               3                                 24   
174            3                                 12            my $j_cols  = join(',', sort @{$fks[$j]->{cols}} );
               3                                 16   
175            3                                  9            my $i_pcols = join(',', sort @{$fks[$i]->{parent_cols}} );
               3                                 16   
176            3                                 12            my $j_pcols = join(',', sort @{$fks[$j]->{parent_cols}} );
               3                                 15   
177                                                   
...
235           92                   92           443      my ( $self, $left_keys, $right_keys, %args ) = @_;
236           92                                247      my @dupes;
237           92                                204      my $right_offset;
238           92                                195      my $last_left_key;
239           92                                295      my $last_right_key = scalar(@$right_keys) - 1;
240                                                   
...
244                                                   
245           92    100                         324      if ( $right_keys != $left_keys ) {
246                                                         # Right and left keys are different lists.
247                                                   
248            1                                  4         @$left_keys = sort { $a->{colnames} cmp $b->{colnames} }
              27                                139   
249           44                                184                       grep { defined $_; }
250                                                                       @$left_keys;
251           31                                141         @$right_keys = sort { $a->{colnames} cmp $b->{colnames} }
              48                                177   
252           44                                167                        grep { defined $_; }
253                                                                       @$right_keys;
254                                                   
255                                                         # Last left key is its very last key.
256           44                                158         $last_left_key = scalar(@$left_keys) - 1;
257                                                   
258                                                         # No need to offset where we begin looping through the right keys.
259           44                                115         $right_offset = 0;
260                                                      }
261                                                      else {
...
277                                                   
278                                                      LEFT_KEY:
279           92                                334      foreach my $left_index ( 0..$last_left_key ) {
...
284   ***     36     50                         171            next RIGHT_KEY unless defined $right_keys->[$right_index];
285                                                   
286           36                                136            my $left_name      = $left_keys->[$left_index]->{name};
287           36                                128            my $left_cols      = $left_keys->[$left_index]->{colnames};
288           36                                125            my $left_len_cols  = $left_keys->[$left_index]->{len_cols};
289           36                                118            my $right_name     = $right_keys->[$right_index]->{name};
290           36                                122            my $right_cols     = $right_keys->[$right_index]->{colnames};
291           36                                123            my $right_len_cols = $right_keys->[$right_index]->{len_cols};
292                                                   
...
299           36    100                         174            if (    substr($left_cols,  0, $right_len_cols)
...
304           18    100    100                  114               if ( $args{exact_duplicates} && ($right_len_cols<$left_len_cols) ) {
305            1                                  2                  MKDEBUG && _d($right_name, 'not exact duplicate of', $left_name);
306            1                                  4                  next RIGHT_KEY;
307                                                               }
308                                                   
...
312           17    100                          77               if ( exists $right_keys->[$right_index]->{unique_col} ) {
...
317                                                               }
318                                                   
319           16                                 44               MKDEBUG && _d('Remove', $right_name);
320           16                                 39               my $reason;
321           16    100                          74               if ( $right_keys->[$right_index]->{unconstrained} ) {
322            3                                 21                  $reason .= "Uniqueness of $right_name ignored because "
323                                                                     . $right_keys->[$right_index]->{constraining_key}->{name}
324                                                                     . " is a stronger constraint\n"; 
325                                                               }
326           16    100                          81               $reason .= $right_name
...
338           16                                 50               delete $right_keys->[$right_index];
339                                                   
340   ***     16     50                         110               $args{callback}->($dupe, %args) if $args{callback};
341                                                            }
342                                                            else {
343           18                                 41               MKDEBUG && _d($right_name, 'not left-prefix of', $left_name);
344           18                                 71               next LEFT_KEY;
345                                                            }
346                                                         } # RIGHT_KEY
347                                                      } # LEFT_KEY
348           92                                286      MKDEBUG && _d('No more keys');
349                                                   
350                                                      # Cleanup the lists: remove removed keys.
351           92                                299      @$left_keys  = grep { defined $_; } @$left_keys;
              53                                190   
352           92                                298      @$right_keys = grep { defined $_; } @$right_keys;
              72                                241   
353                                                   
354           92                                367      return @dupes;
355                                                   }
356                                                   
...
370            4                    4            26      my ( $self, $primary_key, $keys, %args ) = @_;
371   ***      4     50                          18      die "I need a primary_key argument" unless $primary_key;
372   ***      4     50                          12      die "I need a keys argument"        unless $keys;
373            4                                 15      my $pkcols = $primary_key->{colnames};
374            4                                  8      my @dupes;
375                                                   
376                                                      KEY:
...
382   ***      1     50                           7            if ( substr($suffix, 0, $len) eq substr($pkcols, 0, $len) ) {
383            1                                 18               my $dupe = {
384                                                                  key               => $keys->[$i]->{name},
...
401            4                                 14      @$keys = grep { defined $_; } @$keys;
               1                                  4   
402                                                   
403            4                                 17      return @dupes;
404                                                   }
405                                                   
...
409           24                   24            95      my ( $self, $primary_key, $unique_keys ) = @_;
410   ***     24     50                          89      die "I need a unique_keys argument" unless $unique_keys;
411           24                                 53      my %unique_cols;
412           24                                 54      my @unique_sets;
413           24                                 60      my %unconstrain;
414           24                                 52      my @unconstrained_keys;
415                                                   
416           24                                 52      MKDEBUG && _d('Unconstraining redundantly unique keys');
417                                                   
...
422           36    100                         140         next unless $unique_key; # primary key may be undefined
423           22                                 70         my $cols = $unique_key->{cols};
424           22    100                          77         if ( @$cols == 1 ) {
425           11                                 25            MKDEBUG && _d($unique_key->{name},'defines unique column:',$cols->[0]);
...
429   ***     11     50                          53            if ( !exists $unique_cols{$cols->[0]} ) {
430           11                                 39               $unique_cols{$cols->[0]}  = $unique_key;
431           11                                 58               $unique_key->{unique_col} = 1;
432                                                            }
433                                                         }
434                                                         else {
435           11                                 34            local $LIST_SEPARATOR = '-';
...
446           11                                 41         COL:
447           11                                 34         foreach my $col ( @{$unique_set->{cols}} ) {
448           24    100                         108            if ( exists $unique_cols{$col} ) {
449            7                                 16               MKDEBUG && _d('Unique set', $unique_set->{key}->{name},
450                                                                  'has unique col', $col);
451   ***      7     50                          29               last COL if ++$n_unique_cols > 1;
452            7                                 29               $unique_set->{constraining_key} = $unique_cols{$col};
453                                                            }
454                                                         }
455           11    100    100                  100         if ( $n_unique_cols && $unique_set->{key}->{name} ne 'PRIMARY' ) {
...
462            6                                 36            $unconstrain{$unique_set->{key}->{name}}
...
470           24                                122      for my $i ( 0..(scalar @$unique_keys-1) ) {
471           12    100                          66         if ( exists $unconstrain{$unique_keys->[$i]->{name}} ) {
472            6                                 15            MKDEBUG && _d('Unconstraining', $unique_keys->[$i]->{name});
473            6                                 28            $unique_keys->[$i]->{unconstrained} = 1;
474            6                                 34            $unique_keys->[$i]->{constraining_key}
475                                                               = $unconstrain{$unique_keys->[$i]->{name}};
476            6                                 21            push @unconstrained_keys, $unique_keys->[$i];
477            6                                 23            delete $unique_keys->[$i];
478                                                         }
479                                                      }
480                                                   
481           24                                 58      MKDEBUG && _d('No more keys');
482           24                                127      return @unconstrained_keys;
483                                                   }
484                                                   
485                                                   sub _d {
486            1                    1            24      my ($package, undef, $line) = caller 0;
487   ***      2     50                           8      @_ = map { (my $temp = $_) =~ s/\n/\n# /g; $temp; }
               2                                  8   
               2                                 10   
488            1                                  5           map { defined $_ ? $_ : 'undef' }
489                                                           @_;
490            1                                  4      print STDERR "# $package:$line $PID ", join(' ', @_), "\n";
491                                                   }
492                                                   
493                                                   1;
494                                                   # ###########################################################################
