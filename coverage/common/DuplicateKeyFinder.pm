...
OS:           linux
Start:        Fri Jul 31 18:51:38 2009
Finish:       Fri Jul 31 18:51:38 2009
...
32             1                                  4      my $self = {};
33             1                                 15      return bless $self, $class;
34                                                    }
35                                                    
...
54            24                   24           130      my ( $self, $keys,  %args ) = @_;
55    ***     24     50                          98      die "I need a keys argument" unless $keys;
56            24                                130      my %keys = %$keys;  # Copy keys because we remove non-duplicates.
57            24                                 66      my $primary_key;
58            24                                 51      my @unique_keys;
59            24                                 53      my @normal_keys;
60            24                                 51      my @fulltext_keys;
61            24                                 56      my @dupes;
62                                                    
63                                                       KEY:
64            24                                 89      foreach my $key ( values %keys ) {
65                                                          # Save real columns before we potentially re-order them.  These are
66                                                          # columns we want to print if the key is a duplicate.
67            59                                235         $key->{real_cols} = $key->{colnames}; 
68                                                    
69                                                          # We use column lengths to compare keys.
70            59                                226         $key->{len_cols}  = length $key->{colnames};
71                                                    
...
76            59    100                         264         if ( $key->{name} eq 'PRIMARY' ) {
77            10                                 26            $primary_key = $key;
78            10                                 34            next KEY;
79                                                          }
80                                                    
...
83            49    100                         196         my $is_fulltext = $key->{type} eq 'FULLTEXT' ? 1 : 0;
84            49    100    100                  358         if ( $args{ignore_order} || $is_fulltext  ) {
85            11                                 77            my $ordered_cols = join(',', sort(split(/,/, $key->{colnames})));
86            11                                 23            MKDEBUG && _d('Reordered', $key->{name}, 'cols from',
87                                                                $key->{colnames}, 'to', $ordered_cols); 
88            11                                 38            $key->{colnames} = $ordered_cols;
89                                                          }
90                                                    
...
94            49    100                         215         my $push_to = $key->{is_unique} ? \@unique_keys : \@normal_keys;
95            49    100                         200         if ( !$args{ignore_structure} ) {
96            47    100                         166            $push_to = \@fulltext_keys if $is_fulltext;
97                                                             # TODO:
98                                                             # $push_to = \@hash_keys     if $is_hash;
99                                                             # $push_to = \@spatial_keys  if $is_spatial;
100                                                         }
101           49                                193         push @$push_to, $key; 
102                                                      }
103                                                   
104                                                      # Redundantly constrained unique keys are treated as normal keys.
105           24                                114      push @normal_keys, $self->unconstrain_keys($primary_key, \@unique_keys);
106                                                   
...
115           24    100                          85      if ( $primary_key ) {
116           10                                 21         MKDEBUG && _d('Comparing PRIMARY KEY to UNIQUE keys');
117           10                                 59         push @dupes,
118                                                            $self->remove_prefix_duplicates([$primary_key], \@unique_keys, %args);
119                                                   
120           10                                 28         MKDEBUG && _d('Comparing PRIMARY KEY to normal keys');
121           10                                 58         push @dupes,
122                                                            $self->remove_prefix_duplicates([$primary_key], \@normal_keys, %args);
123                                                      }
124                                                   
125           24                                 59      MKDEBUG && _d('Comparing UNIQUE keys to normal keys');
126           24                                130      push @dupes,
127                                                         $self->remove_prefix_duplicates(\@unique_keys, \@normal_keys, %args);
128                                                   
129           24                                 56      MKDEBUG && _d('Comparing normal keys');
130           24                                120      push @dupes,
...
135           24                                 56      MKDEBUG && _d('Comparing FULLTEXT keys');
136           24                                129      push @dupes,
...
142           24    100    100                  242      if ( $primary_key
      ***                   66                        
                           100                        
143                                                           && $args{clustered}
...
146                                                      {
147            2                                 14         MKDEBUG && _d('Removing UNIQUE dupes of clustered key');
148            2                                 12         push @dupes,
149                                                            $self->remove_clustered_duplicates($primary_key, \@unique_keys, %args);
150                                                   
151            2                                  4         MKDEBUG && _d('Removing ordinary dupes of clustered key');
152            2                                 11         push @dupes,
153                                                            $self->remove_clustered_duplicates($primary_key, \@normal_keys, %args);
154                                                      }
155                                                   
156           24                                113      return \@dupes;
157                                                   }
158                                                   
159                                                   sub get_duplicate_fks {
160            3                    3            15      my ( $self, $fks, %args ) = @_;
161   ***      3     50                          13      die "I need a fks argument" unless $fks;
162            3                                 14      my @fks = values %$fks;
163            3                                  6      my @dupes;
164                                                   
165            3                                 19      foreach my $i ( 0..$#fks - 1 ) {
166   ***      3     50                          12         next unless $fks[$i];
167            3                                 14         foreach my $j ( $i+1..$#fks ) {
...
172                                                            # can be compared.
173            3                                  7            my $i_cols  = join(',', sort @{$fks[$i]->{cols}} );
               3                                 23   
174            3                                  9            my $j_cols  = join(',', sort @{$fks[$j]->{cols}} );
               3                                 16   
175            3                                  7            my $i_pcols = join(',', sort @{$fks[$i]->{parent_cols}} );
               3                                 16   
176            3                                  9            my $j_pcols = join(',', sort @{$fks[$j]->{parent_cols}} );
               3                                 18   
177                                                   
178   ***      3    100     66                   49            if ( $fks[$i]->{parent_tbl} eq $fks[$j]->{parent_tbl}
      ***                   66                        
...
194                                                               };
195            2                                  6               push @dupes, $dupe;
196            2                                  6               delete $fks[$j];
197   ***      2     50                          15               $args{callback}->($dupe, %args) if $args{callback};
198                                                            }
199                                                         }
200                                                      }
201            3                                 33      return \@dupes;
202                                                   }
203                                                   
...
235           92                   92           481      my ( $self, $left_keys, $right_keys, %args ) = @_;
236           92                                252      my @dupes;
237           92                                197      my $right_offset;
238           92                                202      my $last_left_key;
239           92                                286      my $last_right_key = scalar(@$right_keys) - 1;
240                                                   
...
244                                                   
245           92    100                         297      if ( $right_keys != $left_keys ) {
246                                                         # Right and left keys are different lists.
247                                                   
248            1                                  5         @$left_keys = sort { $a->{colnames} cmp $b->{colnames} }
              27                                139   
249           44                                172                       grep { defined $_; }
250                                                                       @$left_keys;
251           31                                118         @$right_keys = sort { $a->{colnames} cmp $b->{colnames} }
              48                                151   
252           44                                155                        grep { defined $_; }
253                                                                       @$right_keys;
254                                                   
255                                                         # Last left key is its very last key.
256           44                                153         $last_left_key = scalar(@$left_keys) - 1;
257                                                   
258                                                         # No need to offset where we begin looping through the right keys.
259           44                                114         $right_offset = 0;
260                                                      }
261                                                      else {
262                                                         # Right and left keys are the same list.
263                                                   
264           17                                 68         @$left_keys = reverse sort { $a->{colnames} cmp $b->{colnames} }
              36                                110   
265           48                                181                       grep { defined $_; }
266                                                                       @$left_keys;
267                                                         
268                                                         # Last left key is its second-to-last key.
269                                                         # The very last left key will be used as a right key.
270           48                                150         $last_left_key = scalar(@$left_keys) - 2;
271                                                   
...
275           48                                150         $right_offset = 1;
276                                                      }
277                                                   
278                                                      LEFT_KEY:
279           92                                321      foreach my $left_index ( 0..$last_left_key ) {
280           41    100                         174         next LEFT_KEY unless defined $left_keys->[$left_index];
281                                                   
282                                                         RIGHT_KEY:
283           40                                152         foreach my $right_index ( $left_index+$right_offset..$last_right_key ) {
284   ***     36     50                         167            next RIGHT_KEY unless defined $right_keys->[$right_index];
285                                                   
286           36                                128            my $left_name      = $left_keys->[$left_index]->{name};
287           36                                123            my $left_cols      = $left_keys->[$left_index]->{colnames};
288           36                                115            my $left_len_cols  = $left_keys->[$left_index]->{len_cols};
289           36                                121            my $right_name     = $right_keys->[$right_index]->{name};
290           36                                115            my $right_cols     = $right_keys->[$right_index]->{colnames};
291           36                                124            my $right_len_cols = $right_keys->[$right_index]->{len_cols};
292                                                   
293           36                                 75            MKDEBUG && _d('Comparing left', $left_name, '(',$left_cols,')',
...
299           36    100                         159            if (    substr($left_cols,  0, $right_len_cols)
...
312           17    100                          75               if ( exists $right_keys->[$right_index]->{unique_col} ) {
313            1                                  2                  MKDEBUG && _d('Cannot remove', $right_name,
314                                                                     'because is constrains col',
315                                                                     $right_keys->[$right_index]->{cols}->[0]);
316            1                                  3                  next RIGHT_KEY;
317                                                               }
318                                                   
319           16                                 33               MKDEBUG && _d('Remove', $right_name);
320           16                                 35               my $reason;
321           16    100                          68               if ( $right_keys->[$right_index]->{unconstrained} ) {
322            3                                 19                  $reason .= "Uniqueness of $right_name ignored because "
...
335                                                                  reason            => $reason,
336                                                               };
337           16                                 46               push @dupes, $dupe;
338           16                                 49               delete $right_keys->[$right_index];
339                                                   
340   ***     16     50                         108               $args{callback}->($dupe, %args) if $args{callback};
341                                                            }
342                                                            else {
343           18                                 37               MKDEBUG && _d($right_name, 'not left-prefix of', $left_name);
344           18                                 72               next LEFT_KEY;
345                                                            }
346                                                         } # RIGHT_KEY
347                                                      } # LEFT_KEY
348           92                                272      MKDEBUG && _d('No more keys');
349                                                   
350                                                      # Cleanup the lists: remove removed keys.
351           92                                287      @$left_keys  = grep { defined $_; } @$left_keys;
              53                                200   
352           92                                294      @$right_keys = grep { defined $_; } @$right_keys;
              72                                231   
353                                                   
354           92                                352      return @dupes;
355                                                   }
356                                                   
...
370            4                    4            28      my ( $self, $primary_key, $keys, %args ) = @_;
371   ***      4     50                          17      die "I need a primary_key argument" unless $primary_key;
372   ***      4     50                          13      die "I need a keys argument"        unless $keys;
373            4                                 14      my $pkcols = $primary_key->{colnames};
374            4                                  8      my @dupes;
375                                                   
376                                                      KEY:
377            4                                 18      for my $i ( 0 .. @$keys - 1 ) {
378            2                                  8         my $suffix = $keys->[$i]->{colnames};
379                                                         SUFFIX:
380            2                                 14         while ( $suffix =~ s/`[^`]+`,// ) {
381            1                                 11            my $len = min(length($pkcols), length($suffix));
382   ***      1     50                          10            if ( substr($suffix, 0, $len) eq substr($pkcols, 0, $len) ) {
383            1                                 14               my $dupe = {
384                                                                  key               => $keys->[$i]->{name},
...
390                                                               };
391            1                                  4               push @dupes, $dupe;
392            1                                  2               delete $keys->[$i];
393   ***      1     50                           9               $args{callback}->($dupe, %args) if $args{callback};
394            1                                  9               last SUFFIX;
395                                                            }
396                                                         }
397                                                      }
398            4                                  9      MKDEBUG && _d('No more keys');
399                                                   
400                                                      # Cleanup the lists: remove removed keys.
401            4                                 15      @$keys = grep { defined $_; } @$keys;
               1                                  4   
402                                                   
...
409           24                   24            93      my ( $self, $primary_key, $unique_keys ) = @_;
410   ***     24     50                          92      die "I need a unique_keys argument" unless $unique_keys;
411           24                                 51      my %unique_cols;
412           24                                 57      my @unique_sets;
413           24                                 54      my %unconstrain;
414           24                                 51      my @unconstrained_keys;
415                                                   
416           24                                 47      MKDEBUG && _d('Unconstraining redundantly unique keys');
417                                                   
418                                                      # First determine which unique keys define unique columns
419                                                      # and which define unique sets.
420                                                      UNIQUE_KEY:
421           24                                 85      foreach my $unique_key ( $primary_key, @$unique_keys ) {
422           36    100                         136         next unless $unique_key; # primary key may be undefined
423           22                                 66         my $cols = $unique_key->{cols};
424           22    100                          79         if ( @$cols == 1 ) {
425           11                                 21            MKDEBUG && _d($unique_key->{name},'defines unique column:',$cols->[0]);
...
429   ***     11     50                          56            if ( !exists $unique_cols{$cols->[0]} ) {
430           11                                 36               $unique_cols{$cols->[0]}  = $unique_key;
431           11                                 52               $unique_key->{unique_col} = 1;
432                                                            }
433                                                         }
434                                                         else {
435           11                                 30            local $LIST_SEPARATOR = '-';
436           11                                 23            MKDEBUG && _d($unique_key->{name}, 'defines unique set:', @$cols);
437           11                                 62            push @unique_sets, { cols => $cols, key => $unique_key };
438                                                         }
439                                                      }
440                                                   
...
443                                                      UNIQUE_SET:
444           24                                 78      foreach my $unique_set ( @unique_sets ) {
445           11                                 28         my $n_unique_cols = 0;
446           11                                 41         COL:
447           11                                 32         foreach my $col ( @{$unique_set->{cols}} ) {
448           24    100                         101            if ( exists $unique_cols{$col} ) {
449            7                                 14               MKDEBUG && _d('Unique set', $unique_set->{key}->{name},
450                                                                  'has unique col', $col);
451   ***      7     50                          28               last COL if ++$n_unique_cols > 1;
452            7                                 30               $unique_set->{constraining_key} = $unique_cols{$col};
453                                                            }
454                                                         }
455           11    100    100                   87         if ( $n_unique_cols && $unique_set->{key}->{name} ne 'PRIMARY' ) {
456                                                            # Unique set is redundantly constrained.
457            6                                 13            MKDEBUG && _d('Will unconstrain unique set',
...
462            6                                 32            $unconstrain{$unique_set->{key}->{name}}
...
470           24                                116      for my $i ( 0..(scalar @$unique_keys-1) ) {
471           12    100                          60         if ( exists $unconstrain{$unique_keys->[$i]->{name}} ) {
472            6                                 12            MKDEBUG && _d('Unconstraining', $unique_keys->[$i]->{name});
473            6                                 23            $unique_keys->[$i]->{unconstrained} = 1;
474            6                                 32            $unique_keys->[$i]->{constraining_key}
475                                                               = $unconstrain{$unique_keys->[$i]->{name}};
476            6                                 18            push @unconstrained_keys, $unique_keys->[$i];
477            6                                 23            delete $unique_keys->[$i];
478                                                         }
479                                                      }
480                                                   
481           24                                 53      MKDEBUG && _d('No more keys');
482           24                                103      return @unconstrained_keys;
483                                                   }
484                                                   
485                                                   sub _d {
486            1                    1            22      my ($package, undef, $line) = caller 0;
