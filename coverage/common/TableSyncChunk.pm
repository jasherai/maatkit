...
OS:           linux
Start:        Fri Jul 31 18:53:39 2009
Finish:       Fri Jul 31 18:53:40 2009
...
44                                                                            dumper trim) ) {
45    ***     60     50                         257         die "I need a $arg argument" unless defined $args{$arg};
46                                                       }
47                                                    
48                                                       # Sanity check.  The row-level (state 2) checksums use __crc, so the table
49                                                       # had better not use that...
50             4                                 15      $args{crc_col} = '__crc';
51             4                                 27      while ( $args{struct}->{is_col}->{$args{crc_col}} ) {
52    ***      0                                  0         $args{crc_col} = "_$args{crc_col}"; # Prepend more _ until not a column.
53                                                       }
54             4                                 10      MKDEBUG && _d('CRC column will be named', $args{crc_col});
55                                                    
...
60             4                                 17      $args{index} = $idx;
61    ***      4     50                          17      if ( $col ) {
62             4                                 33         my %params = $args{chunker}->get_range_statistics(
63                                                             $args{dbh}, $args{database}, $args{table}, $col,
64                                                             $args{where});
65    ***      4     50                          18         if ( !grep { !defined $params{$_} }
              12                                 61   
66                                                                qw(min max rows_in_range) )
67                                                          {
68             4                                 46            @chunks = $args{chunker}->calculate_chunks(
69                                                                dbh      => $args{dbh},
...
93                                                          count       => 1,
94                                                       );
95             4                                 30      $args{func} = $args{checksum}->choose_hash_func(
96                                                          func => $args{func},
97                                                          dbh  => $args{dbh},
98                                                       );
99             4                                 32      $args{crc_wid}    = $args{checksum}->get_crc_wid($args{dbh}, $args{func});
100            4                                 24      ($args{crc_type}) = $args{checksum}->get_crc_type($args{dbh}, $args{func});
101   ***      4     50     33                   52      if ( $args{algorithm} eq 'BIT_XOR' && $args{crc_type} !~ m/int$/ ) {
102            4                                 31         $args{opt_slice}
103                                                            = $args{checksum}->optimize_xor(dbh => $args{dbh}, func => $args{func});
104                                                      }
105   ***      4            50                   81      $args{chunk_sql} ||= $args{checksum}->make_checksum_query(
...
118                                                      );
119   ***      4            50                   48      $args{row_sql} ||= $args{checksum}->make_row_checksum(
...
125                                                      );
126                                                   
127            4                                 13      $args{state} = 0;
128            4                                 29      $args{handler}->fetch_back($args{dbh});
129            4                                111      return bless { %args }, $class;
130                                                   }
131                                                   
...
135                                                   sub get_sql {
136            5                    5            69      my ( $self, %args ) = @_;
137            5    100                          24      if ( $self->{state} ) {
...
149                                                      }
150                                                      else {
151            4                                 55         return $self->{chunker}->inject_chunks(
...
168                                                   
169                                                   sub same_row {
170            3                    3            38      my ( $self, $lr, $rr ) = @_;
171   ***      3    100     33                   21      if ( $self->{state} ) {
      ***            50                               
172            2    100                          28         if ( $lr->{$self->{crc_col}} ne $rr->{$self->{crc_col}} ) {
173            1                                  7            $self->{handler}->change('UPDATE', $lr, $self->key_cols());
174                                                         }
175                                                      }
176                                                      elsif ( $lr->{cnt} != $rr->{cnt} || $lr->{crc} ne $rr->{crc} ) {
177            1                                  3         MKDEBUG && _d('Rows:', Dumper($lr, $rr));
178            1                                  2         MKDEBUG && _d('Will examine this chunk before moving to next');
179            1                                  5         $self->{state} = 1; # Must examine this chunk row-by-row
180                                                      }
181                                                   }
182                                                   
...
187            1                    1             4      my ( $self, $lr ) = @_;
188   ***      1     50                           6      die "Called not_in_right in state 0" unless $self->{state};
189            1                                  5      $self->{handler}->change('INSERT', $lr, $self->key_cols());
190                                                   }
191                                                   
192                                                   sub not_in_left {
193            2                    2            54      my ( $self, $rr ) = @_;
194            2    100                           8      die "Called not_in_left in state 0" unless $self->{state};
195            1                                  5      $self->{handler}->change('DELETE', $rr, $self->key_cols());
196                                                   }
197                                                   
198                                                   sub done_with_rows {
199            4                    4            17      my ( $self ) = @_;
200            4    100                          20      if ( $self->{state} == 1 ) {
201            1                                  4         $self->{state} = 2;
202            1                                  3         MKDEBUG && _d('Setting state =', $self->{state});
203                                                      }
204                                                      else {
205            3                                 10         $self->{state} = 0;
206            3                                 11         $self->{chunk_num}++;
207            3                                  9         MKDEBUG && _d('Setting state =', $self->{state},
...
215            1                                  2         scalar(@{$self->{chunks}}), 'chunks');
216            1                                  3      MKDEBUG && $self->{state} && _d('Chunk differs; must examine rows');
217            1                                 15      return $self->{state} == 0
218   ***      1            33                   11         && $self->{chunk_num} >= scalar(@{$self->{chunks}})
219                                                   }
220                                                   
221                                                   sub pending_changes {
222            3                    3            17      my ( $self ) = @_;
223            3    100                          14      if ( $self->{state} ) {
224            2                                  5         MKDEBUG && _d('There are pending changes');
225            2                                 13         return 1;
226                                                      }
227                                                      else {
...
232                                                   
233                                                   sub key_cols {
234            5                    5            19      my ( $self ) = @_;
235            5                                 15      my @cols;
236            5    100                          25      if ( $self->{state} == 0 ) {
237            1                                  5         @cols = qw(chunk_num);
238                                                      }
239                                                      else {
240            4                                 18         @cols = $self->{chunk_col};
241                                                      }
242            5                                 10      MKDEBUG && _d('State', $self->{state},',', 'key cols', join(', ', @cols));
243            5                                 37      return \@cols;
244                                                   }
245                                                   
246                                                   sub _d {
