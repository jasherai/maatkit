...
OS:           linux
Start:        Fri Jul 31 18:53:27 2009
Finish:       Fri Jul 31 18:53:28 2009
...
17                                                    # ###########################################################################
18                                                    # SlowLogParser package $Revision: 3980 $
19                                                    # ###########################################################################
...
70                                                    sub parse_event {
71            89                   89          3606      my ( $self, $fh, $misc, @callbacks ) = @_;
72            89                                253      my $oktorun_here = 1;
73            89    100                         412      my $oktorun      = $misc->{oktorun} ? $misc->{oktorun} : \$oktorun_here;
74            89                                221      my $num_events   = 0;
75                                                    
...
83            89                                204      my @pending;
84            89                                391      local $INPUT_RECORD_SEPARATOR = ";\n#";
85            89                                259      my $trimlen    = length($INPUT_RECORD_SEPARATOR);
86            89                                274      my $pos_in_log = tell($fh);
87            89                                198      my $stmt;
88                                                    
89                                                       EVENT:
90            89           100                 1508      while ( $$oktorun
                           100                        
91                                                               && (defined($stmt = shift @pending) or defined($stmt = <$fh>)) ) {
92            66                                274         my @properties = ('cmd', 'Query', 'pos_in_log', $pos_in_log);
93            66                                192         $pos_in_log = tell($fh);
94                                                    
...
98            66    100                         755         if ( $stmt =~ s/$slow_log_hd_line//go ){ # Throw away header lines in log
99             6                                 50            my @chunks = split(/$INPUT_RECORD_SEPARATOR/o, $stmt);
100            6    100                          32            if ( @chunks > 1 ) {
...
109           66    100                         333         $stmt = '#' . $stmt unless $stmt =~ m/\A#/;
110           66                                318         $stmt =~ s/;\n#?\Z//;
111                                                   
...
120                                                         # regexes.
121           66                                204         my ($got_ts, $got_uh, $got_ac, $got_db, $got_set, $got_embed);
122           66                                161         my $pos = 0;
123           66                                171         my $len = length($stmt);
124           66                                158         my $found_arg = 0;
125                                                         LINE:
126           66                                343         while ( $stmt =~ m/^(.*)$/mg ) { # /g is important, requires scalar match.
127          425                               1167            $pos     = pos($stmt);  # Be careful not to mess this up!
128          425                               1381            my $line = $1;          # Necessary for /g and pos() to work.
129          425                                898            MKDEBUG && _d($line);
130                                                   
...
134          425    100                        1752            if ($line =~ m/^(?:#|use |SET (?:last_insert_id|insert_id|timestamp))/o) {
135                                                   
...
141          366    100    100                 6300               if ( !$got_ts && (my ( $time ) = $line =~ m/$slow_log_ts_line/o)) {
                    100    100                        
      ***           100     66                        
                    100    100                        
      ***           100     66                        
...
143           29                                 97                  push @properties, 'ts', $time;
144           29                                 71                  ++$got_ts;
145                                                                  # The User@Host might be concatenated onto the end of the Time.
146   ***     29    100     66                  331                  if ( !$got_uh
147                                                                     && ( my ( $user, $host, $ip ) = $line =~ m/$slow_log_uh_line/o )
148                                                                  ) {
149           12                                 26                     MKDEBUG && _d("Got user, host, ip", $user, $host, $ip);
150           12                                 55                     push @properties, 'user', $user, 'host', $host, 'ip', $ip;
151           12                                 35                     ++$got_uh;
152                                                                  }
153                                                               }
154                                                   
...
159                                                               ) {
160           53                                109                  MKDEBUG && _d("Got user, host, ip", $user, $host, $ip);
161           53                                225                  push @properties, 'user', $user, 'host', $host, 'ip', $ip;
162           53                                149                  ++$got_uh;
163                                                               }
164                                                   
...
168            4                                  9                  MKDEBUG && _d("Got admin command");
169            4                                 16                  push @properties, 'cmd', 'Admin', 'arg', $line;
170            4                                 16                  push @properties, 'bytes', length($properties[-1]);
171            4                                  9                  ++$found_arg;
172            4                                 12                  ++$got_ac;
173                                                               }
174                                                   
...
182          214                                471                  MKDEBUG && _d("Got some line with properties");
183                                                                  # I tried using split, but coping with the above bug makes it
184                                                                  # slower than a complex regex match.
185          214                               2331                  my @temp = $line =~ m/(\w+):\s+(\d+(?:\.\d+)?|\S+|\Z)/g;
186          214                               1122                  push @properties, @temp;
187                                                               }
188                                                   
...
192           26                                 54                  MKDEBUG && _d("Got a default database:", $db);
193           26                                 82                  push @properties, 'db', $db;
194           26                                 70                  ++$got_db;
195                                                               }
196                                                   
...
207            9                                 21                  MKDEBUG && _d("Got some setting:", $setting);
208            9                                 97                  push @properties, split(/,|\s*=\s*/, $setting);
209            9                                 26                  ++$got_set;
210                                                               }
211                                                   
...
217          366    100    100                 3788               if ( !$found_arg && $pos == $len ) {
218            3                                  6                  MKDEBUG && _d("Did not find arg, looking for special cases");
219            3                                 15                  local $INPUT_RECORD_SEPARATOR = ";\n";
220            3    100                          17                  if ( defined(my $l = <$fh>) ) {
221            2                                  7                     chomp $l;
222            2                                  5                     MKDEBUG && _d("Found admin statement", $l);
223            2                                  9                     push @properties, 'cmd', 'Admin', 'arg', '#' . $l;
224            2                                  7                     push @properties, 'bytes', length($properties[-1]);
225            2                                 15                     $found_arg++;
226                                                                  }
227                                                                  else {
...
244           59                                120               MKDEBUG && _d("Got the query/arg line");
245           59                                240               my $arg = substr($stmt, $pos - length($line));
246           59                                234               push @properties, 'arg', $arg, 'bytes', length($arg);
247                                                               # Handle embedded attributes.
248   ***     59    100     66                  577               if ( $misc && $misc->{embed}
      ***                   66                        
249                                                                  && ( my ($e) = $arg =~ m/($misc->{embed})/)
250                                                               ) {
251            1                                 12                  push @properties, $e =~ m/$misc->{capture}/g;
252                                                               }
253           59                                168               last LINE;
254                                                            }
255                                                         }
256                                                   
257                                                         # Don't dump $event; want to see full dump of all properties, and after
258                                                         # it's been cast into a hash, duplicated keys will be gone.
259           65                                139         MKDEBUG && _d('Properties of event:', Dumper(\@properties));
260           65                                728         my $event = { @properties };
261           65                                217         foreach my $callback ( @callbacks ) {
262           72    100                         384            last unless $event = $callback->($event);
263                                                         }
264           65                                702         ++$num_events;
265           65    100                         412         last EVENT unless @pending;
266                                                      }
267           89                                987      return $num_events;
268                                                   }
269                                                   
270                                                   sub _d {
271            1                    1            22      my ($package, undef, $line) = caller 0;
272   ***      2     50                           8      @_ = map { (my $temp = $_) =~ s/\n/\n# /g; $temp; }
               2                                  8   
               2                                 11   
273            1                                  4           map { defined $_ ? $_ : 'undef' }
274                                                           @_;
275            1                                  4      print STDERR "# $package:$line $PID ", join(' ', @_), "\n";
276                                                   }
277                                                   
278                                                   1;
279                                                   
280                                                   # ###########################################################################
281                                                   # End SlowLogParser package
282                                                   # ###########################################################################
...
141          100     29    337   if (not $got_ts and my($time) = $line =~ /$slow_log_ts_line/o) { }
             ...
             100    214     66   elsif ($line =~ /^# +[A-Z][A-Za-z_]+: \S+/) { }
             100     26     40   elsif (not $got_db and my($db) = $line =~ /^use ([^;]+)/) { }
             100      9     31   elsif (not $got_set and my($setting) = $line =~ /^SET\s+([^;]*)/) { }
146          100     12     17   if (not $got_uh and my($user, $host, $ip) = $line =~ /$slow_log_uh_line/o)
217          100      3    363   if (not $found_arg and $pos == $len)
220          100      2      1   if (defined(my $l = <$fh>)) { }
248          100      1     58   if ($misc and $$misc{'embed'} and my($e) = $arg =~ /($$misc{'embed'})/)
262          100      3     69   unless $event = &$callback($event)
265          100     64      1   unless @pending
272   ***     50      2      0   defined $_ ? :
...
141          100    107    230     29   not $got_ts and my($time) = $line =~ /$slow_log_ts_line/o
             100    282      2     53   not $got_uh and my($user, $host, $ip) = $line =~ /$slow_log_uh_line/o
      ***     66      0    280      4   not $got_ac and $line =~ /^# (?:administrator command:.*)$/
             100      3     37     26   not $got_db and my($db) = $line =~ /^use ([^;]+)/
      ***     66      0     31      9   not $got_set and my($setting) = $line =~ /^SET\s+([^;]*)/
146   ***     66      0     17     12   not $got_uh and my($user, $host, $ip) = $line =~ /$slow_log_uh_line/o
217          100      4    359      3   not $found_arg and $pos == $len
248   ***     66      0     58      1   $misc and $$misc{'embed'}
...
_d              1 /home/daniel/dev/maatkit/common/SlowLogParser.pm:271
