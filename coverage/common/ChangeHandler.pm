...
OS:           linux
Start:        Sat Aug 29 15:01:30 2009
Finish:       Sat Aug 29 15:01:31 2009
...
20             1                    1             8   use strict;
               1                                  3   
               1                                  7   
21             1                    1           110   use warnings FATAL => 'all';
               1                                  3   
               1                                  8   
22                                                    
...
44             2                                 10      foreach my $arg ( qw(quoter database table sdatabase stable replace queue)
45                                                       ) {
46             8    100                          32         die "I need a $arg argument" unless defined $args{$arg};
47                                                       }
48             1                                  8      my $self = { %args, map { $_ => [] } @ACTIONS };
               4                                 27   
49             1                                 10      $self->{db_tbl}  = $self->{quoter}->quote(@args{qw(database table)});
50             1                                  6      $self->{sdb_tbl} = $self->{quoter}->quote(@args{qw(sdatabase stable)});
51             1                                  5      $self->{changes} = { map { $_ => 0 } @ACTIONS };
               4                                 19   
...
60                                                    # to fetch.
61                                                    sub fetch_back {
62             1                    1             6      my ( $self, $dbh ) = @_;
63             1                                 12      $self->{fetch_back} = $dbh;
64             1                                  5      MKDEBUG && _d('Will fetch rows from source when updating destination');
65                                                    }
66                                                    
67                                                    sub take_action {
68             6                    6            30      my ( $self, @sql ) = @_;
69             6                                 13      MKDEBUG && _d('Calling subroutines on', @sql);
70             6                                 14      foreach my $action ( @{$self->{actions}} ) {
               6                                 27   
71             6                                 28         $action->(@sql);
72                                                       }
73                                                    }
...
76                                                    sub change {
77             6                    6            85      my ( $self, $action, $row, $cols ) = @_;
78             6                                 15      MKDEBUG && _d($action, 'where', $self->make_where_clause($row, $cols));
79                                                       $self->{changes}->{
80    ***      6     50     33                   47         $self->{replace} && $action ne 'DELETE' ? 'REPLACE' : $action
81                                                       }++;
82             6    100                          23      if ( $self->{queue} ) {
83             2                                  9         $self->__queue($action, $row, $cols);
84                                                       }
85                                                       else {
86             4                                 17         eval {
87             4                                 15            my $func = "make_$action";
88             4                                 32            $self->take_action($self->$func($row, $cols));
89                                                          };
90    ***      4     50                          70         if ( $EVAL_ERROR =~ m/$DUPE_KEY/ ) {
      ***            50                               
...
104            2                                  8      MKDEBUG && _d('Queueing change for later');
105   ***      2     50                           9      if ( $self->{replace} ) {
...
114            3                    3            31      my ( $self, $queue_level ) = @_;
115            3                                 10      my $error_count = 0;
116                                                      TRY: {
117            3    100    100                    6         if ( $queue_level && $queue_level < $self->{queue} ) { # see redo below!
               3                                 26   
118            1                                  3            MKDEBUG && _d('Not processing now', $queue_level, '<', $self->{queue});
119            1                                  4            return;
120                                                         }
121                                                   
...
125            8                                 34               my $func = "make_$action";
126            8                                 27               my $rows = $self->{$action};
127            8                                 17               MKDEBUG && _d(scalar(@$rows), 'to', $action);
128            8                                 21               $cur_act = $action;
129            8                                 34               while ( @$rows ) {
130            2                                  7                  $row = shift @$rows;
131            2                                 11                  $self->take_action($self->$func(@$row));
132                                                               }
133                                                            }
134            2                                 17            $error_count = 0;
135                                                         };
136   ***      2     50     33                   30         if ( !$error_count++ && $EVAL_ERROR =~ m/$DUPE_KEY/ ) {
      ***            50                               
137   ***      0                                  0            MKDEBUG
...
160   ***      2     50                          12      if ( $self->{replace} ) {
161   ***      0                                  0         return $self->make_row('REPLACE', $row, $cols);
162                                                      }
163            2                                  9      my %in_where = map { $_ => 1 } @$cols;
               2                                 19   
164            2                                 16      my $where = $self->make_where_clause($row, $cols);
165            2    100                          14      if ( my $dbh = $self->{fetch_back} ) {
166            1                                 11         my $sql = "SELECT * FROM $self->{sdb_tbl} WHERE $where LIMIT 1";
167            1                                  2         MKDEBUG && _d('Fetching data for UPDATE:', $sql);
168            1                                  4         my $res = $dbh->selectrow_hashref($sql);
169            1                                 10         @{$row}{keys %$res} = values %$res;
               1                                  7   
170            1                                 14         $cols = [sort keys %$res];
171                                                      }
172                                                      else {
173            1                                  9         $cols = [ sort keys %$row ];
174                                                      }
175            2                                 12      return "UPDATE $self->{db_tbl} SET "
176                                                         . join(', ', map {
177            4                                 41               $self->{quoter}->quote($_)
178                                                               . '=' .  $self->{quoter}->quote_val($row->{$_})
179            2                                 16            } grep { !$in_where{$_} } @$cols)
180                                                         . " WHERE $where LIMIT 1";
181                                                   }
182                                                   
183                                                   sub make_INSERT {
184            2                    2             9      my ( $self, $row, $cols ) = @_;
...
195                                                   
196                                                   sub make_row {
197            2                    2            11      my ( $self, $verb, $row, $cols ) = @_;
198            2                                 18      my @cols = sort keys %$row;
199            2    100                          12      if ( my $dbh = $self->{fetch_back} ) {
200            1                                  4         my $where = $self->make_where_clause($row, $cols);
201            1                                  5         my $sql = "SELECT * FROM $self->{sdb_tbl} WHERE $where LIMIT 1";
202            1                                  3         MKDEBUG && _d('Fetching data for UPDATE:', $sql);
203            1                                  2         my $res = $dbh->selectrow_hashref($sql);
204            1                                  9         @{$row}{keys %$res} = values %$res;
               1                                  6   
205            1                                  9         @cols = sort keys %$res;
206                                                      }
207            4                                 18      return "$verb INTO $self->{db_tbl}("
208            2                                 10         . join(', ', map { $self->{quoter}->quote($_) } @cols)
209                                                         . ') VALUES ('
...
216            5                                 16      my @clauses = map {
217            5                                 17         my $val = $row->{$_};
218   ***      5     50                          22         my $sep = defined $val ? '=' : ' IS ';
219            5                                 35         $self->{quoter}->quote($_) . $sep . $self->{quoter}->quote_val($val);
220                                                      } @$cols;
221            5                                 31      return join(' AND ', @clauses);
222                                                   }
223                                                   
224                                                   sub get_changes {
225            1                    1            14      my ( $self ) = @_;
226            1                                  2      return %{$self->{changes}};
               1                                 13   
227                                                   }
228                                                   
229                                                   sub _d {
