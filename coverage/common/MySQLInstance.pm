...
OS:           linux
Start:        Sat Aug 29 15:03:17 2009
Finish:       Sat Aug 29 15:03:18 2009
...
111                                                   {
112            2                    2            54      my ( $ps_output ) = @_;
113            2                                 10      my @mysqld_processes;
114            2                                  9      my $cmd = 'ps -o euser,%cpu,rss,vsz,cmd -e | grep -v grep | grep mysql';
115   ***      2     50                          19      my $ps  = defined $ps_output ? $ps_output : `$cmd`;
116   ***      2     50                          12      if ( $ps ) {
117            2                                  5         MKDEBUG && _d('ps full output:', $ps);
118            2                                 27         foreach my $line ( split("\n", $ps) ) {
119           25                                 84            MKDEBUG && _d('ps line:', $line);
120           25                                260            my ($user, $pcpu, $rss, $vsz, $cmd) = split(/\s+/, $line, 5);
121           25                                166            my $bin = find_mysqld_binary_unix($cmd);
122           25    100                         111            if ( !$bin ) {
123           19                                 44               MKDEBUG && _d('No mysqld binary in ps line');
124           19                                 71               next;
125                                                            }
126            6                                 17            MKDEBUG && _d('mysqld binary from ps:', $bin);
127            6    100                       47677            push @mysqld_processes,
                    100                               
128                                                               { user    => $user,
129                                                                 pcpu    => $pcpu,
130                                                                 rss     => $rss,
...
138                                                         }
139                                                      }
140            2                                 45      MKDEBUG && _d('mysqld processes:', Dumper(\@mysqld_processes));
141            2                                 46      return \@mysqld_processes;
142                                                   }
143                                                   
144                                                   sub new {
145           11                   11           277      my ( $class, $cmd ) = @_;
146           11                                 42      my $self = {};
147           11                                 26      MKDEBUG && _d('cmd:', $cmd);
148           11    100                          58      $self->{mysqld_binary} = find_mysqld_binary_unix($cmd)
149                                                         or die "No mysqld binary found in $cmd";
150           10                              84515      my $file_output  = `file $self->{mysqld_binary} 2>&1`;
151           10                                270      $self->{regsize} = get_register_size($file_output);
152           10                                292      %{ $self->{cmd_line_ops} }
              76                                652   
153                                                         = map {
154           10                                260              my ( $var, $val ) = m/$option_pattern/o;
155           76                                314              $var =~ s/-/_/go;
156           76           100                  314              $val ||= $undef_for{$var} || '';
                           100                        
157           76                                301              $var => $val;
158                                                           } ($cmd =~ m/--(\S+)/g);
159           10           100                  100      $self->{cmd_line_ops}->{defaults_file} ||= '';
160           10                                 54      $self->{conf_sys_vars}   = {};
161           10                                 53      $self->{online_sys_vars} = {};
162           10                                 31      MKDEBUG && _d('new MySQLInstance:', Dumper($self));
163           10                                232      return bless $self, $class;
164                                                   }
165                                                   
166                                                   # Extracts the register size (64-bit, 32-bit, ???) from the output of 'file'.
167                                                   sub get_register_size {
168           12                   12           650      my ( $file_output ) = @_;
169           12                                787      my ( $size ) = $file_output =~ m/\b(\d+)-bit/;
170           12           100                 1760      return $size || 0;
171                                                   }
172                                                   
173                                                   sub find_mysqld_binary_unix {
174           39                   39           272      my ( $cmd ) = @_;
175           39                                427      my ( $binary ) = $cmd =~ m/(\S+mysqld)\b(?=\s|\Z)/;
176           39           100                  375      return $binary || '';
177                                                   }
178                                                   
179                                                   sub load_sys_vars {
180            2                    2            71      my ( $self, $dbh ) = @_;
181                                                   
...
192            2                                 20      my $cmd = "$self->{mysqld_binary} $defaults_file_op --help --verbose";
193            2                                  5      MKDEBUG && _d('Getting sys vars from mysqld:', $cmd);
194            2                              28394      my $retval = system("$cmd 1>/dev/null 2>/dev/null");
195            2                                 23      $retval = $retval >> 8;
196   ***      2     50                          31      if ( $retval != 0 ) {
197   ***      0                                  0         MKDEBUG && _d('self dump:', Dumper($self));
198   ***      0                                  0         warn "Cannot execute $cmd\n" . $mysqld_broken_msg;
199                                                      }
200                                                      else {
201            2    100                        9902         if ( my $mysqld_output = `$cmd` ) {
202                                                            # Parse from mysqld output the list of sys vars and their
203                                                            # default values listed at the end after all the help info.
204            1                                228            my ($sys_vars) = $mysqld_output =~ m/---\n(.*?)\n\n/ms;
205            1                                340            %{ $self->{conf_sys_vars} }
             258                               1479   
206                                                               = map {
207            1                                 71                    my ( $var, $val ) = m/^(\S+)\s+(?:(\S+))?/;
208          258                                918                    $var =~ s/-/_/go;
209          258    100    100                 1782                    if ( $val && $val =~ m/\(No/ ) { # (No default value)
210           36                                103                       $val = undef;
211                                                                    }
212          258           100                 1102                    $val ||= $undef_for{$var} || '';
                           100                        
213          258                                927                    $var => $val;
214                                                                 } split "\n", $sys_vars;
215                                                   
...
222            1                                 84            $self->_load_default_defaults_files($mysqld_output);
223                                                         }
224                                                         else {
...
231            2                                 44      $self->_load_online_sys_vars($dbh);
232                                                   
...
238            2                                 15      $self->{defaults_files_sys_vars}
239                                                         = $self->_vars_from_defaults_file($defaults_file_op); 
240            2                                  8      foreach my $var_val ( reverse @{ $self->{defaults_file_sys_vars} } ) {
               2                                 18   
241           44                                189         my ( $var, $val ) = ( $var_val->[0], $var_val->[1] );
242           44    100                         215         if ( !exists $self->{conf_sys_vars}->{$var} ) {
243           22                                147            $self->{conf_sys_vars}->{$var} = $val;
244                                                         }
245           44    100                         222         if ( !exists $self->{online_sys_vars}->{$var} ) {
246            4                                 25            $self->{online_sys_vars}->{$var} = $val;
247                                                         }
248                                                      }
249                                                   
250            2                                 16      return;
...
256            2                    2             8      my ( $self, $ddf )   = @_;  # ddf = default defaults file (optional)
257            2                                  9      my $defaults_file_op = '';
258            2                                  6      my $tmp_file         = undef;
259   ***      2     50                          20      my $defaults_file    = defined $ddf ? $ddf
260                                                                           : $self->{cmd_line_ops}->{defaults_file};
261                                                   
262   ***      2     50     33                   52      if ( $defaults_file && -f $defaults_file ) {
...
265                                                         # locations.
266            2                                 52         $tmp_file = File::Temp->new();
267            2                                 14         my $cp_cmd = "cp $defaults_file "
268                                                                    . $tmp_file->filename;
269            2                              23409         `$cp_cmd`;
270            2                                 95         $defaults_file_op = "--defaults-file=" . $tmp_file->filename;
271                                                   
...
281            2                                 37      return ( $defaults_file_op, $tmp_file );
282                                                   }
283                                                   
...
288            2                    2           151      my ( $self, $mysqld_output ) = @_;
289            2                                553      my ( $ddf_list ) = $mysqld_output =~ /Default options.+order:\n(.*?)\n/ms;
...
295            2                                  7      my %have_seen;
296            2                                 14      @{ $self->{default_defaults_files} }
               6                                 33   
...
305            4                    4            47      my ( $self, $defaults_file_op, $my_print_defaults ) = @_;
306                                                   
...
311            4                              43462      my $retval = system("$my_print_defaults_cmd --help 1>/dev/null 2>/dev/null");
312            4                                 41      $retval = $retval >> 8;
313            4    100                          56      if ( $retval != 0 ) {
314            1                                 15         MKDEBUG && _d('self dump:', Dumper($self));
315            1                                 12         die "Cannot execute my_print_defaults command '$my_print_defaults_cmd'";
316                                                      }
317                                                   
318            3                                 14      my @defaults_file_ops;
319            3                                 11      my @ddf_ops;
320                                                   
321            3    100                          44      if( !$defaults_file_op ) {
...
327                                                   
328            1                                 20         foreach my $ddf ( @{ $self->{default_defaults_files} } ) {
               1                                 30   
...
338                                                      }
339                                                   
340            3    100                          34      if ( scalar @defaults_file_ops == 0 ) {
...
344            1                                  8         MKDEBUG && _d('self dump:', Dumper($self));
345                                                         # TODO: change to warn and try to continue
346            1                                 11         die 'MySQL instance has no valid defaults files.'
347                                                      }
348                                                   
349            2                                 20      foreach my $defaults_file_op ( @defaults_file_ops ) {
350            2                                 21         my $cmd = "$my_print_defaults_cmd $defaults_file_op mysqld";
351            2                                  7         MKDEBUG && _d('my_print_defaults cmd:', $cmd);
352   ***      2     50                       14297         if ( my $my_print_defaults_output = `$cmd` ) {
353            2                                 65            foreach my $var_val ( split "\n", $my_print_defaults_output ) {
...
357           44                                404               my ( $var, $val ) = $var_val =~ m/^--$option_pattern/o;
358           44                                167               $var =~ s/-/_/go;
...
365           44    100    100                  395               if ( defined $val && $val =~ /(\d+)([kKmMgGtT]?)/) {
366           34    100                         167                  if ( $2 ) {
367            4                                 65                     my %digits_for = (
368                                                                        'k'   => 1_024,
369                                                                        'K'   => 1_204,
370                                                                        'm'   => 1_048_576,
371                                                                        'M'   => 1_048_576,
...
378                                                                  }
379                                                               }
380   ***     44            50                  170               $val ||= $undef_for{$var} || '';
                           100                        
381           44                                102               push @{ $self->{defaults_file_sys_vars} }, [ $var, $val ];
              44                                310   
382                                                            }
383                                                         }
384                                                      }
...
390            2                                551      %{ $self->{online_sys_vars} }
             480                               2173   
391            2                                 90         = map { $_->{Variable_name} => $_->{Value} }
392            2                                  6               @{ $dbh->selectall_arrayref('SHOW /*!40101 GLOBAL*/ VARIABLES',
393                                                                                           { Slice => {} })
394                                                               };
395            2                                251      return;
396                                                   }
...
400                                                   # to 127.0.0.1.
401                                                   sub get_DSN {
402            6                    6           422      my ( $self, $o ) = @_;
403   ***      6     50                          49      die 'I need an OptionParser object' unless ref $o eq 'OptionParser';
404   ***      6            50                   51      my $port   = $self->{cmd_line_ops}->{port} || '';
...
420            2                                  8      my @duplicate_vars;
421            2                                  5      my %have_seen;
422            2                                  6      foreach my $var_val ( @{ $self->{defaults_file_sys_vars} } ) {
               2                                  9   
423           36                                157         my ( $var, $val ) = ( $var_val->[0], $var_val->[1] );
424           36    100                         153         next if $can_be_duplicate{$var};
425           22    100                         119         push @duplicate_vars, $var if $have_seen{$var}++ == 1;
426                                                      }
...
436            1                                  3      foreach my $var_val ( @{ $self->{defaults_file_sys_vars} } ) {
               1                                  5   
437           22                                102         my ( $var, $val ) = ( $var_val->[0], $var_val->[1] );
438   ***     22     50     33                  164         if ( !defined $var || !defined $val ) {
439   ***      0                                  0            MKDEBUG && _d('Undefined var or val:', Dumper($var_val));
440   ***      0                                  0            next;
441                                                         }
442           22    100                         100         if ( exists $self->{cmd_line_ops}->{$var} ) {
443   ***      8    100     33                  106            if(    ( !defined $self->{cmd_line_ops}->{$var} && !defined $val)
      ***                   66                        
444                                                                || ( $self->{cmd_line_ops}->{$var} ne $val) ) {
445            2                                 14               $overriden_vars{$var} = [ $self->{cmd_line_ops}->{$var}, $val ];
446                                                            }
447                                                         }
448                                                      }
...
462            3                    3           320      my ( $self ) = @_;
463            3                                 16      my %out_of_sync_vars;
464                                                   
465            3                                 89      VAR:
466            3                                 11      foreach my $var ( keys %{ $self->{conf_sys_vars} } ) {
467          259    100                         888         next VAR if exists $ignore_sys_var{$var};
468          256    100                        1148         next VAR unless exists $self->{online_sys_vars}->{$var};
469                                                   
470          183                                642         my $conf_val        = $self->{conf_sys_vars}->{$var};
471          183                                630         my $online_val      = $self->{online_sys_vars}->{$var};
472          183                                429         my $var_out_of_sync = 0;
473                                                   
...
482          183    100    100                 1533         if ( ($conf_val || $online_val) && ($conf_val ne $online_val) ) {
                           100                        
483           41                                100            $var_out_of_sync = 1;
484                                                   
485                                                            # Try some exceptions, cases like where ON and TRUE are the
486                                                            # same to us but not to Perl.
487           41    100                         147            if ( exists $eq_for{$var} ) {
488                                                               # If they're equal then they're not (!) out of sync
489            9                                 51               $var_out_of_sync = !$eq_for{$var}->($conf_val, $online_val);
490                                                            }
491           41    100                         156            if ( exists $alias_for{$online_val} ) {
492           34    100                         154               $var_out_of_sync = 0 if $conf_val eq $alias_for{$online_val};
493                                                            }
494                                                         }
495                                                   
496          183    100                         671         if ( $var_out_of_sync ) {
497            2                                 17            $out_of_sync_vars{$var} = { online=>$online_val, config=>$conf_val };
498                                                         }
499                                                      }
500                                                   
501            3                                 30      return \%out_of_sync_vars;
502                                                   }
503                                                   
504                                                   sub load_status_vals {
505            1                    1            17      my ( $self, $dbh ) = @_;
506            1                                302      %{ $self->{status_vals} }
             253                               1114   
507            1                                 19         = map { $_->{Variable_name} => $_->{Value} }
...
514                                                   sub get_eq_for {
515            5                    5            32      my ( $var ) = @_;
516   ***      5     50                          29      if ( exists $eq_for{$var} ) {
517            5                                 24         return $eq_for{$var};
518                                                      }
519   ***      0                                  0      return;
520                                                   }
521                                                   
...
524                                                   sub _veq { 
525            5                    5            27      my ( $x, $y, $val1, $val2 ) = @_;
526   ***      5     50     33                   97      return 1 if ( ($x eq $val1 || $x eq $val2) && ($y eq $val1 || $y eq $val2) );
      ***                   33                        
      ***                   33                        
527   ***      0                                  0      return 0;
528                                                   }
...
532                                                   sub _patheq {
533            2                    2            11      my ( $x, $y ) = @_;
534   ***      2     50                          14      $x .= '/' if $x !~ m/\/$/;
535   ***      2     50                          14      $y .= '/' if $y !~ m/\/$/;
536            2                                 11      return $x eq $y;
537                                                   }
538                                                   
...
541                                                   sub _eqifon { 
542            2                    2             9      my ( $x, $y ) = @_;
543   ***      2     50     33                   23      return 1 if ( $x && $x eq 'ON' && $y );
      ***                   33                        
