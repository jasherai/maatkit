...
---------------------------- ------ ------ ------ ------ ------ ------ ------
...kit/common/TableParser.pm   91.4   70.5   50.0   93.3    n/a  100.0   83.7
Total                          91.4   70.5   50.0   93.3    n/a  100.0   83.7
---------------------------- ------ ------ ------ ------ ------ ------ ------
...
OS:           linux
Start:        Fri Jul 31 18:53:37 2009
Finish:       Fri Jul 31 18:53:38 2009
...
17                                                    # ###########################################################################
18                                                    # TableParser package $Revision: 4161 $
19                                                    # ###########################################################################
...
44                                                    
45                                                    sub parse {
46            14                   14           193      my ( $self, $ddl, $opts ) = @_;
47            14    100                          62      return unless $ddl;
...
57                                                       }
58                                                    
59            13    100                          99      if ( $ddl !~ m/CREATE (?:TEMPORARY )?TABLE `/ ) {
60             2                                  5         die "Cannot parse table definition; is ANSI quoting "
61                                                             . "enabled or SQL_QUOTE_SHOW_CREATE disabled?";
62                                                       }
63                                                    
64                                                       # Lowercase identifiers to avoid issues with case-sensitivity in Perl.
65                                                       # (Bug #1910276).
66            11                                342      $ddl =~ s/(`[^`]+`)/\L$1/g;
67                                                    
68            11                                 50      my $engine = $self->get_engine($ddl);
69                                                    
70            11                                353      my @defs   = $ddl =~ m/^(\s+`.*?),?$/gm;
71            11                                 42      my @cols   = map { $_ =~ m/`([^`]+)`/ } @defs;
              48                                243   
72            11                                 28      MKDEBUG && _d('Columns:', join(', ', @cols));
73                                                    
74                                                       # Save the column definitions *exactly*
75            11                                 28      my %def_for;
76            11                                 73      @def_for{@cols} = @defs;
77                                                    
78                                                       # Find column types, whether numeric, whether nullable, whether
79                                                       # auto-increment.
80            11                                 32      my (@nums, @null);
81            11                                 34      my (%type_for, %is_nullable, %is_numeric, %is_autoinc);
82            11                                 38      foreach my $col ( @cols ) {
83            48                                145         my $def = $def_for{$col};
84            48                                267         my ( $type ) = $def =~ m/`[^`]+`\s([a-z]+)/;
85    ***     48     50                         177         die "Can't determine column type for $def" unless $type;
86            48                                156         $type_for{$col} = $type;
87            48    100                         284         if ( $type =~ m/(?:(?:tiny|big|medium|small)?int|float|double|decimal|year)/ ) {
88            26                                 77            push @nums, $col;
89            26                                 87            $is_numeric{$col} = 1;
90                                                          }
91            48    100                         195         if ( $def !~ m/NOT NULL/ ) {
92            21                                 63            push @null, $col;
93            21                                 73            $is_nullable{$col} = 1;
94                                                          }
95            48    100                         252         $is_autoinc{$col} = $def =~ m/AUTO_INCREMENT/i ? 1 : 0;
96                                                       }
97                                                    
...
101           11                                 59      my $keys = $self->get_keys($ddl, $opts, \%is_nullable);
102                                                   
103                                                      return {
104           48                                212         cols           => \@cols,
105           48                                295         col_posn       => { map { $cols[$_] => $_ } 0..$#cols },
106           11                                 93         is_col         => { map { $_ => 1 } @cols },
107                                                         null_cols      => \@null,
...
123            2                    2            36      my ( $self, $tbl ) = @_;
124                                                   
125                                                      my @indexes
126            2                                  8         = sort {
127            8                                 31            (($a ne 'PRIMARY') <=> ($b ne 'PRIMARY'))
128                                                            || ( !$tbl->{keys}->{$a}->{is_unique} <=> !$tbl->{keys}->{$b}->{is_unique} )
129                                                            || ( $tbl->{keys}->{$a}->{is_nullable} <=> $tbl->{keys}->{$b}->{is_nullable} )
130   ***      8    100     66                  110            || ( scalar(@{$tbl->{keys}->{$a}->{cols}}) <=> scalar(@{$tbl->{keys}->{$b}->{cols}}) )
               2           100                   13   
131                                                         }
132                                                         grep {
133            2                                 17            $tbl->{keys}->{$_}->{type} eq 'BTREE'
134                                                         }
135            2                                  6         sort keys %{$tbl->{keys}};
136                                                   
...
144            3                    3            14      my ( $self, $tbl, $index ) = @_;
145            3                                  8      my $best;
146            3    100                          13      if ( $index ) {
147            2                                  5         ($best) = grep { uc $_ eq uc $index } keys %{$tbl->{keys}};
               8                                 29   
               2                                 11   
148                                                      }
149            3    100                          14      if ( !$best ) {
150            2    100                           7         if ( $index ) {
...
157            1                                  4            ($best) = $self->sort_indexes($tbl);
158                                                         }
159                                                      }
160            2                                  5      MKDEBUG && _d('Best index found is', $best);
...
170            2                    2           423      my ( $self, $dbh, $database, $table, $quoter, $where ) = @_;
171   ***      2     50                           9      return () unless $where;
172            2                                 11      my $sql = 'EXPLAIN SELECT * FROM ' . $quoter->quote($database, $table)
173                                                         . ' WHERE ' . $where;
174            2                                  7      MKDEBUG && _d($sql);
175            2                                  5      my $expl = $dbh->selectrow_hashref($sql);
176                                                      # Normalize columns to lowercase
177            2                                 17      $expl = { map { lc($_) => $expl->{$_} } keys %$expl };
              20                                 87   
178   ***      2     50                          17      if ( $expl->{possible_keys} ) {
179            2                                  5         MKDEBUG && _d('possible_keys =', $expl->{possible_keys});
180            2                                 17         my @candidates = split(',', $expl->{possible_keys});
181            2                                  7         my %possible   = map { $_ => 1 } @candidates;
               4                                 20   
182   ***      2     50                          11         if ( $expl->{key} ) {
183            2                                  6            MKDEBUG && _d('MySQL chose', $expl->{key});
184            2                                  9            unshift @candidates, grep { $possible{$_} } split(',', $expl->{key});
               3                                 13   
185            2                                  5            MKDEBUG && _d('Before deduping:', join(', ', @candidates));
186            2                                  4            my %seen;
187            2                                  7            @candidates = grep { !$seen{$_}++ } @candidates;
               7                                 31   
188                                                         }
189            2                                  6         MKDEBUG && _d('Final list:', join(', ', @candidates));
190            2                                 25         return @candidates;
191                                                      }
192                                                      else {
...
201            3                    3            29      my ( $self, $dbh, $db, $tbl, $q, $can_insert ) = @_;
202            3                                 10      my $result = 0;
203            3                                 15      my $db_tbl = $q->quote($db, $tbl);
204            3                                 12      my $sql    = "SHOW FULL COLUMNS FROM $db_tbl";
205            3                                  8      MKDEBUG && _d($sql);
206            3                                  7      eval {
207            3                                  6         my $sth = $dbh->prepare($sql);
208            3                                585         $sth->execute();
209            1                                  6         my @columns = @{$sth->fetchall_arrayref({})};
               1                                 28   
210   ***      1     50                          10         if ( $can_insert ) {
...
215                                                         }
216                                                      };
217            3                                 20      if ( MKDEBUG && $EVAL_ERROR ) {
218                                                         _d($EVAL_ERROR);
219                                                      }
220            3                                 23      return $result;
221                                                   }
222                                                   
223                                                   sub get_engine {
224           22                   22           101      my ( $self, $ddl, $opts ) = @_;
225           22                                379      my ( $engine ) = $ddl =~ m/\).*?(?:ENGINE|TYPE)=(\w+)/;
226           22                                 54      MKDEBUG && _d('Storage engine:', $engine);
227   ***     22            50                  111      return $engine || undef;
228                                                   }
229                                                   
...
246                                                   sub get_keys {
247           11                   11            55      my ( $self, $ddl, $opts, $is_nullable ) = @_;
248           11                                 42      my $engine = $self->get_engine($ddl);
249           11                                 35      my $keys   = {};
250                                                   
251                                                      KEY:
252           11                                139      foreach my $key ( $ddl =~ m/^  ((?:[A-Z]+ )?KEY .*)$/gm ) {
253                                                   
254                                                         # If you want foreign keys, use get_fks() below.
255   ***     16     50                          62         next KEY if $key =~ m/FOREIGN/;
256                                                   
257           16                                 36         MKDEBUG && _d('Parsed key:', $key);
258                                                   
...
263   ***     16     50                          79         if ( $engine !~ m/MEMORY|HEAP/ ) {
264           16                                 49            $key =~ s/USING HASH/USING BTREE/;
265                                                         }
266                                                   
267                                                         # Determine index type
268           16                                110         my ( $type, $cols ) = $key =~ m/(?:USING (\w+))? \((.+)\)/;
269           16                                 69         my ( $special ) = $key =~ m/(FULLTEXT|SPATIAL)/;
270   ***     16            33                  173         $type = $type || $special || 'BTREE';
      ***                   50                        
271   ***     16     50     33                   94         if ( $opts->{mysql_version} && $opts->{mysql_version} lt '004001000'
      ***                   33                        
...
275                                                         }
276                                                   
277           16                                122         my ($name) = $key =~ m/(PRIMARY|`[^`]*`)/;
278           16    100                          76         my $unique = $key =~ m/PRIMARY|UNIQUE/ ? 1 : 0;
279           16                                 36         my @cols;
280           16                                 38         my @col_prefixes;
281           16                                 73         foreach my $col_def ( split(',', $cols) ) {
282                                                            # Parse columns of index including potential column prefixes
283                                                            # E.g.: `a`,`b`(20)
284           21                                133            my ($name, $prefix) = $col_def =~ m/`([^`]+)`(?:\((\d+)\))?/;
285           21                                 67            push @cols, $name;
286           21                                 84            push @col_prefixes, $prefix;
287                                                         }
288           16                                 68         $name =~ s/`//g;
289                                                   
290           16                                 35         MKDEBUG && _d('Key', $name, 'cols:', join(', ', @cols));
291                                                   
292           21                                 84         $keys->{$name} = {
293                                                            name         => $name,
294                                                            type         => $type,
...
299           21                                184            is_nullable  => scalar(grep { $is_nullable->{$_} } @cols),
300           16                                 91            is_col       => { map { $_ => 1 } @cols },
301                                                         };
302                                                      }
303                                                   
304           11                                 45      return $keys;
305                                                   }
306                                                   
307                                                   # Like get_keys() above but only returns a hash of foreign keys.
308                                                   sub get_fks {
309            4                    4            41      my ( $self, $ddl, $opts ) = @_;
310            4                                 13      my $fks = {};
311                                                   
312            4                                 37      foreach my $fk (
313                                                         $ddl =~ m/CONSTRAINT .* FOREIGN KEY .* REFERENCES [^\)]*\)/mg )
314                                                      {
315            4                                 25         my ( $name ) = $fk =~ m/CONSTRAINT `(.*?)`/;
316            4                                 23         my ( $cols ) = $fk =~ m/FOREIGN KEY \(([^\)]+)\)/;
317            4                                 25         my ( $parent, $parent_cols ) = $fk =~ m/REFERENCES (\S+) \(([^\)]+)\)/;
318                                                   
319   ***      4    100     66                   41         if ( $parent !~ m/\./ && $opts->{database} ) {
320            1                                  6            $parent = "`$opts->{database}`.$parent";
321                                                         }
322                                                   
323            4                                 17         $fks->{$name} = {
324                                                            name           => $name,
325                                                            colnames       => $cols,
326            4                                 24            cols           => [ map { s/[ `]+//g; $_; } split(',', $cols) ],
               4                                 17   
327                                                            parent_tbl     => $parent,
328                                                            parent_colnames=> $parent_cols,
329            4                                 21            parent_cols    => [ map { s/[ `]+//g; $_; } split(',', $parent_cols) ],
               4                                 34   
330                                                         };
331                                                      }
332                                                   
...
340            1                    1            15      my ( $self, $ddl ) = @_;
341            1                                 16      $ddl =~ s/(^\).*?) AUTO_INCREMENT=\d+\b/$1/m;
342            1                                  7      return $ddl;
343                                                   }
344                                                   
345                                                   sub _d {
