...
---------------------------- ------ ------ ------ ------ ------ ------ ------
...tkit/common/IndexUsage.pm   90.3   62.5   40.0   90.0    0.0   59.0   78.6
IndexUsage.t                  100.0   50.0   40.0  100.0    n/a   41.0   93.8
...
OS:           linux
Start:        Tue May 18 16:28:08 2010
Finish:       Tue May 18 16:28:08 2010

Run:          IndexUsage.t
Perl version: 118.53.46.49.48.46.48
OS:           linux
Start:        Tue May 18 16:28:09 2010
Finish:       Tue May 18 16:28:09 2010
...
17                                                    # ###########################################################################
18                                                    # IndexUsage package $Revision: 6326 $
19                                                    # ###########################################################################
...
37                                                       };
38             1                                 30      return bless $self, $class;
39                                                    }
40                                                    
...
47                                                    sub add_indexes {
48    ***      4                    4      0     25      my ( $self, %args ) = @_;
49             4                                 18      my @required_args = qw(db tbl indexes);
50             4                                 11      foreach my $arg ( @required_args ) {
51    ***     12     50                          51         die "I need a $arg argument" unless $args{$arg};
52                                                       }
53             4                                 18      my ($db, $tbl, $indexes) = @args{@required_args};
54                                                    
55             4                                 19      $self->{tables_for}->{$db}->{$tbl}  = 0;
...
60             4                                 18      foreach my $index ( keys %$indexes ) {
...
70    ***      3                    3      0     14      my ( $self, $db, $tbl ) = @_;
71    ***      3     50     33                   26      die "I need a db and table" unless defined $db && defined $tbl;
72             3                                 13      ++$self->{tables_for}->{$db}->{$tbl};
73                                                    }
74                                                    
...
102   ***      1     50                           7      die "I need a callback" unless $callback;
103                                                   
...
107                                                   
108            1                                  8      DATABASE:
109            1                                  3      foreach my $db ( sort keys %{$self->{indexes_for}} ) {
110            1                                  8         TABLE:
111            1                                  3         foreach my $tbl ( sort keys %{$self->{indexes_for}->{$db}} ) {
112            4    100                          22            next TABLE unless $self->{tables_for}->{$db}->{$tbl}; # Skip unused
113            3                                 12            my $indexes = $self->{indexes_for}->{$db}->{$tbl};
114            3                                  6            my @unused_indexes;
115            3                                 15            foreach my $index ( sort keys %$indexes ) {
116            5    100                          24               if ( !$indexes->{$index}->{cnt} ) { # count of times accessed/used
117            2                                 10                  push @unused_indexes, $indexes->{$index};
118                                                               }
119                                                            }
120            3    100                          13            if ( @unused_indexes ) {
121            2                                 12               $callback->(
122                                                                  {  db  => $db,
123                                                                     tbl => $tbl,
...
16             1                    1            11   use MaatkitTest;
               ...
               1                                 39   
17                                                    
18    ***      1            50      1             7   use constant MKDEBUG => $ENV{MKDEBUG} || 0;
               1                                  2   
               1                                 26   
19                                                    
20             1                    1             6   use Data::Dumper;
               1                                  2   
               1                                  6   
21             1                                  5   $Data::Dumper::Indent    = 1;
22             1                                  3   $Data::Dumper::Sortkeys  = 1;
23             1                                  2   $Data::Dumper::Quotekeys = 0;
24                                                    
25             1                                  7   my $iu = new IndexUsage();
26                                                    
...
45             1                                  9   $iu->add_indexes(db=>'sakila', tbl=>'actor',      indexes=>$actor_idx);
46             1                                  5   $iu->add_indexes(db=>'sakila', tbl=>'film_actor', indexes=>$film_actor_idx );
47             1                                  6   $iu->add_indexes(db=>'sakila', tbl=>'film',       indexes=>$film_idx );
48             1                                  4   $iu->add_indexes(db=>'sakila', tbl=>'othertbl',   indexes=>$othertbl_idx);
49                                                    
50                                                    # Now, we see some queries that use some tables, but not all of them.
51             1                                  5   $iu->add_table_usage(qw(sakila      actor));
52             1                                  4   $iu->add_table_usage(qw(sakila film_actor));
