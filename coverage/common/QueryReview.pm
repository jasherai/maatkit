...
OS:           linux
Start:        Sat Aug 29 15:03:38 2009
Finish:       Sat Aug 29 15:03:38 2009
...
57                                                    #               testing.
58                                                    sub new {
59             1                    1            28      my ( $class, %args ) = @_;
60             1                                  4      foreach my $arg ( qw(dbh db_tbl tbl_struct quoter) ) {
61    ***      4     50                          19         die "I need a $arg argument" unless $args{$arg};
62                                                       }
63                                                    
64             1                                  6      foreach my $col ( keys %basic_cols ) {
65    ***      8     50                          40         die "Query review table $args{db_tbl} does not have a $col column"
66                                                             unless $args{tbl_struct}->{is_col}->{$col};
67                                                       }
68                                                    
69    ***      1     50                           7      my $now = defined $args{ts_default} ? $args{ts_default} : 'NOW()';
70                                                    
...
85                                                          SQL
86             1                                  5      MKDEBUG && _d('SQL to insert into review table:', $sql);
87             1                                  3      my $insert_sth = $args{dbh}->prepare($sql);
88                                                    
89                                                       # The SELECT statement does not need to get the fingerprint, sample or
90                                                       # checksum.
91             1                                  6      my @review_cols = grep { !$skip_cols{$_} } @{$args{tbl_struct}->{cols}};
               8                                 29   
               1                                  5   
92             5                                 21      $sql = "SELECT "
93             1                                  5           . join(', ', map { $args{quoter}->quote($_) } @review_cols)
94                                                            . ", CONV(checksum, 10, 16) AS checksum_conv FROM $args{db_tbl}"
95                                                            . " WHERE checksum=CONV(?, 16, 10)";
96             1                                  3      MKDEBUG && _d('SQL to select from review table:', $sql);
97             1                                  7      my $select_sth = $args{dbh}->prepare($sql);
98                                                    
99             1                                 12      my $self = {
100                                                         dbh         => $args{dbh},
...
106                                                         ts_default  => $now,
107                                                      };
108            1                                 16      return bless $self, $class;
109                                                   }
110                                                   
111                                                   # Tell QueryReview object to also prepare to save values in the review history
112                                                   # table.
113                                                   sub set_history_options {
114            1                    1            84      my ( $self, %args ) = @_;
115            1                                  6      foreach my $arg ( qw(table dbh tbl_struct col_pat) ) {
116   ***      4     50                          18         die "I need a $arg argument" unless $args{$arg};
117                                                      }
118                                                   
...
121            1                                  3      my @cols;
122            1                                  4      my @metrics;
123            1                                  3      foreach my $col ( @{$args{tbl_struct}->{cols}} ) {
               1                                  5   
124           29                                230         my ( $attr, $metric ) = $col =~ m/$args{col_pat}/;
125   ***     29    100     66                  210         next unless $attr && $metric;
126           27    100                         114         $attr = ucfirst $attr if $attr =~ m/_/; # TableParser lowercases
127           27                                 80         push @cols, $col;
128           27                                123         push @metrics, [$attr, $metric];
129                                                      }
130                                                   
131           29                                119      my $sql = "REPLACE INTO $args{table}("
132                                                         . join(', ',
133           27    100    100                  218            map { $self->{quoter}->quote($_) } ('checksum', 'sample', @cols))
134                                                         . ') VALUES (CONV(?, 16, 10), ?, '
135                                                         . join(', ', map {
136                                                            # ts_min and ts_max might be part of the PK, in which case they must
137                                                            # not be NULL.
138            1                                  8            $_ eq 'ts_min' || $_ eq 'ts_max'
139                                                               ? "COALESCE(?, $self->{ts_default})"
140                                                               : '?'
141                                                           } @cols) . ')';
142            1                                  9      MKDEBUG && _d($sql);
143                                                   
144            1                                  3      $self->{history_sth}     = $args{dbh}->prepare($sql);
145            1                                 10      $self->{history_cols}    = \@cols;
146            1                                  7      $self->{history_metrics} = \@metrics;
147                                                   }
148                                                   
...
153            2                    2            72      my ( $self, $id, $sample, %data ) = @_;
154                                                      # Need to transform ts->min/max into timestamps
155            2                                  9      foreach my $thing ( qw(min max) ) {
156   ***      4    100     66                   61         next unless defined $data{ts} && defined $data{ts}->{$thing};
157            2                                 11         $data{ts}->{$thing} = parse_timestamp($data{ts}->{$thing});
158                                                      }
159           54                                996      $self->{history_sth}->execute(
160                                                         make_checksum($id),
161                                                         $sample,
162            2                                 14         map { $data{$_->[0]}->{$_->[1]} } @{$self->{history_metrics}});
               2                                 10   
163                                                   }
164                                                   
165                                                   # Fetch information from the database about a query that's been reviewed.
166                                                   sub get_review_info {
167            1                    1            20      my ( $self, $id ) = @_;
168            1                                  7      $self->{select_sth}->execute(make_checksum($id));
169            1                                  9      my $review_vals = $self->{select_sth}->fetchall_arrayref({});
170   ***      1     50     33                   17      if ( $review_vals && @$review_vals == 1 ) {
171            1                                  7         return $review_vals->[0];
172                                                      }
...
184            9                    9           500      my ( $self, %args ) = @_;
185           54    100                         520      $self->{insert_sth}->execute(
186                                                         make_checksum($args{fingerprint}),
187                                                         @args{qw(fingerprint sample)},
188            9                                 62         map { $args{$_} ? parse_timestamp($args{$_}) : undef }
...
193                                                   sub review_cols {
194            1                    1            19      my ( $self ) = @_;
195            1                                  4      return grep { !$skip_cols{$_} } @{$self->{tbl_struct}->{cols}};
               8                                 37   
               1                                 10   
196                                                   }
197                                                   
