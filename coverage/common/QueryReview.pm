...
OS:           linux
Start:        Fri Jul 31 18:53:15 2009
Finish:       Fri Jul 31 18:53:15 2009
...
57                                                    #               testing.
58                                                    sub new {
59             1                    1            30      my ( $class, %args ) = @_;
60             1                                  5      foreach my $arg ( qw(dbh db_tbl tbl_struct quoter) ) {
61    ***      4     50                          20         die "I need a $arg argument" unless $args{$arg};
62                                                       }
63                                                    
64             1                                  6      foreach my $col ( keys %basic_cols ) {
65    ***      8     50                          43         die "Query review table $args{db_tbl} does not have a $col column"
66                                                             unless $args{tbl_struct}->{is_col}->{$col};
67                                                       }
68                                                    
69    ***      1     50                           8      my $now = defined $args{ts_default} ? $args{ts_default} : 'NOW()';
70                                                    
...
85                                                          SQL
86             1                                  2      MKDEBUG && _d('SQL to insert into review table:', $sql);
87             1                                  2      my $insert_sth = $args{dbh}->prepare($sql);
88                                                    
...
97             1                                  2      my $select_sth = $args{dbh}->prepare($sql);
98                                                    
99             1                                 14      my $self = {
100                                                         dbh         => $args{dbh},
...
106                                                         ts_default  => $now,
107                                                      };
108            1                                 15      return bless $self, $class;
109                                                   }
110                                                   
...
116   ***      4     50                          20         die "I need a $arg argument" unless $args{$arg};
117                                                      }
118                                                   
...
123            1                                  3      foreach my $col ( @{$args{tbl_struct}->{cols}} ) {
               1                                  6   
124           29                                242         my ( $attr, $metric ) = $col =~ m/$args{col_pat}/;
125   ***     29    100     66                  244         next unless $attr && $metric;
126           27    100                         128         $attr = ucfirst $attr if $attr =~ m/_/; # TableParser lowercases
127           27                                 83         push @cols, $col;
128           27                                134         push @metrics, [$attr, $metric];
129                                                      }
130                                                   
131           29                                130      my $sql = "REPLACE INTO $args{table}("
132                                                         . join(', ',
133           27    100    100                  235            map { $self->{quoter}->quote($_) } ('checksum', 'sample', @cols))
...
153            2                    2            71      my ( $self, $id, $sample, %data ) = @_;
154                                                      # Need to transform ts->min/max into timestamps
155            2                                  8      foreach my $thing ( qw(min max) ) {
156   ***      4    100     66                   85         next unless defined $data{ts} && defined $data{ts}->{$thing};
157            2                                 12         $data{ts}->{$thing} = parse_timestamp($data{ts}->{$thing});
158                                                      }
159           54                               1132      $self->{history_sth}->execute(
160                                                         make_checksum($id),
161                                                         $sample,
162            2                                 12         map { $data{$_->[0]}->{$_->[1]} } @{$self->{history_metrics}});
               2                                 10   
163                                                   }
164                                                   
165                                                   # Fetch information from the database about a query that's been reviewed.
166                                                   sub get_review_info {
167            1                    1            18      my ( $self, $id ) = @_;
168            1                                  7      $self->{select_sth}->execute(make_checksum($id));
169            1                                  8      my $review_vals = $self->{select_sth}->fetchall_arrayref({});
170   ***      1     50     33                   16      if ( $review_vals && @$review_vals == 1 ) {
171            1                                  8         return $review_vals->[0];
172                                                      }
...
184            9                    9           254      my ( $self, %args ) = @_;
185           54    100                         580      $self->{insert_sth}->execute(
