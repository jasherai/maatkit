...
my $o  = new OptionParser(
   strict      => 0,
   prompt      => '[OPTION...] HOST FILE',
   description => q{executes query in FILE on HOST.},
);
$o->get_specs();
$o->get_opts();
...
my $query = load_file(shift @ARGV);
print "Read query: $query\n";

print "Executing query on ", $dp->as_string($dsn), "...\n";
...
print "DBI::selectall_arrayref()\n";
my $r;
eval {
   $r = $dbh->selectall_arrayref($query);
};
if ( $EVAL_ERROR ) {
   print "Error: $EVAL_ERROR\n";
}
else {
   print "No error; results: ", Dumper($r);
}
